"""Bounded descriptor-based key reads for runtime and release tooling."""
...
def read_key_file(path: Path, *, private: bool) -> bytes:
    """Read one regular key file without following links or special files.

    Private input must be owned by the current effective user with exact
    mode ``0600``. The bounded nonblocking descriptor prevents a configured
    FIFO, device, or oversized file from stalling or exhausting startup.
    """
