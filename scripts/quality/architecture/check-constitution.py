...
REQUIRED_PHRASES: Mapping[str, tuple[str, ...]] = {
    ...
    "docs/roadmap/decisioning/escalation-and-standing-authority.md": (
        ...
        "Handover suspends until reconfirmed",
...
IMPLEMENTED_PROOF_TESTS: Mapping[str, tuple[str, ...]] = {
    ...
    "FDAI-CONST-005": (
        ...
        "services/core-control-plane/tests/scenarios/test_v2026_10_outcomes.py::test_arb_outcome_binds_approval_conditions_and_post_change_verification",
        "services/core-control-plane/tests/scenarios/test_v2026_10_outcomes.py::test_finops_outcome_requires_realized_savings_and_reliability",
        "services/core-control-plane/tests/scenarios/test_v2026_10_outcomes.py::test_dr_outcome_binds_integrity_and_measured_rto_rpo",
        "services/core-control-plane/tests/scenarios/test_v2026_10_outcomes.py::test_chaos_outcome_requires_human_approval_continuous_guards_and_recovery",
    ),
    "FDAI-CONST-006": (
...
def _validate_traceability(root: Path) -> list[str]:
    ...
    for item in requirements:
        ...
        if status == "implemented":
            required_proofs = IMPLEMENTED_PROOF_TESTS.get(requirement_id)
            if required_proofs is None:
                errors.append(
                    f"{TRACEABILITY_MANIFEST}: {requirement_id} implemented status "
                    "requires registered proof tests"
                )
                continue
            listed_tests = item.get("tests")
            if not isinstance(listed_tests, list):
                continue
            for proof_ref in required_proofs:
                proof_path = proof_ref.partition("::")[0]
                if proof_path not in listed_tests:
                    errors.append(
                        f"{TRACEABILITY_MANIFEST}: {requirement_id} required proof test "
                        f"is not listed: {proof_ref}"
                    )
                elif not _test_selector_exists(root, proof_ref):
                    errors.append(
                        f"{TRACEABILITY_MANIFEST}: {requirement_id} missing proof selector: "
                        f"{proof_ref}"
                    )
    return errors
