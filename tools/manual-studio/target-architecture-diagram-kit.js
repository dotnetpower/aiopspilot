...
export function archLegend(items) {
  return `<div class="ta-arch-legend" aria-label="연결선 범례">${items.map(({ kind, label }) => `<span><i class="ta-legend-line ta-link-${kind}" aria-hidden="true"></i>${label}</span>`).join("")}</div>`;
}
