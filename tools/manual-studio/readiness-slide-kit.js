...
import { icon } from "./readiness-diagrams.js";
...
export const chapters = [
  "질문과 범위", "데이터 준비도", "AI 검증과 운영", "근거 기반 성숙도", "다음 30일의 결정",
];

const stateLabels = {
  GUIDE: "진단 관점",
  PROPOSAL: "워크숍 제안",
  EXAMPLE: "설명을 위한 예시 / 운영 근거 아님",
  CONTRACT: "FDAI 설계 기준",
  STATUS: "구현과 운영 근거 구분",
};
...
export function evidenceLine(keys) {
  return `<small class="rm-source" title="${keys.map(key => sources[key]).join(" | ")}">근거: ${keys.map(key => sourceLabels[key]).join(" / ")}</small>`;
}
...
export function slide({ id, chapter, title, lead, visual, body, takeaway, evidence, state = "GUIDE" }) {
  return {
    ...
    content: `<div class="rm-meta"><span class="rm-state" data-state="${state}">${stateLabels[state]}</span><span class="rm-chapter-markers" aria-label="5개 장 중 ${chapter}번째">${chapters.map((_, index) => `<i class="${index + 1 === chapter ? "is-current" : ""}" aria-hidden="true"></i>`).join("")}</span></div>
      <section class="rm-visual rm-${visual}" aria-label="${title}">${body}</section>
      <p class="rm-takeaway"><span>핵심</span>${takeaway}</p>
