...
export function evidenceLine(keys) {
  ...
  return `<small class="vp-source" title="${paths.join(" | ")}">근거: ${labels.join(" / ")}</small>`;
}
  ...
  return {
    ...
    content: `
      <div class="vp-meta">
        ...
        <span class="vp-progress" aria-label="5개 장 중 ${chapter}번째 장">${chapters.map((name, position) => `<i class="${position + 1 === chapter ? "is-current" : ""}"><b>0${position + 1}</b>${name}</i>`).join("")}</span>
      </div>
      <section class="vp-visual vp-${id}" aria-label="${title}">${body}</section>
      <p class="vp-takeaway"><span>결정 원칙</span>${takeaway}</p>
