...
const statusLabels = {
  FOUNDATION: "기반", HISTORY: "역사", PRINCIPLE: "원칙", PROBLEM: "문제",
  BOUNDARY: "경계", CURRENT: "현재 구현", GAP: "진행 중", ILLUSTRATIVE: "예시", DECISION: "다음 단계",
};
...
export function evidenceLine(items, label) {
  ...
  return `<small class="ontology-evidence-source" title="${items.join(" | ")}">근거: ${label ?? [primary, ...compact].join(" · ")}</small>`;
}
