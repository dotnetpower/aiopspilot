...
export function dimensionMap(items) {
  ...
  }).join("")}${node("decision", "ASSESSMENT SUBJECT", "하나의 운영 판단", "같은 범위와 근거로\n여섯 역량을 검토", "target", "grid-column:3;grid-row:1 / 4")}</div>`;
}
...
export function reviewCycle(steps) {
  return `<div class="rm-graph rm-review-cycle">${node("detect", ...steps[0], "grid-column:1;grid-row:1")}
    ...
    <span class="rm-cycle-label">변경마다 새 검토</span></div>`;
}
...
export function maturityPosition(level) {
  if (level === null) return '<div class="rm-category-axis is-unassessed" data-level="unassessed"><span>미평가 / 근거 요청</span></div>';
  return `<div class="rm-category-axis" data-level="${level}" aria-label="현재 판정 ${level}">${[1, 2, 3, 4, 5].map(index =>
...
export function agendaRing(agenda) {
  ...
  let elapsed = 0;
  return `<div class="rm-agenda-ring" role="img" aria-label="제안 워크숍 ${total}분: ${agenda.map(item => `${item.activity} ${item.minutes}분`).join(", ")}">
    <svg viewBox="0 0 240 240" aria-hidden="true">${agenda.map((item, index) => {
      ...
      return arc;
    }).join("")}</svg><div><strong>${total}</strong><span>분 / 제안</span></div></div>`;
}
