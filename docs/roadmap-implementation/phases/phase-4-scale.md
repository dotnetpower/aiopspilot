...
| Pattern intake and temporal holdout | implemented | `core/measurement/pattern_growth.py`; `delivery/measurement/holdout.py`; focused holdout and runner tests | The growth composition requires a complete post-learning holdout envelope before a shadow candidate can be written. Accepted and rejected outcomes are audited with no promotion or execution authority. |
| Model cost and quality swap policy | implemented | `core/measurement/model_tracking.py`; `delivery/measurement/measured_policy.py`; focused model and measured-policy tests | Durable paired cost/quality batches produce review-only recommendations. No model or enforce binding is changed, and rollback remains an explicit no-authority record. |
| Per-tier latency budget policy | implemented | `core/measurement/latency_budget.py`; `delivery/measurement/measured_policy.py`; focused latency and measured-policy tests | Durable T0/T1/T2 rows retain server-owned budgets, producer-reported budgets for comparison, volume, p50/p95/p99, and explicit unavailable outcomes. Missing budget and insufficient samples no longer appear as pass. |
| Measurement runner CLI and delivery adapters | implemented | `delivery/measurement_runner_cli.py`; `delivery/measurement/{holdout,measured_policy,scenario_replayer}.py`; focused CLI, runner, and runtime-image smoke tests | Baseline resolves the service-owned frozen scenario assets shipped in the image, drains durable model/latency evidence, and growth enforces complete holdout. Atomic processed records make restart, duplicate, and malformed input idempotent without blocking later batches; partial, stale, future, rollback, and unavailable inputs remain audited. Scheduled live receipts remain incomplete. |
...
|------|-------|--------|----------|-----------|
| 2026-09-09 | implemented | Aligned baseline runtime discovery with the service-owned frozen-scenario path already shipped by the Core image. Missing assets still fail closed, while an image smoke replay now proves the deployed layout is discoverable. | `current change`; `delivery/{measurement_runner_cli.py,measurement/scenario_replayer.py}`; focused CLI and runner tests; local Core image discovery and replay smoke. | Publish the corrected image, retain successful scheduled executions, and build the separately governed reference/treatment cohort receipt before making a claim. |
| 2026-08-31 | implemented | Bound temporal holdout, paired model cost/quality, and per-tier latency evidence into the existing growth and baseline jobs. Complete holdout is required before shadow pattern write. Model swap remains review-only, latency preserves unavailable state, and restart, duplicate, partial, stale, future, and rollback paths converge through durable no-authority audit. | `current change`; measurement reducers, delivery holdout and measured-policy runners, CLI composition, and focused Core/delivery/CLI checks. | Retain protected scheduled-run and claim-eligible Azure cohort receipts under issue #352; none is claimed by this local transition. |
...
- [x] Bind temporal holdout, model-swap, and latency policies into durable measured runners under [issue #353](https://github.com/dotnetpower/fdai/issues/353), then pass focused holdout, model, latency, restart, duplicate, partial, stale, unavailable, and rollback tests.
