...
| Governance artifact separation | in-progress | `rule_catalog/schema/{governance_catalog,override,exemption_lifecycle,parameter_relaxation_policy}.py`; `delivery/catalog_exemption.py`; current change | Assignments, overrides, and exemptions remain validated catalog-as-code inputs outside ontology facts. Exact-scope resolution and expiry evidence grant no query, approval, or execution authority. Complete focused governance results remain to be recorded. |
| Bounded resource-state query semantics | in-progress | `core/conversation/semantic_resource_state_planning.py`; `core/ontology_platform/resource_state_queries.py`; focused test paths; current change | Concrete catalog state concepts override the observed sentinel, while empty and incomplete results retain explicit scope and source limitations. Complete focused semantic test results remain to be recorded. |
| Bounded ObjectSet candidate scan | validated | `OntologyInstanceStore.scan_objects`; PostgreSQL and in-memory adapters; `object_sets.py`; focused checks and authenticated recent-transition replay | Object-only predicates use a 1,000-object probe and at most one 50,000-object, one-connection candidate snapshot. Relationship-bearing queries remain on the ordinary bounded path, and candidate truncation stays incomplete. |
