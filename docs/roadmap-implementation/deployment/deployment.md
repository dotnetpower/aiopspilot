...
| Single-maintainer direct dev apply | implemented | `.github/workflows/service-deploy.yml`, `.github/workflows/deploy-dev.yml`, `verify-github-environment.py`, and focused verifier and workflow tests | An explicit zero-review dev policy validates the Environment while preserving exact-plan, attestation, identity, health, and rollback gates. |
| Bounded database host binding | implemented | `.github/workflows/service-deploy.yml`, `guard_plan.py`, `plan_bundle.py`, `deployment_recovery.py`, and focused service-deploy tests | The workflow requires successful exact-commit CI before planning or applying. Post-apply verification compares the complete primary runtime contract, including the single non-secret host binding and readiness probes, with the sealed plan. Governed apply evidence remains open. |
...
|------|-------|--------|----------|-----------|
| 2026-09-09 | implemented | Required exact-commit CI inside every service deployment, sealed a provider-normalized primary runtime contract, and made post-apply verification reject host, probe, command, argument, environment, or resource drift. | `current change`; 347 focused deployment, graph, OI-12, and assurance checks; Ruff, strict mypy, workflow validation, design-route validation, and Console typecheck. | Complete the five protected host-binding plans and applies, then retain authoritative graph and authenticated evidence in Issue #262. |
| 2026-09-07 | implemented | Added a direct dev apply policy for single-maintainer repositories without weakening staging, production, or bot-owned approval boundaries. | `current change`; focused verifier and deployment workflow tests | Retain one successful direct dev apply and post-apply projection readback. |
