...
# 에이전트 기반 자동화(Agent-driven 자동화)

FDAI의 에이전트가 cloud operations를 수행합니다. 에이전트는 타입이 지정된 이벤트로 관측, 판단, 계획,
승인, 실행, 검증, 복구, 감사, 학습을 수행합니다. **온톨로지**는 에이전트의 공유 정확성
인프라입니다. 에이전트가 사용할 서비스, 리소스, 목표, 근거, 관계, 허용
액션을 정의하지만 그래프 자체를 행위자 또는 실행 표면으로 만들지 않습니다.

이 페이지에서는 에이전트가 세 가지 선언 타입을 사용하는 방식, 배포 관측이
범위가 제한된 맥락이 되는 방식, `ActionType` 제안이 통제된 컨트롤 루프를 통과하는 과정을
설명합니다.

> **권한 경계:** 온톨로지는 의미를 제공하는 읽기 모델입니다. 이벤트, 승인된 구성,
> 텔레메트리 프로바이더, catalog-as-code, 추가 전용 감사 원장은 각자 담당하는 사실의
> 권위 있는 원본으로 유지됩니다.
>
> **안전 경계:** 온톨로지 컨텍스트는 자율성을 유지하거나 낮출 수만 있습니다. 컨텍스트가
> 누락되거나 오래됐거나 충돌하거나 근거가 부족하면 범위가 제한된 근거 복구, 더 작은 safe
> 계획, no-op 또는 검토를 유발하며 실행 권한을 부여하지 않습니다.
...
관계 같은 런타임 인스턴스는 승인된 배포 소스에서 변환 결과되고 공유 온톨로지 프로바이더를
통해 저장됩니다. 선언은 유효한 의미를 기술하고, 인스턴스는 한 배포에서 무엇이 존재하거나
발생했는지 기록합니다.

배포되는 모든 선언에는 출처가 포함됩니다. 카탈로그 로더는 내용 해시를 다시 계산하고
...
이 구조로 쓰기 책임을 분명히 하고 엔드포인트를 결정론적으로 검증합니다.
...
- **영향 범위:** `blast_radius`가 선언된 범위를 제한하고, 실시간 탐색이 범위를 더 낮출 수
  있습니다.
...
네 가지 작업 범주는 `remediation`, `ops`, `governance`, `tool`입니다. 도구 작업은
...
- **규칙 위반:** 컨트롤 루프가 일치한 규칙, 발견된 문제, 리소스, 타입 계약으로 인스턴스를
  만듭니다.
- **운영자 요청:** 작업의 쓰기 조정기가 활성화된 경우 콘솔이 형식화된 의도, 요청자,
...
| 인스턴스 변환 결과 | 최신 서비스, 워크로드, 목표, 리소스, 관계 인스턴스 제공 | 컨텍스트를 알 수 없음 또는 오래됨으로 표시하고 자율성을 낮춤 |
| 조정기 또는 디스패처 | 허용된 트리거를 범위가 제한된 작업 인스턴스로 변환 | 트리거를 차단하거나 관찰 모드로 유지 |
| 실행 프로바이더 | `pr_native`, `direct_api`, `pr_manual`, `tool_call` 구현 | 변경을 적용하지 않고 사유를 감사에 기록 |
| 전달 및 감사 | 효과를 전달하고 모든 종료 경로 기록 | 작업을 불완전 상태로 처리하며 성공으로 간주하지 않음 |

이 분리를 통해 다운스트림 배포판은 코어 엔진을 바꾸지 않고 지원되는 조립 확장
지점에서 선언과 프로바이더 구현을 추가할 수 있습니다. YAML 파일만으로 권한 있는 클라우드
통합이 만들어지지는 않습니다.
...
4. **안전성 검토:** 정책 리스크와 타입의 티어 상한, 영향 범위, 호출자 역할, 환경, 승격 상태,
   컨트롤 플레인 상태를 결합합니다.
5. **실행 및 전달:** 실행기가 리소스별 잠금을 확보하고 중단 및 복구 계약을 지키면서 선택된
   실행 경로를 적용합니다.
6. **감사 및 관측:** 미실행, 승인, 차단, 시간 초과, 실행 시도, 최종 증적, 독립적으로 관측된
   효과를 기록합니다.

감사 레코드에는 최종 상한과 근거 참조가 포함됩니다. 이를 통해 작업을 허용하거나 보류하거나
차단한 이유를 증명하고 당시 시점 기준으로 재생할 수 있습니다.
...
일상적인 표현으로 재고 질문을 하면 FDAI는 의미를 단정하지 않고 해석안을 제안합니다. 단어 일치,
임베딩, 모델 추론이 질문의 뜻을 제안할 수 있지만, 그 제안은 검증되기 전까지 **후보**로
남습니다.

각 후보는 무엇을 조회하려 했고 무엇을 근거로 판단했는지 함께 기록합니다.

| 후보가 기록하는 내용 | 필요한 이유 |
|----------------------|-----------|
| 요청한 작업과 그 작업이 가리키는 온톨로지 타입 | FDAI가 무엇을 조회할지 그대로 확인할 수 있습니다 |
| 질문 문장에서 추론한 인자 | 잘못된 가정을 결과가 나오기 전에 발견할 수 있습니다 |
| 판단에 사용한 카탈로그 버전 | 같은 질문을 나중에 같은 의미로 다시 실행할 수 있습니다 |

후보는 일치도가 아무리 높아 보여도 실행 권한을 얻지 못합니다. 다음과 같은 경우 FDAI는 답을
만들지 않고 판단을 보류한 다음 확인을 요청합니다.

- 질문에 쓰인 표현이 카탈로그의 어떤 항목과도 연결되지 않을 때
- 후보를 만든 뒤에 카탈로그가 바뀌었을 때
- 대상이 요청한 작업에 유효하지 않을 때

카탈로그와 정확히 일치하는 검토된 근거만이 후보를 확정된 해석으로 바꿉니다. 확정된 뒤에도
작업 해석은 여전히 제안입니다. 다른 `ActionType` 인스턴스와 똑같이 통제된 파이프라인을
통과해야 실행이 시작됩니다.

예시: "payments 서비스의 데이터베이스를 보여주세요"라고 물었는데 `payments-db-2`가 지난주에
이름이 바뀌었습니다. FDAI는 예전 이름을 확인되지 않은 표현으로 보고하고 어떤 서비스를
말하는지 되묻습니다. 오래된 이름을 근거로 답하지 않습니다.
...
| 핵심 내용 | 타입, 관계, 제약, 수명주기, 출처, 경우에 따라 추론 규칙 | 노드, 간선, 속성, 인덱스, 질의 언어, 영속성 동작 |
| 정확성의 의미 | 도메인에서 어떤 개념과 관계가 유효한지 규정 | 저장된 그래프가 데이터베이스 스키마와 트랜잭션 규칙을 따르도록 보장 |
| 저장소 의존성 | 관계형 테이블, 문서, RDF 저장소, 메모리, Graph DB 모두 사용 가능 | 도메인 온톨로지가 거의 없거나 전혀 없는 데이터도 저장 가능 |
...
FDAI는 측정된 전달 경로가 제한된 교집합과 짧은 순회이므로 현재 관계형 인덱스를 사용합니다.
향후 다중 홉 워크로드가 필요성을 입증하면 전용 그래프 엔진을 선택할 수 있지만, 이는 구현
