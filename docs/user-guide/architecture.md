...
<fdai-architecture-diagram manifest="../diagrams/generated/fdai-azure-resource-network-flow.manifest.json" locale="en" style="display:block">
  <img src="../diagrams/generated/fdai-azure-resource-network-flow.en.svg" alt="An operator signs in through Microsoft Entra ID and uses the FDAI Web Console on Azure Static Web Apps. A planned private Application Gateway protected by a WAF policy routes requests to the Operator Service and Document Ingestion API. A VNet-integrated Container Apps environment runs the Core Control Plane, Operator Service, Document Ingestion API, Document Processing Worker, Isolated Executor, and scheduled jobs. The Core has no effect identity, and the Isolated Executor alone may hold explicitly approved effect roles. Azure Event Hubs, Container Registry, Key Vault, Azure OpenAI, Microsoft Foundry, Azure Database for PostgreSQL, and optional ADLS Gen2 storage connect through dedicated private endpoints. Azure Resource Graph supplies inventory, Application Insights and Log Analytics receive telemetry, and Azure Managed Grafana reads monitoring data. Email, Teams, and Slack carry human approvals. GitHub, GitLab, and Azure DevOps receive governed remediation pull requests." loading="lazy" style="display:block;width:100%;height:auto" />
</fdai-architecture-diagram>
...
gateway and APIM remain in their feature-specific profiles.
...
|--------------------------------------|-------------------|--------|
| Operational Event Hubs namespace and its private endpoint | Aggregate under Event Hubs | It uses the same Event Hubs and private-link symbols; the node description covers primary and operational topics |
| Case-history Blob account and its private endpoint | Keep documented | It uses the same Storage Account and Blob private-endpoint symbols, so repeating the pair adds clutter without a new interaction pattern |
...
No extra nodes are required for these resources. They and the other supporting
resources remain intentional abstractions rather than missing architecture.
