...
translation_source_sha: 98b6b91adf3b95c3703279be1eafce3f07c7d08a
...
- 검토된 블루/그린 후보를 승격할 때는 VM과 네트워크 인터페이스를 Bootstrap 상태로 가져오기
  ...
  특수화 VM의 공개 SSH 입력을 보호된 호스트에서 복구한 뒤 구조화된 드리프트 작업이 없는지 검증합니다. Bootstrap은 명시적으로 검토한 교체 전까지 채택한 이미지
  참조를 보존합니다.
