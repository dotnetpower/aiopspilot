...
배포합니다. Workflow는 tenant 값을 GitHub 구성에 유지하고 Microsoft Graph에는 배포 전용
OIDC(OpenID Connect) 신원을 사용하며 런타임 신원에는 app catalog 또는 실행 권한을 주지
않습니다.
...
- 필수 CI를 통과하고 attestation을 받은 `fdai-system-knowledge-service` 이미지가 있는 clean
  protected-main 개정
- 배포 전용 installer 신원을 초기화할 tenant 관리 권한
...
| Secret | 내용 |
|--------|------|
| `SYSTEM_KNOWLEDGE_TEAMS_PROFILE_JSON` | 서비스 이름, Bot 이름, tenant, 승인된 team과 channel 목록, Bot service URL allowlist 및 JWKS URL |
| `SYSTEM_KNOWLEDGE_PRINCIPAL_MAP_JSON` | Teams sender `aadObjectId`와 배포 로컬 지식 principal의 매핑 |

Profile은 다음 형태를 사용합니다.
...
입력에 유지되며 접근이 제한된 Terraform state에 나타납니다.
...
Plan에 전용 UAMI, 비공개 claim container, 최소 role assignment 세 개, replica 하나의 Container
App, F0 Azure Bot, Teams channel 및 `execution_authority=false` 계약만 있을 때 수락합니다.
...
5. 결정적 Teams package를 build합니다.
6. 배포 전용 OIDC 신원으로 package를 업로드하고 설치합니다.

## Channel 검증

승인된 표준 channel에서 다음을 확인합니다.

1. `FDAI Knowledge`를 멘션하고 FDAI가 작업을 안전하게 유지하는 방법을 묻습니다.
