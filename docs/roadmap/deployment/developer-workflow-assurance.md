...
| 2 | Transport retry delay | One fixed 60-second sleep per retryable failure | Derive a bounded exponential delay clamped to a declared maximum. |
| 3 | Assurance stall detection | One 4-hour envelope with no per-turn or per-question deadline | Fail a stalled turn in 3 minutes and a stalled question in 5 minutes. |
| 4 | Assurance resume | An external termination discarded every completed question | Persist a checkpoint bound to source, workspace, target stack, and evidence identity, and resume the outstanding questions. |
...
| 8 | Roadmap service envelope | `TimeoutStartSec=5h` and `2h` exceeded the real stage sum | Bound each unit just above its declared stage budgets. |
...
| 24 | Silent deploy and developer waits | The 900-second Container App health poll emitted nothing and then fell through its expired deadline as if it had converged, auto-pull ran `git fetch` and `git pull --rebase` with no timeout inside a 180-second loop, seven retrying workflow downloads declared a retry count but no cumulative window, and a resumed assurance cohort disclosed its restored turn count only in the final artifact | Emit one progress line per health poll and fail on the health deadline, bound every auto-pull remote call below its interval, declare `--retry-max-time` on every retrying download, and print one resume line before the cohort loop. |
| 25 | Unbounded validation stage | A validation stage ran under `Popen` and `wait()` with no deadline at all, so one hung gate blocked the queue and every push behind it indefinitely; round 24 also introduced a `timeout` around `git pull --rebase` that could leave a half-finished rebase in a developer tree, and the readiness probe retried inside a 60-second envelope with no per-request cap | Kill a stage that stops producing output for 30 minutes or outlives its 4-hour budget, report that kill as a distinct status instead of blaming the commit, advance a strictly-behind branch with a local fast-forward, and declare the readiness retry window. |
| 26 | Shared ref and late watchdog | Auto-pull compared and advanced against the default fetch head, which every process on the Git common directory overwrites, so a concurrent fetch could advance the branch onto an unrelated ref; the round 25 watchdog could also mark a stage killed after it had already exited successfully | Fetch into the per-branch remote-tracking ref and compare and advance against it, skip a kill once the stage has exited, and honor an expiry only when the stage really died from that kill. |
| 27 | Last unbounded validator waits | The validator ran local Git plumbing through a `subprocess.run` with no timeout, the roadmap agent waited without a bound after its own SIGKILL, and a fetch that failed for any reason was reported as a timeout | Bound the Git helper, bound the post-kill wait, and name the fetch failure the loop actually saw. |
| 28 | Deploy jobs with only a runner default | The protected Terraform and service deploy jobs declared no budget, so the six-hour runner default was their only bound and held the single deploy runner meanwhile, and the health verifier ran two unbounded local Python steps after its own deadline | Declare a job budget for both deploy jobs and bound the verifier and readiness-path steps. |
