...
| Fresh-subscription local coordinator | implemented | `genesis-up.sh`; private coordinator, approval, image, Foundation apply, Bastion, enrollment, and state-handoff modules; focused lifecycle tests | The 15-stage route resumes claims by verification only and stops before protected application planning. A governed Azure receipt remains open. |
