...
- **ACR handoff**: upstream GHCR is the generic build-evidence registry. A fork that requires
  ...
  an explicit promotion input, normalizes the Terraform ACR output to its exact Azure login host,
  verifies the ACR digest is identical, and then binds the digest to Terraform. Exact apply cannot
  promote or replace the image recorded in the protected plan.
