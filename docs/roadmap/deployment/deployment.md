...
|------|-------|--------|----------|-----------|
| 2026-09-10 | implemented | Replaced the failing generic `docker login` ping with a mode-0600 temporary Docker auth config consumed only by exact OCI attestation verification. The config is removed at step exit and does not change image promotion authority. | Failed protected plans `34425662989` and `34425764304`; focused credential-hygiene and workflow tests. | Rerun the exact runtime-image-bound plan, review all retained changes and cost-sensitive resources, and stop before apply. |
