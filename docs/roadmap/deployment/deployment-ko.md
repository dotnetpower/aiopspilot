...
translation_source_sha: bb5905dbe6cd51f06a57fa5deced55741fe8dd71
...
| 날짜 | 상태 | 변경 | 근거 | 남은 작업 |
|------|------|------|------|-----------|
| 2026-09-10 | implemented | OCI 증명, ACR 조회, 정확한 가져오기, 다이제스트 확인 주변에 내용이 없는 진행 경계와 명시적 실패를 추가했습니다. binder는 가져오기 요청 전에 ACR 리소스 ID를 검증하며 레지스트리, 이미지, 자격 증명, 테넌트 값을 로그에 기록하지 않습니다. | `current change`; 실패한 보호 인증 `34424068277`; 정제한 실행기 ACR 조회 검증; 집중 이미지 연결 및 자격 증명 위생 검사. | 범위가 제한된 진단을 게시하고 정확히 증명된 Core 이미지를 생성한 뒤 통과한 보호 OI-12 증적을 보존합니다. |
...
- **ACR 인계**: 업스트림 GHCR은 범용 build-evidence 레지스트리입니다. ACR이 필요한 포크는
  ...
  Terraform에 연결합니다. 내용이 없는 진행 표시는 배포 값을 노출하지 않으면서 증명, 레지스트리
  조회, 가져오기 수락, 다이제스트 확인을 구분합니다. Exact 적용은 protected 계획에 기록된
  이미지를 promote하거나 교체할 수 없습니다.
  보호된 OI-12 연결은 인벤토리 Job, 이력 Job, 보관 컨테이너 URL의 플랫폼 최상위 출력을 우선
  사용합니다. 배포된 상태가 해당 출력보다 오래된 경우에는 정확한 플랫폼 리소스 그룹 안에서
  일반 ARM Job ID를 최대 64개까지 열거하고, 각 리소스를 안정 Container Apps `2024-03-01`
  API로 최대 30초 동안 조회합니다. 검토된 인벤토리 런타임 계약과 이력 런타임 계약에 일치하는
