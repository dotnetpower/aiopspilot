...
translation_source_sha: 61dc297957c0b5e25a279bfe2042546b0746a6b6
...
| 날짜 | 상태 | 변경 | 근거 | 남은 작업 |
|------|------|------|------|-----------|
| 2026-09-10 | implemented | 실패하는 일반 `docker login` ping을 정확한 OCI 증명 검증에서만 사용하는 mode-0600 임시 Docker 인증 구성으로 교체했습니다. 이 구성은 단계 종료 시 삭제되며 이미지 승격 권한을 바꾸지 않습니다. | 실패한 보호 계획 `34425662989`, `34425764304`; 집중 자격 증명 위생 및 workflow 테스트. | 정확한 런타임 이미지 결속 계획을 다시 실행하고 보존된 모든 변경과 비용 민감 리소스를 검토한 뒤 apply 전에 중단합니다. |
