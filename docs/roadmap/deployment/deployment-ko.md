...
translation_source_sha: 6d9185793b8448cf6d559c4cc2fdd807de4ae139
...
- **ACR 인계**: 업스트림 GHCR은 범용 build-evidence 레지스트리입니다. ACR이 필요한 포크는
  ...
  배포된 상태가 해당 출력보다 오래된 경우에는 정확한 플랫폼 리소스 그룹 안에서 일반 ARM Job
  ID를 최대 64개까지 열거하고, 각 리소스를 안정 Container Apps `2024-03-01` API로 최대 30초
  동안 조회합니다. 검토된 인벤토리 이름, 명령, 빈 인자를 가진 컨테이너가 정확히 하나여야
  합니다. fallback은 이름 패턴으로 Job 신원을 유추하거나 프로바이더 출력을 저장하지 않습니다.
