...
translation_source_sha: bb61247bb36564df79a7d96781cf179ebd558c2a
...
구성 표류도 같은 경계를 따릅니다. 테스트는 합성 파일과 가짜 전송 계층을 사용합니다. 배포에서는
Virtual Network에 통합된 러너가 명시적으로 검토된 스냅샷을 다시 만들고, 정규 다이제스트와 완전한
리소스 수가 일치할 때만 private 콘텐츠 주소 기반 Blob에 저장합니다. 독립 배포되는 Core 서비스의
보호된 계획에는 정확한 Blob URL, 버전, 다이제스트, 범위, 구독, 스칼라 속성 경로만 전달됩니다.
Core는 Managed Identity로 Blob을 읽습니다. 적용 후 검증은 배포된 바인딩과 Blob을 독립적으로 다시
읽고 새 Azure Resource Graph 관측값과 비교한 뒤, 리소스 이름이나 프로바이더 식별자가 없는 정제된
증적을 보존합니다.
...
근거를 요구합니다. 통제된 온톨로지 보증 아티팩트는 정확한 source revision, 정규 run configuration과 digest, workspace patch digest, authentication attestation 및 정확한 request와 projection id를 기록하며, runner는 malformed source 또는 workspace 출처 이력을 첫 요청 전에 거부합니다. 인증된 외부 증적은 표준 `http://localhost:5273` Console과 `http://127.0.0.1:8010` Operator API 원본만 허용합니다. 작업 영역 패치 다이제스트에는 무시 대상이 아닌 추적되지 않은 파일도 포함되므로, 커밋되지 않은 파일이 기록된 출처에서 누락되지 않습니다.
### dev-up.sh 필요 (여전히 로컬)
...
운영 composition은 채널 검증, 영속 HIL 전달 및 IAM 데이터베이스 조립을 `iam_composition.py`에 위임하고 로컬 identity factory는 기존 authentication 모듈에 유지합니다. 따라서 composition root는 검토된 fanout 상한 아래를 유지합니다. Azure Resource Manager 토큰은 API 프로세스 안에 유지합니다. Psycopg를 직접 사용하는 저장소는
...
Workspace는 `.github/workflows/deploy-dev.yml`과 로컬 복합 액션 메타데이터를 plain YAML로 처리하고 로컬 `action.yml` 및 `action.yaml` 파일에 GitHub Action JSON 스키마를 적용합니다.
확장 진단 없이 편집기 자동 완성과 구조 검증을 사용할 수 있으며 원격 태그 검사, 저장소 워크플로 계약, GitHub Actions 런타임이 최종 근거입니다.
