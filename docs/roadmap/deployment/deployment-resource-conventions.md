...
| Channel-edge identity substrate | implemented | `infra/main.tf`; `infra/services/operator-service/`; `deploy-channel-edge-secrets.yml`; root, service, and protected secret workflow checks | Platform grants the dedicated edge identity access to the Operator DSN. Additional provider/principal secret scopes remain optional platform inputs, while the service root rejects an enabled edge without principal scopes and one complete provider contract. The exact-green development secret workflow selects one tagged deployment-owned Key Vault, uses the stable deploy identity, writes only four fixed Slack secret names, verifies value readback, and retains no value or identifier artifact. Platform and tenant policy remain authoritative for private-network and RBAC posture. |
