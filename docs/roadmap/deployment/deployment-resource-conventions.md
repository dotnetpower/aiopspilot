...
| Channel-edge identity substrate | implemented | `infra/main.tf`; `infra/services/operator-service/`; `deploy-dev.yml`; `deploy-channel-edge-secrets.yml`; root, service, and protected secret workflow checks | Platform grants the dedicated edge identity access to the Operator DSN. Additional provider/principal secret scopes remain optional platform inputs, while the service root rejects an enabled edge without principal scopes and one complete provider contract. An exclusive platform plan targets only the resource-group state move, dedicated identity, ACR pull, semantic Event Hubs sender/receiver, and fixed secret scopes. The exact-green development secret workflow selects one tagged deployment-owned Key Vault, uses the stable deploy identity, writes only four fixed Slack secret names, verifies value readback, and retains no value or identifier artifact. Platform and tenant policy remain authoritative for private-network and RBAC posture. |
...
| 2026-09-08 | implemented | Added a masked, bounded Operator channel-edge provider binding that accepts only four fixed versionless Key Vault secret resource identifiers plus one Slack workspace identifier. The service materializer derives the deployment name and closed Slack-only defaults, rejects key drift, fixed-name drift, and cross-vault references, and leaves the encrypted base tfvars unchanged. | `current change`; focused service materializer and workflow tests. | Run a create-only Operator plan and retain the exact protected coordinates before apply. |
| 2026-09-08 | implemented | Bound the explicit channel-edge enable or disable transition into service tfvars materialization before Terraform planning instead of passing it only to the plan guard. | Failed service plan `34222085011`; `current change`; focused service workflow tests. | Rerun the Operator enable plan with complete provider bindings and retain a guarded create-only result. |
| 2026-09-08 | implemented | Added an exclusive platform plan target set for the channel-edge identity and its ACR, semantic Event Hubs, Operator DSN, and four provider/principal secret scopes. | `current change`; platform workflow contract tests. | Run a zero-destroy plan and direct single-maintainer dev apply before enabling the Operator workload. |
| 2026-09-08 | implemented | Bound the secret helper to the frozen Operator Service package environment that owns its HTTP dependency instead of the root minimum environment. | Failed workflow `34217394441`; `current change`; focused helper and workflow tests. | Rerun secret materialization and retain the successful value readback result. |
| 2026-09-08 | implemented | Derived the canonical public-Azure Key Vault URI from the unique validated vault name because the runner's list projection omits `properties.vaultUri`. The helper still rejects any URI outside Azure Vault DNS. | Failed workflow `34216680762`; `current change`; focused workflow and helper tests. | Rerun secret materialization and retain the successful value readback result. |
| 2026-09-08 | implemented | Removed duplicate Key Vault posture reads after every exact management-plane read variant was denied on the runner. Unique deployment tags select the vault, the helper validates its Azure Vault URI, and actual data-plane write/readback proves reachability while platform policy remains the posture authority. | Failed workflow `34215982420`; prior value-free posture evidence; `current change`; focused workflow and transport tests. | Rerun secret materialization and retain the successful value readback result. |
| 2026-09-08 | implemented | Removed the runner-time GitHub CLI download from channel secret bootstrap and replaced it with a bounded Python check-runs request using the workflow token only in memory. | Failed workflow `34214928124`; `current change`; focused workflow and CI contract tests. | Rerun secret materialization and retain the successful readback result. |
| 2026-09-08 | implemented | Replaced the runner-denied generic ARM REST lookup with the supported Key Vault name-and-subscription read derived from the unique tag-bound selection. | Failed workflow `34214223069`; direct value-free named Key Vault posture check; `current change`; focused workflow tests. | Rerun secret materialization and retain the successful readback result. |
| 2026-09-08 | implemented | Replaced runner-dependent generic resource lookup with a direct API-versioned ARM REST read while preserving the same value-blind RBAC and private-network assertions. | Failed workflow `34213551618`; direct value-free ARM REST posture check; `current change`; focused workflow tests. | Rerun secret materialization and retain the successful readback result. |
| 2026-09-08 | implemented | Limited exact ARM posture verification to RBAC and public-network access because the generic resource response omits the vault URI already obtained from the unique tag-bound Key Vault list result. | Failed workflow `34212885604`; direct value-free field comparison; `current change`; focused workflow tests. | Rerun secret materialization and retain the successful readback result. |
| 2026-09-08 | implemented | Replaced the unsupported Key Vault CLI `show --id` posture lookup with an exact API-versioned ARM resource read. | Failed workflow `34212165849`; direct value-free ARM posture check; `current change`; focused workflow tests. | Rerun secret materialization and retain the successful readback result. |
| 2026-09-08 | implemented | Split private-vault selection into a tag-bound list lookup and an exact resource readback because the list response omits RBAC and public-network posture fields. | Failed workflow `34211312612`; `current change`; focused workflow and CI contract tests. | Rerun secret materialization and retain the successful readback result. |
| 2026-09-08 | implemented | Restored required-CI verification on a fresh self-hosted runner slot by installing the repository-pinned GitHub CLI before the protected secret workflow uses it. | Failed workflow `34210210151`; `current change`; focused workflow and CI contract tests report 52 passed. | Rerun the exact-green secret workflow and retain its value-blind readback result. |
| 2026-09-08 | implemented | Added a value-blind protected workflow for materializing the fixed Slack channel-edge secrets in the deployment-owned private Key Vault. | `current change`; secret transport, workflow security, runner registration, stable identity, CI contract, and A3 channel tests. | Run the exact-green workflow, bind versionless secret identifiers to the channel-edge platform and service plans, and retain provider delivery and rollback receipts. |
...
| 2026-08-28 | implemented | Exported the OpenAI and conditional Foundry account origins from platform Terraform and delivered the bounded map to measurement jobs and protected independent Core inputs. | `current change`; focused materializer, plan-guard, runtime, root Terraform, and service-root checks. | Activate the reviewed partner registry entry, then retain protected deployment and runtime readback evidence. |
| 2026-08-28 | implemented | Activated the `Mistral-Large-3` secondary fallback at `GlobalStandard` 1K TPM, made quota checks SKU-qualified, and bound the opt-in staging ChatOps fence into protected plan metadata for apply revalidation. | `current change`; focused resolver, quota, request, capability-gate, plan-verifier, sealer, and workflow checks. | Retain the protected plan/apply, model deployment readback, runtime binding, and provider channel receipts. |
| 2026-08-28 | implemented | Replaced the extra ChatOps dispatch boolean with bounded `plan-chatops-*` and `apply-chatops-*` request ids and centralized restored-plan mode matching in the capability gate. | `current change`; focused workflow-budget, request, capability-gate, lifecycle, and plan-verifier checks (`117 passed`). | Retain the protected plan/apply, model deployment readback, runtime binding, and provider channel receipts. |
| 2026-08-28 | implemented | Corrected the ChatOps LLM-enable expression after GitHub rejected a same-map job environment reference before dispatch. The job now derives both values independently from the bounded request id. | Blocked dispatch for merge `d936ccfb2`; `current change`; focused workflow parsing, budget, and binding checks. | Rerun the exact merged staging plan and retain its protected evidence. |
| 2026-08-28 | implemented | Added per-Job bounded naming after the first Terraform staging plan found analyzer, inventory, observation, provider-schema, and scheduler names over Azure's 32-character limit. | Failed plan-only run `33111410162`; `current change`; focused naming checks and Terraform validation. | Rerun plan-only; channel-edge secret references remain an external prerequisite. |
| 2026-08-28 | implemented | Decoupled platform identity preparation from Operator channel provider configuration. Platform unions the DSN into the existing role-assignment address, deduplicates an older explicit DSN input, and accepts zero additional secret scopes. | Failed plan-only runs `33111410162` and `33112559478`; `current change`; root Terraform checks (`7 passed`), service roots (`5 passed`), and deployment checks (`192 passed`). | Retain a platform plan; independent Operator activation still requires principal and provider Key Vault references. |
| 2026-08-28 | implemented | Isolated protected Terraform plan execution from an inherited runner-wide `TF_CLI_ARGS` value and disabled ANSI output after run `33114757285` rendered a full plan but exited without a diagnostic body. | Unknown plan-only run `33114757285`; `current change`; actionlint and focused workflow checks. | Retry once on the exact merged revision and retain either the saved plan or an explicit no-color diagnostic. |
| 2026-08-28 | implemented | Used the deterministic no-color retry to identify unversioned synthesized model deployments, then propagated catalog versions before Terraform input materialization. | Explicit failed plan-only run `33115864100`; `current change`; focused resolver and deployment-projection checks (`67 passed`). | Rerun and retain an apply-eligible plan or the next explicit blocker. |
| 2026-08-28 | implemented | Corrected version handling after the next retry found an optional family without a GA version. Such preferences are excluded, and an unresolved optional capability emits no Terraform route. | Explicit failed plan-only run `33118464379`; `current change`; focused resolver and deployment-projection checks (`98 passed`). | Rerun and retain an apply-eligible plan or the next explicit blocker. |
| 2026-08-28 | implemented | Added canonical live-preflight mappings for Foundry account projects and model deployments after versioned planning reached the policy guard with unmapped Terraform resource types. | Failed plan-only run `33120128352`; `current change`; actionlint, workflow-budget, model-binding, and live-preflight checks. | Rerun and retain an apply-eligible plan or the next explicit blocker. |
| 2026-08-28 | implemented | Made incomplete live-preflight mapping errors report a sorted, bounded list of missing Terraform resource types after the first Foundry mappings exposed another generic failure. | Failed plan-only run `33121335105`; `current change`; focused live-preflight checks. | Rerun once, add the exact remaining mappings, and retain the next protected result. |
| 2026-08-28 | implemented | Added canonical mappings for the 12 types reported by full-staging preflight. Policy evaluation now covers subscription and application scopes for all planned types plus the ops resource group for cross-scope VNet peering. | Failed plan-only run `33122246033`; `current change`; actionlint and focused workflow/preflight checks. | Rerun and retain an apply-eligible plan or the next explicit blocker. |
| 2026-08-28 | implemented | Completed the exact-revision staging plan after all mapped policy scopes passed live preflight. The protected plan is non-destructive and seals ChatOps validation mode with the resolved model evidence. | Merge `bbb113dad`; successful plan-only run `33124447506`; `155 add / 0 change / 0 destroy`; live preflight `clear`; protected plan `plan-33124447506-1`. | Obtain explicit apply approval before the protected plan expires. Independent Operator activation still requires principal and provider Key Vault references. |
