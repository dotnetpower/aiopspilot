...
| Channel-edge identity substrate | implemented | `infra/main.tf`; `infra/services/operator-service/`; root and service Terraform checks | Platform grants the dedicated edge identity access to the Operator DSN. Additional provider/principal secret scopes remain optional platform inputs, while the service root rejects an enabled edge without principal scopes and one complete provider contract. |
