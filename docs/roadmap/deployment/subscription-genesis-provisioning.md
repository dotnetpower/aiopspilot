...
|------|------------------|------------------------|
| Operator entry point | `fdaictl` provides bootstrap reconciliation plus protected application plan, exact apply, status, and verification-only resume. `genesis-up.sh` now composes the local private Foundation lifecycle. | Protected application dispatch, the Blob-to-Operator mirror, and one complete ready receipt remain open. |
| Genesis progress | `genesis-up.sh` runs 15 explicit stages with private JSON status, provider reconciliation, an effective-policy probe, optional exact runner-image plan/apply, Foundation plan/apply, Bastion enrollment and attestation, and claim-safe private-state handoff. Completed receipts are checked again on restart, and a claim always selects verification-only resume. | The local route stops at `application-plan`. It does not configure or dispatch the protected application deployment and never reports subscription readiness. |
