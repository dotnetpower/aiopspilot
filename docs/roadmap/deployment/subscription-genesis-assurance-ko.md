...
translation_source_sha: c379aaf97f5bb82f8da1ca848ef4cd42c27cb280
...
로컬 라우터는 아티팩트, 신뢰, 프로필, 변수 경로가 모두 완전한 절대 경로일 때만 서명된 키트로
기반 계층 계획을 자동화할 수 있습니다. 비공개 exact 계획을 저장하고 검토 다이제스트, 계획
다이제스트, 만료 시점, 권한 없음 플래그만 표시합니다. 호스트를 등록하거나 계획을 적용하지
않습니다. 범위가 제한된 각 자식 명령은 별도 프로세스 그룹에서 실행합니다. 명령이 실행 중이면
표시 전용 점 하나를 10초마다 stderr로 보내고, 제한 시간이 지나면 라우터가 실패를 기록하기
전에 전체 그룹을 종료합니다. 캡처한 stdout과 진단은 다시 쓰거나 이동 가능한 상태에 복사하지
않습니다.
