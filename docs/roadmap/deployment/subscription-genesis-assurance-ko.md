...
로컬 라우터는 아티팩트, 신뢰, 프로필, 변수, 단계별 접근 경로가 모두 완전한 절대 경로일 때만
서명된 키트 기반 이미지 계획 및 적용, 기반 계층 계획 및 적용, Bastion 등록, 호스트 증명,
비공개 상태 인계를 구성합니다. 새 효과마다 실행, 소스, 단계, 정확한 근거 다이제스트, 최대
1시간인 UTC 구간에 연결된 모드 `0600` 승인 기록이 필요합니다. 이 단일 사용자 파일 전송은
`dev`로 제한하며 staging과 production은 보호된 정족수 전송을 유지합니다. 다른 단계, 변경된
다이제스트, 만료된 기록, 응답 없음은 권한을 부여하지 않습니다.

각 효과는 변경 전에 변경 불가능한 점유를 기록합니다. 점유 또는 종결 증적이 이미 있으면
재시작 시 검증만 선택하고 Terraform 적용, 토큰 등록, 아카이브 전송, 백엔드 이전을 반복하지
않습니다. 보존된 등록 증적은 사람 행위자가 표시된 정확한 점유를 연결하고 Bastion 신원, 도구
체인, 서비스, GitHub 실행기 상태를 새로 읽어 검증한 뒤에만 수락합니다. 실행기 등록 자료는
정확한 Bastion 터널의 SSH 표준 입력으로만 전달합니다. Terraform, 프로세스 인자, Azure Run
Command, 로그, 상태, 증적에 넣지 않습니다. 이식 가능한 상태에는 다이제스트, 개수, 단계 상태,
안전한 부울 값만 유지합니다. 리소스 ID, SSH 경로, 로컬 상태 경로, 원시 계획은 비공개로
유지합니다. 상태 인계 점유는 인증된 사람 Azure 운영자의 대상 범위 다이제스트를 연결하며,
백엔드 권위와 종결 증적은 모두 해당 점유를 정확히 연결합니다. 원격 정리는 정확한 의도를
영속화하고 전송 아카이브와 원시 작업 트리를
삭제한 뒤 부재를 검증한 다음에만 종결 정리 근거를 기록합니다. 종결 표시가 있어도 원시 잔여물이
있으면 완료를 차단합니다.

범위가 제한된 각 하위 명령은 별도 프로세스 그룹에서 실행합니다. 명령이 실행 중이면 표시
전용 점 하나를 10초마다 stderr로 보내고, 제한 시간이 지나면 라우터가 실패를 기록하기 전에
전체 그룹을 종료합니다. 캡처한 stdout과 진단은 다시 쓰거나 이식 가능한 상태에 복사하지
않습니다. 로컬 순서는 검증된 기반 계층 상태 인계 뒤에 중단하며, 보호된 애플리케이션 계획과
완전한 준비 상태는 별도의 근거 게이트로 유지합니다.
...
검토된 상태 보강기는 완전한 수집이 끝난 뒤 승격 전에 실행합니다. 각 정확한 하위 리소스 조회는
서명된 대상, 응답, 동시성, 제한 시간 상한을 동일하게 적용하며 타입이 지정된 공급자 근거만 추가할
수 있습니다. 보강기 체인은 현재 기준 세대 하나를 고정합니다. 고정 값이 바뀌면 서로 다른 세대에서
유지된 상태를 합치지 않고 승격을 차단합니다.
