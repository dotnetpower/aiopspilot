...
| Bounded analyzer Job convergence | implemented | `deploy-dev.yml`; `enforce_plan_scope.py`; focused scope and workflow contract tests | A `plan-observability-*` request targets only the analyzer Container Apps Job. Its matching apply consumes the sealed plan, and the scope guard rejects every changed address outside that Job. |
...
|------|-------|--------|----------|-----------|
| 2026-09-10 | implemented | Removed a redundant live `docker login` from Core runtime verification. The binder now writes a mode-`0600` temporary Docker auth config, then relies on the retry-bounded GHCR token exchange and OCI attestation verification to authenticate and authorize the exact digest. | `current change`; focused runtime-image binder tests and credential non-disclosure assertions. | Retain one successful protected analyzer plan after the repeated pre-plan login failures. |
