...
| Bounded analyzer Job convergence | implemented | `deploy-dev.yml`; `enforce_plan_scope.py`; focused state, scope, and workflow contract tests | Before a `plan-observability-*` request targets only the analyzer Container Apps Job, state reconciliation moves only the two known legacy Job addresses to their indexed destinations and records before/after state digests. The scope guard rejects every changed resource outside the analyzer Job. |
