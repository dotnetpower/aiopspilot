...
| Bounded analyzer Job convergence | implemented | `deploy-dev.yml`; `enforce_plan_scope.py`; focused scope and workflow contract tests | A `plan-observability-*` request admits changes only to the analyzer Container Apps Job. The Terraform target closure also includes the two legacy Job addresses required to complete recorded state moves, while the scope guard rejects any non-noop change at those addresses. |
