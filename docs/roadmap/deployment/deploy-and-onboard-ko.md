...
translation_source_sha: 43b2d26ece4c4634d388419f4ff3ad201fc0d3f7
...
- 공개 IP 없이 지속형 `Standard_D4ds_v5`와 `Local` `ResourceDisk` 임시 OS에서 실행기 자리 1-5개를 등록하는 **자체 호스팅 배포 실행기 VM**.
  ...
  예약된 상태 점검은 검증기가 바뀌면 다시 실행하고 채택한 특수화 VM의 공개 SSH 입력을 Azure 읽기 결과에서 복구하며, 모델에만 존재하는 디스크 ID를 실제 인벤토리와 비교하고 구조화된 드리프트 작업이 없을 때만 성공합니다.
