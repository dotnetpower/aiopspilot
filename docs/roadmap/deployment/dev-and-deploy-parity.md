...
fake and real adapters, but real mode is an explicit CI opt-in. It creates one exact temporary
database and UUID-scoped topics and consumer groups, then deletes only those resources. A
loopback-only socket guard blocks cloud calls, and missing Docker services fail the real job instead
of becoming a successful provider result.
...
origin. Production composition delegates channel verification, durable HIL delivery, and IAM database assembly to `iam_composition.py`, while the local identity factory stays in the existing authentication module. This keeps the composition root below its reviewed fanout ceiling. The Azure Resource Manager token remains inside the API process. Direct Psycopg stores
...
The workspace treats `.github/workflows/deploy-dev.yml` and local composite action metadata as plain YAML and applies the GitHub Action JSON schema to local `action.yml` and `action.yaml` files.
Editor completion and structural validation remain available without extension diagnostics; remote tag checks, repository workflow contracts, and GitHub Actions runtime remain authoritative.
...
Deployed runtime-call evidence uses that provider through the inventory Job, but its producer is
intentionally venue-specific. The deployed Operator emits an authority-free caller witness only
after the authenticated broker accepts a semantic request, and deployed Core emits the matching
target witness when that delivery reaches its consumer boundary, before semantic processing can
reject the turn. Terraform supplies the same exact Operator and Core Resource IDs to those two
services. Azure Monitor independently binds each witness to its
platform workload, and the inventory identity re-reads the exact revision and replica beneath each
claimed ARM ID. When Operator exists, protected Core and Operator plans require this platform-owned
binding. Its first adoption uses the independent `runtime_call_evidence_transition` plan mode so
unrelated environment or resource-identity drift remains blocked; the same sealed mode removes both
bindings when Operator is disabled. The inventory Job activates the source only in the deployed
venue with its explicit platform flag. A headless Core deployment keeps the source explicitly unavailable. Interactive local uses
the same projection and unavailable-state contracts, but it cannot claim those deployed platform
witnesses; it keeps `runtime_call_graph` unavailable.
