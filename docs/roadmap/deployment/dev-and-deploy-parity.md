...
The WAF and CAF assessment consumer follows the same parity rule. Local and deployed Operator
profiles consume the same logical topic, validate the same immutable snapshot, and write the same
PostgreSQL projection. The protected live-validation workflow is deliberately audit-only because
the deploy runner has no assessment sender role; its artifact never substitutes for an Operator
projection.
