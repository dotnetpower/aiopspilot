...
Both profiles start the retry-safe Incident intervention outbox only when PostgreSQL and the event
bus are configured. The same readiness check blocks service readiness if that worker stops, without
granting the Console or Operator API execution authority.
