...
- a **self-hosted deploy runner VM** (no public IP) with one to five slots on sustained `Standard_D4ds_v5` compute and a `Local` `ResourceDisk` ephemeral OS.
  ...
  Scheduled posture checks rerun when their verifier changes, recover an adopted specialized VM's public SSH input from Azure readback, compare its model-only disk ID with actual inventory, and succeed only when structured drift actions are empty.
