...
- a **self-hosted deploy runner VM** (no public IP) with one to five slots on sustained `Standard_D4ds_v5` compute and a `Local` `ResourceDisk` ephemeral OS.
  ...
  Scheduled posture checks compare any model-only OS disk ID with the actual Azure disk inventory, and the bootstrap refresh plan verifies structured drift actions instead of trusting the provider exit code alone.
