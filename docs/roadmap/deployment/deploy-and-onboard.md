...
- **Runner inputs:** Supply an SSH public key, quota headroom, and the Log Analytics destination. Run `prepare-genesis-access-tools.sh` before connected Bastion or Microsoft Entra SSH checks; it pins stable CLI extensions without creating Azure resources. Offline bootstrap also requires an exact prebuilt image.
