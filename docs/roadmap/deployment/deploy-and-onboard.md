...
| `FDAI_DECISION_EVIDENCE_CONTAINER_URL` | env | deployment | Optional private Blob container containing immutable content-addressed decision-evidence proof, requirement, bundle, and admission records. Core uses its attached read-only Managed Identity and never receives a storage key. When unset, the runtime uses the StateStore-backed provider and missing records continue to hold positive decisions for review. |
