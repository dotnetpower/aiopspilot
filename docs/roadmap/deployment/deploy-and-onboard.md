...
The ops layer creates one outbound path by default for GitHub and the Azure management and identity
planes. A closed network with an approved route sets `enable_public_egress = false`. If it also
sets `runner_bootstrap_mode = "offline"`, bootstrap requires one exact managed-image or numeric
gallery-version ID and disables marketplace selection, cloud-init, and GitHub registration.
...
commit code runs. A
workflow that needs an additional deployment helper installs it only in runner-temporary storage, pins its exact release and SHA-256 digest, and verifies it before use. The exact CI version proves parsing and plan assertions. Upgrades verify action runtime metadata, and the self-hosted runner install always resolves the latest published GitHub Actions runner release rather than a pinned floor version. When private networking is enabled, PostgreSQL public access and the broad Azure-services firewall
...
their private endpoints, account-specific untagged A records in the runner's existing Blob DNS zone, the read-only binding, and the independently declared root Job. The Job dependency includes private readback without public access or storage-key authentication. Exact apply is
...
- **Azure Bots** - the approval bot remains deployment-owned. The independent System Knowledge
  ...
  container, and exact HTTPS endpoint only through the protected service workflow. The platform `document_blob` diagnostic setting continues to cover the reused claim storage account.
...
- **Cross-tenant SharePoint**: `sharepoint_connector_enabled=true` activates the FDAI-native connector. The Azure ingestion UAMI supplies a federated assertion to the application registered in the Microsoft 365 tenant, which issues the Microsoft Graph token after administrator consent. Root composition forwards only the target tenant, application, site, drive, redirect allowlist, and server-owned document policy into the document ingestion module; it doesn't construct connector state. The protected service workflow requires an explicit `sharepoint_connector_transition` and seals the exact enable or disable environment delta into the plan bundle. Power Platform isn't deployed or invoked.
- **Executor deployment and cutover**: `enable_isolated_executor=true` provisions the internal app and a dedicated UAMI with ACR pull, command receive, receipt/DLQ send, and state-secret read only. The default is `false`; the private-runner workflow remains plan-only by default and installs its checksum-pinned GitHub CLI whenever model verification or isolated Executor image binding requires `gh`. Before Terraform initialization or any ACR command, it resolves the exact GHCR subject digest and verifies only the registry-hosted OCI bundle with `--bundle-from-oci`, the selected revision through `--source-digest`, the SLSA v1 predicate, and the repository's `container-supply-chain.yml` signer. It has no GitHub API bundle fallback, keeps Docker authentication in owner-only temporary files, removes them on exit, and records no binding on failure. It syntax-checks embedded plan-metadata code, verifies the Core artifact under `ghcr.io/<owner>/<repo>/fdai-core-control-plane`, sends the host-only `ghcr.io` registry URI required by the ACR import API, binds the identical ACR digest, and includes the latest revision in health checks. Binding validates the ACR resource ID before import and emits content-free progress for registry lookup, import acceptance, and digest readback. Executable fake-boundary tests cover successful binding, an invalid ACR identity, and an import failure without exposing credentials. `promote_runtime_image=true` imports that verified digest into the ACR `fdai` repository without rebuilding, while exact apply rejects promotion, consumes only the protected plan, and restores the same runtime digest for convergence. `enable_isolated_executor_authority_cutover=true` additionally requires the development operations gateway, removes gateway and vertical effect access from Core, authorizes the isolated identity, and keeps Core transport/read access. `verify_executor_effect=true` runs one reversible NSG rule probe through an explicit pseudo-terminal on the non-interactive runner and preserves the remote exit status. Duplicate delivery shares one issued-at timestamp to retain immutable action and command identity; cleanup receives a fresh bounded deadline. The workflow checks the effect through Azure Resource Manager, rejects duplicate writes, records offsets and terminal receipts, cleans up, and fails after 900 seconds.
...
| `FDAI_REVISION` / `FDAI_OPERATIONAL_PROMOTION_EVIDENCE_ROOT` / `FDAI_OPERATIONAL_PROMOTION_MANIFEST` | env | deployment | Required by the opt-in operational-promotion Job. The revision is a full immutable source revision, the root is an absolute container path supplied by the pinned image or a protected read-only mount, and the manifest path is relative to that root. Evidence files contain bounded canonical facts and digests, not raw provider payloads. |
...
| `FDAI_CHAOS_CONTEXT_JSON` / `FDAI_CHAOS_ENFORCE` | env | deployment | Runtime context for promoted chaos injectors. Enforce stays disabled unless the explicit flag is `1`, the scenario is promoted, and both injector and probe are registered. A failed protected scenario-lab plan emits only allowlisted diagnostic categories, Terraform addresses, and Azure error codes before cleanup shreds the raw provider log. |
...
| `FDAI_CHATOPS_WEBHOOK_SECRET` / `FDAI_CHATOPS_TIMEOUT_SECONDS` / `FDAI_TEAMS_APPLICATION_ID` / `FDAI_TEAMS_TENANT_ID` / `FDAI_TEAMS_APPROVAL_TEAM_ID` / `FDAI_TEAMS_APPROVAL_CHANNEL_ID` / `FDAI_TEAMS_APPROVAL_ACTIVITY_URL` / `FDAI_TEAMS_BOT_MI_CLIENT_ID` / `FDAI_TEAMS_ALLOWED_SERVICE_URLS_JSON` / `FDAI_TEAMS_JWKS_URL` / `FDAI_TEAMS_PRINCIPAL_MAP_JSON` / `FDAI_SLACK_TEAM_ID` / `FDAI_SLACK_PRINCIPAL_MAP_JSON` | env + KV ref | deployment | Human approval callbacks require PostgreSQL, Kafka, a bounded workflow timeout, and the shared HMAC secret for the internal Slack relay. Teams requires the complete Bot application, tenant, group-connected team/channel activity endpoint, dedicated Bot managed identity, service-token trust inputs, and principal map; Slack can operate independently when its workspace and principal map are complete. Missing or partial channel configuration remains unavailable. |
| `FDAI_KAFKA_BOOTSTRAP_SERVERS` / `FDAI_HIL_DECISION_TOPIC` | env | deployment / upstream | Event Hubs Kafka endpoint used by the Operator API durable decision outbox; topic defaults to `fdai.hil.decisions`. Operator persists first, publishes the exact decision fields, and marks delivery only after broker acceptance. Core consumes the same topic and owns resume/execution. |
| `FDAI_NOTIFICATION_RECEIPT_SECRET` / `FDAI_NOTIFICATION_RECEIPT_TOPIC` | env + KV ref | deployment | Authenticates the publication-receipt callback a publishing automation signs. The topic is the fixed logical name `fdai.notifications.delivery-receipts`, multiplexed over the existing primary physical topic; a different value blocks startup. The secret requires PostgreSQL, the semantic physical topic, and a configured Kafka receipt transport. Without the secret the ingress stays bound and fail-closed, so an unsigned callback is refused rather than silently dropped. Core alone applies the resulting `delivered` or `retryable_failed` transition. |
| `FDAI_TEAMS_NOTIFICATION_ENDPOINT` / `FDAI_NOTIFICATION_BINDINGS_JSON` | env + KV ref | deployment | Supplied together by Terraform only when `enable_teams_notification_delivery` and `teams_notification_binding` activate A2/A4 Teams delivery. Saving or testing an endpoint in the Console never sets them. The control plane holds read-only authority on the endpoint secret and refuses the seeded `unconfigured` placeholder at startup. |
| `FDAI_TEAMS_NOTIFICATION_ACTIVATION` | env | local | Local-only explicit activation. When set, the control plane reads the encrypted Operator-owned Teams binding record from the shared loopback database instead of a plaintext endpoint value. Unset means a saved binding delivers nothing. |
...
| `FDAI_GITOPS_TOKEN` / `FDAI_GITOPS_OWNER` / `FDAI_GITOPS_REPO` / `FDAI_GITHUB_WORKFLOW_TOOLS_ENFORCE` | KV ref + env | deployment | Binds the GitHub change feed and workflow tools for fix/release/security/incident/IRP artifacts. The platform exports the enabled stewardship target and token secret id as `stewardship_gitops_binding`; the protected service workflow validates that object and injects only the Key Vault reference into the independent Core service. The enforce flag never bypasses the ActionType promotion and risk/HIL gates. |
...
| `FDAI_STEWARDSHIP_REQUIRE_BINDINGS` / `FDAI_STEWARDSHIP_AUDIT_INTERVAL_SECONDS` / `FDAI_HANDOVER_KNOWLEDGE_INTERVAL_SECONDS` | env | deployment / upstream | Set `FDAI_STEWARDSHIP_REQUIRE_BINDINGS=1` so placeholder identities fail startup. The protected workflow maps `ENABLE_STEWARDSHIP_GOVERNANCE`, `GITOPS_OWNER`, `GITOPS_REPO`, `GITHUB_APP_CLIENT_ID`, and `GITHUB_APP_INSTALLATION_ID` from repository Variables. It maps `GITHUB_APP_PRIVATE_KEY` and `GITHUB_WEBHOOK_SECRET` from repository Secrets; `GITOPS_TOKEN` remains a mutually exclusive compatibility secret. GitHub controls App-key overlap and installation-token expiry, so the Key Vault entries carry reviewed coordinated-rotation exceptions instead of independent fixed expirations. Terraform also supplies `FDAI_MAINTAINERS`, `FDAI_STEWARD_<AGENT>`, Key Vault references, and the readiness-gated Entra health and content-free handover lifecycle cadences defined in [Agent operational ownership lifecycle](../interfaces/agent-stewardship-operations.md). The cadence defaults are 3600 and 60 seconds and grant no merge, IAM, approval, or execution authority. |
...
| `FDAI_DR_DRILL_SOURCE_SERVER_ARM_ID` / `FDAI_DR_DRILL_TARGET_LOCATION` / `FDAI_DR_DRILL_TARGET_RESOURCE_GROUP` / `FDAI_DR_DRILL_TARGET_SERVER_PREFIX` / `FDAI_DR_DRILL_PITR_OFFSET_MINUTES` / `FDAI_DR_DRILL_INTEGRITY_TABLES` / `FDAI_DR_DRILL_DRY_RUN` | env | deployment | Delivery-owned DB-DR drill config (see [DB-DR drill runbook](../../runbooks/db-dr-drill.md)). The job uses a dedicated non-executor identity and a pre-created isolated target group. `DRY_RUN=true` validates complete configuration without making an Azure or database change. |
...
| To learn about | Read |
|----------------|------|
| Delivery status and remaining work | [Implementation ledger](../../roadmap-implementation/deployment/deploy-and-onboard.md) |
