...
validation and plan-scope logic belongs in reviewed helpers instead of inline shell blocks.
...
The protected `history-` request mode targets only the operational-history storage, private
endpoint, and independently declared root Job. Exact apply is re-dispatched by the bot-owned
...
| `FDAI_DECISION_EVIDENCE_CONTAINER_URL` | env | deployment | Optional dedicated private Blob container containing immutable content-addressed decision-evidence proof, requirement, bundle, and admission records. The deploy runner alone writes new records, the container applies a 90-day time-based immutability policy, and Core uses an attached read-only Managed Identity without receiving a storage key. Local execution may use the StateStore-backed provider for parity tests. A deployed runtime with no private container keeps admission unbound, and missing records continue to hold positive decisions for review. |
