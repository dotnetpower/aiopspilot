...
floor. Plan-only retention admits an Operator API OpenAI User role replacement only when its scope and role are unchanged, `principal_id` is the sole role replacement path, and the paired Operator UAMI changes only its name while preserving location, resource group, and tags. The same review copy admits only exact role principal or scope migrations, indexed-to-unindexed measurement Job retirements with created successors, and the reviewed embedding family and SKU replacement; provider-computed field encoding does not grant authority, and retention never authorizes apply. The protected deploy workflow keeps repeated request validation and plan-scope logic in
...
their private endpoints, a root-managed untagged A record in the runner's existing Blob DNS zone, read-only binding, and independently declared root Job. Exact apply is
