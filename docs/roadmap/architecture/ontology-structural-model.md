...
| Provider-observed topology production | validated | `azure-arg-v1.yaml`; `arm_inventory.py`; `kubernetes_api_inventory.py`; `kubernetes_inventory.py`; `refresh-authoritative-inventory.py`; focused Azure, Kubernetes, inventory promotion, catalog, Ruff, strict mypy, and Issue #278 exact-cluster evidence | Ninety-five reviewed mappings cover Azure containment and traffic configuration plus UID-grounded Kubernetes runtime topology, exact Node provider identity, Ingress backend Services, and EndpointSlice exposure. The retained exact-cluster generation contains 103 Kubernetes Resources and 208 independently verified Kubernetes relationships, including one exact Node-to-VMSS-VM bridge. Unconfigured or unreachable sources still remain explicit unavailable evidence. |
