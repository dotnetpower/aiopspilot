...
**초기 설계.** 각 T2 캐시 행에 `expires_at`을 추가하고 만료된 행을 정리하며 승격 중에 이전
카탈로그 파티션을 별도로 삭제합니다.

**비판.** 독립적인 행 삭제 경로와 파티션 삭제 경로는 두 작성자를 만들고 승격 또는 롤백과
경합합니다. 호출자가 제공한 활성 버전을 신뢰하는 로테이션 명령은 최신 상태를 놓쳤을 때 유일한
롤백 파티션도 삭제할 수 있습니다.

**개정 설계.** Core 컨트롤 플레인이 PostgreSQL T2 캐시 어댑터 하나와 기준 카탈로그 상태 행
하나를 소유합니다. 읽기는 정확한 `catalog_version`과 `input_hash`를 선택하고
`expires_at > now()`를 요구하며 `(catalog_version, input_hash, expires_at)` 인덱스를 사용합니다.
쓰기는 먼저 정확한 목록 파티션을 만듭니다. 분리된 레거시 기본 테이블은 쓰기 또는 읽기 대체
경로로 사용하지 않습니다. 파티션 이름은 카탈로그 버전의 SHA-256에서 만들고 레지스트리는 원래의
정확한 버전을 유지합니다. 마이그레이션이 소유하는 security-definer 함수는 파티션 하나를 만들거나
삭제하기 전에 다이제스트, 파생 이름, 레지스트리 항목 및 보호 버전을 검증합니다. 따라서 Core
런타임에는 광범위한 스키마 DDL 권한이 없습니다.

승격은 새 활성 파티션을 만들고 이전 활성 파티션을 롤백용으로 유지합니다. 롤백은 이 정확한 상태
값을 서로 바꾸며 삭제된 데이터를 재구성하지 않습니다. 로테이션은 트랜잭션 범위 advisory lock
하나를 획득하고 명령의 활성 및 롤백 버전을 기준 상태 행과 대조합니다. 그런 다음 보호되는 두
버전 밖에 있으면서 제한된 기준 시점에 모든 행이 만료된 등록 파티션만 삭제합니다. 같은
...
- [x] 캐시 TTL, 정확한 카탈로그 버전 선택, 승격 무효화, 롤백 유지 및 감사 가능한 파티션
  로테이션을 구현하고 로컬 PostgreSQL에서 검사했습니다.
...
| 알아볼 내용 | 읽을 문서 |
|-------------|-----------|
| 구현 상태 및 남은 작업 | [구현 원장](../../roadmap-implementation/architecture/llm-strategy.md) |
