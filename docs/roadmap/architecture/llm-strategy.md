...
Model availability, versions, and deprecations shift continuously. Hard-coding a model id guarantees rot. The provisioning model below keeps the capability→concrete-model mapping
...
compatible. Root Terraform splits publishers and composes private partner resources conditionally; deployment seals partner bindings before hashing, while runtime map delivery and registry activation remain open.
...
| To learn about | Read |
|----------------|------|
| Delivery status and remaining work | [Implementation ledger](../../roadmap-implementation/architecture/llm-strategy.md) |
