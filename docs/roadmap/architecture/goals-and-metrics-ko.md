...
translation_source_sha: 628d31a36ab1438016249820673f77c9d17a5a85
...
| 통제된 기준선 및 처리 코호트 주장 | in-progress | `config/sre-cohort-claim-policy.json`; `services/core-control-plane/src/fdai/core/measurement/{cohort_claim_policy,baseline_cohort_claim,operational_cohort}.py`; `packages/service-contracts/src/fdai_service_contracts/baseline_cohort.py`; 집중 정책, 계약, 주장 및 축약기 테스트 | 이전 정책이 명시적으로 합성인 재생 코퍼스의 다이제스트에 주장용 코호트를 결합한 문제가 검토에서 확인됐습니다. 수정된 설계는 해당 벤치마크를 동결된 향후 운영 프로토콜과 분리합니다. 주장 지표에는 유한한 값과 경계가 필요하며, 코호트 기준 시점은 가장 최신인 군 기준 시점이고, basis point 올림 계산으로 모든 임계값 0 위반을 보존합니다. 프로토콜은 실제 FDAI 도입 전 운영 프로세스를 기준군으로, 배포된 FDAI를 처리군으로 사용하며 동일한 적격성, 구간, 리비전, 출처, 독립성 및 중단 규칙을 적용합니다. 비합성 코호트 보존은 아직 남아 있습니다. |
