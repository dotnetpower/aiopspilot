...
translation_source_sha: 2f984850cc32edf54b40d8545073ef4c9a483e5a
...
| 의사 결정 경계 승인 완전 커버리지 | implemented | `config/decision-boundary-inventory.json`; `scripts/quality/architecture/check-decision-boundary-coverage.py`; `tests/integration/scripts/test_decision_boundary_coverage.py`; 집중 경계 테스트 | 등록된 긍정적 의사 결정 경계 19개가 모두 공유 승인 계약을 통해 의사 결정 핵심 근거를 해석하며, 승인 결과가 없거나 수락되지 않으면 검토 보류로 처리합니다. 가드는 인벤토리를 양방향으로 검사하므로 의도적으로 누락된 등록 경계는 실패합니다. 준비 상태 매트릭스는 누락, 오래됨, 불완전, 충돌, 합성, 잘못된 목적 및 잘못된 범위 근거를 각각 이름으로 차단합니다. |
...
| 정식 의사 결정 핵심 근거 봉투 | implemented | `packages/service-contracts/src/fdai_service_contracts/decision_evidence.py`; `schemas/decision-critical-evidence/1.0.0.json`; 집중 계약 테스트 | 이 봉투는 근거와 해당 인증 증명, 권위, 범위, 목적, 정확한 생성기와 방법, 시간, 정책에서 파생된 최신성, 완전성 증명, 충돌 판정, 출처 계보 및 합성 상태를 연결합니다. 주장 사전 검사는 입력을 차단하거나 별도의 권위 있는 검증으로 전달할 수만 있으며 실제 운영 준비 상태를 주장하지 않습니다. 이제 등록된 모든 긍정적 의사 결정 경계가 공유 승인 계약을 사용하며, 통제된 최신 실제 승인은 열린 작업으로 남아 있습니다. |
...
| 날짜 | 상태 | 변경 | 근거 | 남은 작업 |
|------|------|------|------|-----------|
| 2026-09-11 | implemented | 코호트 축약기와 인벤토리 뒤에 없던 신뢰 관측 반입 경계를 추가했습니다. 정규화한 묶음은 크기가 제한되고 내용이 봉인됩니다. 반입기가 소유하는 신뢰 필드는 산출물에서 허용되지 않으며 정확한 군, 리비전, 프로토콜 및 출처 실행 계보는 보호된 workflow에서 옵니다. 멱등 보존은 충돌하는 재사용을 감지하고, 인벤토리는 기록 시각과 함께 관측 시각도 사용하며, exporter allowlist는 같은 저장소 리비전 내부의 일반 workflow 파일만 수락합니다. | `current change`; 코호트 정책, 관측 반입기와 CLI, 보호된 workflow, 인벤토리 쿼리 및 집중 단위/workflow 테스트. | 권위 있는 기준군 exporter 하나와 배포된 FDAI 처리군 exporter 하나를 군별 allowlist 항목과 함께 추가한 뒤 필수 측정값마다 표본 30개를 보존하고 승인합니다. |
...
| 2026-09-09 | implemented | 코호트 숫자, 시간 및 임계값 0 불변 조건을 강화했습니다. 무한대 지표 값은 유효하지 않고, 코호트 기준 시점은 가장 최신인 군 근거보다 앞설 수 없으며, 큰 코호트의 위반 1건도 0 basis point로 반올림되지 않습니다. | `current change`; 코호트 계약과 운영 축약기; 집중 계약 및 축약기 테스트. | 통제된 비합성 기준군 및 처리군 관측값과 독립 승인을 보존합니다. |
...
| 2026-09-06 | in-progress | 적대적 검토 결과에 따라 고정 말뭉치 승인 절차를 강화했습니다. 완료된 묶음은 시나리오를 하나 이상 소유해야 하며, 모든 커버리지 주장은 검토된 정확한 테스트, 차원 및 시나리오에 결속해야 합니다. 이제 안전 가드는 권한 부여 토큰 변형을 거부하고, 충돌 검색은 파일명 버전과 내용 버전을 결속하며, CI는 새 말뭉치 버전과 완전한 매니페스트 인벤토리를 한 번에 반영하도록 요구합니다. v2026.09 묶음 5개의 상태는 모두 `partial`이고 집계 상태는 `incomplete`로 유지됩니다. | `current change`; `services/core-control-plane/tests/scenarios/{manifest.schema.json,test_frozen.py,test_v2026_09_frozen.py}`; `scripts/quality/ci/check-frozen-scenario-additions.py`; `.github/workflows/ci.yml`; 완전한 집중 스키마, 고정 자료, 재생, 안전 및 CI 계약 테스트 모음에서 사례 551건이 통과했고 선택한 문서 검사도 모두 통과했습니다. | 묶음이나 집계 상태를 변경하기 전에 FDAI-CONST-005의 기능별 결과 근거 집합 5개를 추가합니다. |
| 2026-09-06 | in-progress | v2026.09 완료 관계를 양방향으로 만들었습니다. 기능별 결과 근거를 모두 갖추면 해당 묶음은 `complete`여야 하고, 다섯 묶음이 모두 완료되면 집계도 `complete`여야 합니다. 현재 묶음은 필수 결과가 아직 누락되어 있으므로 정확하게 `partial` 상태를 유지합니다. | `current change`; `services/core-control-plane/tests/scenarios/{manifest.schema.json,test_v2026_09_frozen.py}`; 선택한 시나리오 및 CI 계약 테스트 모음에서 사례 534건이 통과했습니다. 직전의 완전 커버리지 개정에서는 사례 532건이 통과하여 아래의 오래된 482건 기록을 대체합니다. | 현재 묶음이나 집계 상태를 변경하기 전에 FDAI-CONST-005의 기능별 결과 근거 집합 5개를 추가합니다. |
| 2026-09-06 | in-progress | 일반 재생 커버리지를 FDAI-CONST-005에서 요구하는 기능별 결과 5개로 잘못 간주했다는 검토 결과에 따라 v2026.09 완료 계약을 바로잡았습니다. 이제 매니페스트는 각 필수 결과와 정확한 근거 공백을 기록하고, 모든 묶음과 집계는 미완료 상태를 유지하며, 집중 음성 테스트는 비어 있지 않은 일반 커버리지 배열만으로 완료 상태가 될 수 없음을 입증합니다. | `current change`; `services/core-control-plane/tests/scenarios/{manifest.schema.json,manifests/v2026.09.json,test_v2026_09_frozen.py}`; 집중 시나리오, 재생, 안전, CI 및 문서 검사. | SRE 복구 및 재발 종결, ARB 승인 조건 및 변경 후 검증, 신뢰성을 유지한 FinOps 실현 절감, 측정된 RTO/RPO를 포함한 DR 무결성, 지속적인 중지 가드와 검증된 복구를 포함해 명시적으로 사람이 승인한 Chaos 주입 근거를 추가합니다. |
| 2026-09-06 | implemented | SRE, ARB / 변경 안전성, FinOps, DR 및 Chaos Engineering을 위한 완전한 v2026.09 말뭉치를 고정했습니다. 이제 각 묶음은 헌법상 차원 6개를 모두 관리하고, 제공되는 컨트롤 루프를 통해 재생하며, 결함이 있는 효과 근거에서 실패 시 차단하고, 실행 권한을 부여하지 않은 채 통제된 중재 경계에 도달합니다. CI 계약은 각 버전이 처음 반영된 뒤 시나리오, 보강 자료, 충돌 명세 및 매니페스트도 고정합니다. | `current change`; `services/core-control-plane/tests/scenarios/{v2026.09,enrichment/v2026.09,cross-objective/v2026.09-*.json,manifests/v2026.09.json,test_v2026_09_frozen.py,test_v2026_09_replay.py}`; `.github/workflows/ci.yml`; `tests/integration/scripts/test_check_ci_contracts.py`; `uv run pytest -q --no-cov services/core-control-plane/tests/scenarios tests/integration/scripts/test_check_ci_contracts.py`로 사례 185건이 통과했습니다. | 검증된 도메인 커버리지를 주장하기 전에 통제된 배포 런타임 효과 및 중재 증적을 보존하고, 출시된 규칙이나 계획 산출물에서 서명된 목표 효과를 생성해야 합니다. |
| 2026-09-06 | implemented | 최종 업스트림 재배치 뒤 집중 검증 사례 수를 바로잡았습니다. 완전한 선택 시나리오 및 CI 계약 테스트 모음은 앞서 기록한 하위 집합 185건이 아니라 482건입니다. | `current change`; 재배치된 개정에서 `uv run pytest -q --no-cov services/core-control-plane/tests/scenarios tests/integration/scripts/test_check_ci_contracts.py`로 사례 482건이 통과했습니다. | 위에 기록한 배포 런타임 근거 경계에는 변화가 없습니다. |
| 2026-08-31 | in-progress | 코호트 주장을 의사 결정 핵심 긍정 경계로 등록하고 최종 검토에서 확인한 공유 사전 검사 공백을 닫았습니다. 이제 전체 인벤토리 가드는 상수가 아닌 단락 평가 미끼를 차단하고, 작업 흐름 게이트는 리터럴 불리언 `true`만 수락하며, 전달 완료 전에 기록된 효과 관측은 액션을 종결하는 대신 보류합니다. | `current change`; 의사 결정 경계 인벤토리와 검사기, 코호트 주장 평가기, 작업 흐름 게이트 해석기, MSCP 효과 승인 및 집중 경계, 작업 흐름, 효과 테스트. | 배포된 런타임에서 통제된 비합성 이중 코호트와 독립 승인 결과를 보존합니다. |
| 2026-08-31 | in-progress | 코호트 산출물이 스스로를 대변할 수 있던 마지막 두 경로를 닫았습니다. `BaselineTreatmentCohortReceipt`에서 `artifact_origin`을 제거했고, 출처는 이제 신뢰할 수 있는 반입 채널이 전달하는 `import_origin` 평가기 매개변수이며 기본값은 실패 시 차단하는 저장소 출처입니다. 통제된 외부 주장에는 갈래 승인 두 건에 더해 코호트 `receipt_digest` 전체에 결속한 신뢰 코호트 수준 `DecisionEvidenceAdmission`이 필요하므로, 이름표를 바꾸거나 다시 해시한 산출물은 자기 승인에서 벗어나 `cohort_not_admitted`로 보고됩니다. 반입기는 어느 수준에서든 `artifact_origin`을 담은 묶음을 거부합니다. `fdai_revision`은 운영 승격이 요구하는 것과 동일하게 변경 불가능한 40자 또는 64자 16진수 커밋 다이제스트인 `CommitRevision`이 되었고, 증적과 두 갈래, 신뢰 `CohortClaimRequirement`에 모두 적용됩니다. `--cohort-revision`은 근거를 읽기 전에 `main`, `HEAD`, 태그, `refs/heads/` 이름을 거부합니다. | `current change`; `packages/service-contracts/src/fdai_service_contracts/baseline_cohort.py`; `services/core-control-plane/src/fdai/core/measurement/{baseline_cohort_claim.py,cohort_claim_policy.py}`; `tools/{cohort_receipt.py,baseline_run.py}`; `uv run pytest -q --no-cov packages/service-contracts/tests services/core-control-plane/tests/core/measurement services/core-control-plane/tests/tools`가 집중 사례 539건을 통과했습니다. `test_operational_promotion.py`의 실패 6건은 `2ca190d9e`에서도 동일합니다. | 통제된 외부 코호트에는 배포 런타임에서 실제 신뢰 반입 채널과 실제 코호트 수준 승인 공급자가 여전히 필요합니다. |
| 2026-08-31 | in-progress | 검토 후 아래에 기록한 코호트 주장 신뢰 경계를 바로잡았습니다. 이제 코호트 산출물은 `DecisionEvidenceAdmission`도 `CohortClaimRequirement`도 스스로 역직렬화할 수 없습니다. `load_cohort_claim_receipt`는 보존된 증적만 구문 분석하고 `admissions`, `admitted_receipt_digests`, `claim_eligible`, `policy`, `requirement`, `verification_bundles`를 담은 묶음을 거부하므로, 공급자를 주입하지 않은 가져오기는 항상 부적격입니다. 승인은 주입된 신뢰 공유 `DecisionEvidenceAdmissionProvider`에 질의하는 `provider_cohort_admissions`, 또는 별도로 제공한 증명 5종 묶음을 기존 `DecisionEvidenceVerifierRegistry`로 다시 읽는 `verified_cohort_admissions`에서만 옵니다. 평가 요건은 근거와 무관하게 로드하고 코드에서 하한을 검사하는 신뢰 버전 관리 `config/sre-cohort-claim-policy.json`으로 옮겼습니다. 이 정책은 문서화된 성공 지표 5개 전체, 임계값 0 가드 4개 전체, 커밋된 고정 시나리오 집합의 실제 콘텐츠 다이제스트, 완전한 근거, 표본 30개 하한을 고정하며 기대 개정은 신뢰할 수 있는 호출자가 `--cohort-revision`으로 제공합니다. 평가 대상 갈래 사실, 즉 갈래 식별자, 시나리오 집합과 다이제스트, 개정, 표본 수, 절대 지표, 신뢰 구간, 가드, 완전성, 출처, 합성 여부, 보고서 및 출처 다이제스트는 모두 `cohort_arm_fact_digest`가 정규 해시로 계산하며, 승인은 그 `expected_evidence_digest`가 이 해시일 때만 인정하므로 지어낸 다이제스트나 재사용한 다이제스트로는 아무것도 승인되지 않습니다. 두 갈래는 서로 다른 보고서 다이제스트와 서로 다른 근거 증적을 보유해야 하며, 발행되는 `artifact_origin`은 검증된 계약에서 다시 읽으므로 `--cohort-receipt`에 넘긴 저장소 출처 산출물도 여전히 `repository`와 `artifact_ungoverned`를 발행합니다. 재생성한 `docs/baselines/v2026.07.json`은 여전히 표본 12개 합성 하네스로서 `claim_eligible=false`와 `receipt_missing`을 보고하며, 이제 신뢰 정책 식별자, 고정된 시나리오 집합 다이제스트, 필수 지표 및 가드 식별자도 함께 발행합니다. | `current change`; `config/sre-cohort-claim-policy.json`; `services/core-control-plane/src/fdai/core/measurement/{cohort_claim_policy.py,baseline_cohort_claim.py}`; `packages/service-contracts/src/fdai_service_contracts/baseline_cohort.py`; `tools/{cohort_receipt.py,baseline_run.py}`; `docs/baselines/v2026.07.json`; `uv run pytest -q --no-cov packages/service-contracts/tests services/core-control-plane/tests/core/measurement services/core-control-plane/tests/tools`로 집중 510건 통과. `tests/core/measurement/test_operational_promotion.py`의 실패 6건은 이 변경 없이 `e118a95c9`에서도 동일합니다. Ruff와 strict mypy에서 새로운 지적은 없었습니다. | 평가기, 정책, 두 승인 연결부는 아직 고정 자료와 가짜 공급자 및 검증기로만 입증했습니다. 고정된 개정 하나와 동일한 고정 시나리오 집합 위에서 갈래마다 표본 30개 이상을 갖춘 통제된 비합성 기준선 및 처리 코호트를 최신 독립 의사 결정 근거 승인과 함께 보존하는 일은 배포 런타임 근거가 필요하므로 열려 있습니다. |
| 2026-08-31 | in-progress | 비합성 기준선 및 처리 코호트의 적격 판정을 통제 아래에 두었습니다. `BaselineTreatmentCohortReceipt`는 갈래마다 `DecisionCriticalEvidenceReceipt`를 하나씩 담고, 고정 시나리오 집합의 버전과 다이제스트, 고정된 `fdai_revision` 하나, 갈래별 보고서와 출처 다이제스트, 표본 수, 절대값을 포함하는 성공 메트릭 신뢰 구간, 완전성과 출처 참조, 임계값 0 가드 전체를 연결합니다. 증적 자체에는 적격 필드가 없습니다. 적격 여부는 `evaluate_cohort_claim`의 출력으로만 존재하며, 증적 누락, 합성 근거, 거부된 사전 점검, 최신 독립 승인이 없는 근거, 보고서나 출처 다이제스트 불일치, 개정이나 시나리오 집합 불일치, 불완전한 메트릭, 구간, 출처, 가드, 위반된 가드, 어느 한 갈래라도 30 표본 미만인 코호트, 저장소 출처 산출물에 대해 이름이 지정된 사유로 실패 시 차단합니다. 승인된 증적 다이제스트는 산출물이 아니라 기존 의사 결정 근거 승인에서 들어오므로, 이 저장소에 커밋한 산출물은 적격으로 평가될 수 없습니다. `tools/baseline_run.py`에는 선택적인 `--cohort-receipt` 사전 점검과 종료 코드 4를 반환하는 `--require-claim-eligible` 게이트를 추가했고, 다시 생성한 `docs/baselines/v2026.07.json`은 표본 12개의 합성 하네스로 남아 `claim_eligible=false`, `receipt_missing`, 정확한 외부 잔여 과제를 보고합니다. | `current change`; `packages/service-contracts/src/fdai_service_contracts/baseline_cohort.py`; `services/core-control-plane/src/fdai/core/measurement/baseline_cohort_claim.py`; `tools/cohort_receipt.py`; `tools/baseline_run.py`; `docs/baselines/v2026.07.json`; `uv run pytest -q --no-cov packages/service-contracts/tests services/core-control-plane/tests/core/measurement services/core-control-plane/tests/tools services/core-control-plane/tests/scenarios`가 집중 사례 603건을 통과했습니다. `tests/core/measurement/test_operational_promotion.py`의 실패 6건은 이 변경 없이 `c1cb634d9`에서도 동일합니다. Ruff와 strict mypy도 통과했습니다. | 평가기는 아직 픽스처로만 입증했습니다. 고정된 개정 하나와 동일한 고정 시나리오 집합 위에서 갈래마다 30개 이상인 통제된 비합성 기준선 및 처리 코호트를 최신 독립 의사 결정 근거 승인과 함께 보존하는 일은 배포 런타임 근거가 필요하므로 남아 있습니다. |
| 2026-08-31 | in-progress | 검토 후 아래에 기록한 전체 루프 효과 근거를 바로잡았습니다. 계약이 표현할 수 없는 관측을 버리기만 하던 변환은 판정을 `verified`로 남겨 두었기 때문에, 실제 `verify_effect` 결과의 관측이 예측 창 안에 있으면서도 `recorded_at`보다 뒤일 때 여전히 발송 내부에서 계약 검증 오류가 발생했습니다. 이제 허용성 판단이 결정, shadow 효과 감사 항목, 변환보다 먼저 실행되어 그런 근거를 `observation_not_yet_recorded`로 보류하며, 이미 보류된 판정은 원래 사유를 유지합니다. 고정 재생 역시 액션 생성, 전달, 기록에 벽시계를 사용해 고정된 예측과 관측이 그것이 설명하는 액션보다 앞서 있었습니다. 이제 `ControlLoop`와 `ActionBuilder`는 하나의 주입된 시계를 공유하고, shadow 효과 항목은 관측 원본 값과 함께 액션 생성 시각과 전달 구간을 기록하며, 재생은 생성, 예측, 전달, 관측, 기록 순서를 직접 검증합니다. | `current change`; `services/core-control-plane/src/fdai/core/mscp_profile/{effect_verification.py,response_outcome.py,shadow_effect.py}`; `services/core-control-plane/src/fdai/core/control_loop/{orchestrator.py,_execution.py}`; `services/core-control-plane/src/fdai/core/executor/action_builder.py`; `services/core-control-plane/tests/scenarios/{test_v2026_07_replay.py,test_frozen.py}`; `uv run pytest -q --no-cov services/core-control-plane/tests/scenarios services/core-control-plane/tests/core/mscp_profile services/core-control-plane/tests/core/executor services/core-control-plane/tests/pipeline`가 통과했습니다. 새 고정 `not_yet_recorded` 사례는 보류를 되돌리면 계약 검증 오류로 실패하므로 변이 검증됩니다. | 관측 출처는 여전히 shadow에서 재생하는 프로세스 내 고정 공급자이므로 배포 런타임 관측과 나머지 묶음은 열려 있습니다. |
| 2026-08-31 | in-progress | `sre` 묶음에 `successful_full_loop` 근거를 확보했습니다. 고정된 `sre.cluster-diagnostics-missing.001` 시나리오는 이제 자체 증명을 하는 테스트 executor가 아니라 제품에 포함된 `ShadowExecutor` 위에서 실제 제어 루프로 재생되며, MSCP 기대 효과 공급자와 독립 효과 관측자를 제품 이음새에 연결합니다. 재생이 `executed` / `auto`로 종결될 수 있는 이유는 오직 독립적이고 권위 있는 관측이 발송 전 예측과 일치했기 때문입니다. 관측자는 조치와 예측만 받고 executor 증적, publisher, 감사 저장소는 전혀 보지 않으며, 관측자를 연결하지 않으면 shadow PR이 발행되어도 루프는 성공으로 인정하지 않습니다. 누락, 기한 초과, 불완전, 상충이라는 실패 시 차단 사례 4건은 각각 고정된 검증 상태, 사유, unscorable 또는 mismatch 응답 레이블과 함께 `abstain`으로 끝나고, 종단 발송 · 효과 검증 · 측정 감사 항목은 하나의 조치 및 이벤트 계보를 공유하며 모든 실행 결과와 발행된 PR은 shadow에 머무릅니다. 기한 초과 사례가 드러낸 결함도 여기서 고쳤습니다. 창을 벗어났거나 아직 기록되지 않은 관측이 발송 도중 계약 검증 오류를 일으키던 문제로, 이제 응답 결과 투영은 계약이 표현할 수 없는 관측을 버리고 shadow 효과 감사 항목이 원본 값을 그대로 보존합니다. | `current change`; `services/core-control-plane/src/fdai/core/mscp_profile/response_outcome.py`; `services/core-control-plane/tests/scenarios/enrichment/v2026.07/sre-cluster-diagnostics-missing-001.json`; `services/core-control-plane/tests/scenarios/{test_v2026_07_replay.py,test_frozen.py,manifests/v2026.07.json}`; `uv run pytest -q --no-cov services/core-control-plane/tests/scenarios services/core-control-plane/tests/core/mscp_profile services/core-control-plane/tests/contracts/test_response_outcome.py services/core-control-plane/tests/core/measurement services/core-control-plane/tests/core/workflow/test_outcome_verification.py services/core-control-plane/tests/pipeline/test_unified_control_loop.py services/core-control-plane/tests/pipeline/test_control_loop_e2e.py --deselect services/core-control-plane/tests/core/measurement/test_operational_promotion.py`로 집중 사례 485건이 통과했습니다. 제외한 파일 1개는 이 변경 이전에 이미 `4da5efd50`에서 실패하고 있었습니다. 기한 초과 사례는 투영 수정을 되돌리면 계약 검증 오류로 실패하므로 변이 검증까지 마쳤습니다. | 관측 소스는 shadow로 재생한 프로세스 내 고정 공급자이므로 배포 런타임의 권위 있는 관측과 재발 종결 근거는 여전히 열려 있고, 나머지 묶음 4개와 비합성 기준선 및 처리군도 마찬가지입니다. |
| 2026-08-31 | in-progress | 목표 간 충돌에 대한 남은 검토 지적 세 건을 해소했습니다. 자유 형식 ActionType-방향 대응표를 제거했습니다. 이제 각 선택지는 충돌 스키마가 검증하고 새 `DomainOptionEvidence` 계약이 수용하는 고정된 형식 지정 `objective_effects` 기록을 전달하며, 충돌 관계는 두 영역이 동일한 통제 목표 식별자에서 서로 반대 부호의 효용을 가질 때만 중재를 제기하는 독립 검사 `conflicting_objective_effects`입니다. 두 영역에 재생이 실제로 산출한 효과를 동일하게 부여한 음성 대조는 중재를 제기하지 않으므로, 권고 표기만으로는 충돌 관계를 만족시킬 수 없습니다. 각 재생의 표준 이벤트 식별자, 인용 규칙 식별자, 종결 감사 멱등 키, 사전 점검 영수증 다이제스트가 이제 선택지 근거 참조로 전문가 기여, 중재 요청, 의사결정 사례, 종결 판정까지 이어지며, 수용 지점은 계보가 합성 `specialist:*` 표식뿐인 페이로드를 거부합니다. 중재 소유자 부재 시 종결은 자동입니다. 구성 루트가 `runtime_health.bind_availability_probe`로 실행 가용성 탐침을 결속하고, Forseti는 방금 발행한 요청을 ActionType도 개시자도 승리 영역도 없는 명시적 `hil` 판정으로 종결하므로 어떤 시험도 그 종결을 직접 호출하지 않습니다. | `current change`; `services/core-control-plane/src/fdai/core/decision_case/domain.py`; `services/core-control-plane/src/fdai/agents/{forseti.py,_framework/forseti_decision_helpers.py,_framework/runtime_health.py,_framework/runtime.py}`; `services/core-control-plane/src/fdai/core/operational_planning/coordinator.py`; `services/core-control-plane/tests/scenarios/cross-objective/{schema.json,v2026.07-sre.json}`; `services/core-control-plane/tests/scenarios/test_v2026_07_replay.py`; `uv run pytest -q --no-cov services/core-control-plane/tests/scenarios services/core-control-plane/tests/agents/test_arbitration.py services/core-control-plane/tests/core/decision_case services/core-control-plane/tests/core/operational_planning services/core-control-plane/tests/core/operational_context` 명령이 집중 시험 396건을 통과했습니다. | 충돌 근거는 여전히 메모리 내 버스에서 실행한 shadow 재생이므로 배포 실행 중재 영수증은 남아 있고, 부호가 있는 목표 효과를 산출하는 출시 규칙 목록이 아직 없으므로 고정 보강이 이를 공급합니다. |
| 2026-08-31 | in-progress | 검토 후 아래에 기록한 목표 간 충돌 근거를 바로잡았습니다. 이제 중재는 직접 작성한 권고나 합성 대상을 받지 않습니다. 각 선택지의 권고는 해당 선택지의 재생이 실제로 생성한 ActionType에서 도출하며, 경합 대상은 재생된 두 대상이 함께 도달하는 단일 리소스를 고정된 온톨로지 그래프에서 도출합니다. Forseti는 공유 결정 근거 계약으로 승인된 운영 컨텍스트에 결합하므로, 중재 요청과 종단 판정 모두 정규 결정 사례와 그 컨텍스트 스냅숏 식별자, 도메인별 근거 참조를 함께 전달하며 Saga는 종단 판정을 추가 전용 감사 체인에 보존합니다. 중재 권한이 없는 경우에도 더 이상 종단 결과 없이 끝나지 않습니다. 이미 제공되는 `conflicts_require_hil` 저하 정책과 Forseti의 기존 규칙 미일치 판정을 거쳐, ActionType과 개시자와 실행 권한이 모두 없는 shadow 전용 `hil` 판정으로 종결하고 감사 근거를 남기며 새로운 권한을 만들지 않습니다. 고정된 충돌 산출물은 이제 매니페스트 `conflict_spec_ids` 필드를 통해 자체 ID로 직접 등재되며, 고정 스키마, GUID, 고객 데이터, ASCII, 중복 키 보호 장치가 이를 함께 검사합니다. | `current change`; `services/core-control-plane/tests/scenarios/cross-objective/{schema.json,v2026.07-sre.json}`; `services/core-control-plane/tests/scenarios/{test_v2026_07_replay.py,test_frozen.py,manifest.schema.json,manifests/v2026.07.json}`; `uv run pytest -q --no-cov services/core-control-plane/tests/scenarios services/core-control-plane/tests/agents/test_arbitration.py services/core-control-plane/tests/core/operational_planning services/core-control-plane/tests/core/operational_context services/core-control-plane/tests/core/decision_case services/core-control-plane/tests/core/risk_gate/test_precedence.py`로 집중 404건 통과. | 근거는 여전히 메모리 내 버스에서 실행한 shadow 재생이므로 프로덕션 버스의 배포 런타임 중재 증적은 열려 있으며, SRE `successful_full_loop`와 나머지 4개 묶음도 열려 있습니다. |
...
| 2026-08-29 | in-progress | 운영 인텔리전스 완료 경계를 다시 평가했습니다. Azure 발견은 기록된 카나리에 대해 실행 권한 없이 완전하며, 접근 권한으로 필터링한 하이브리드 문서 검색을 구현했습니다. 거버넌스 담당 체계, 승인된 인시던트 말뭉치, 수정 권한과 효과 훈련, 배포 소유 Knowledge 입력, 30-90일 집단 및 운영자 연결 근거는 아직 열려 있습니다. AWS, GCP 및 교차 클라우드 검증은 Azure 전용 구현 정책과 승인된 공급자 범위 부재로 차단된 상태를 유지합니다. | `current change`; Azure 발견 증적; 거버넌스, RCA, 실행, 문서 수집, 보안 및 CSP 중립성 원장; 집중 하이브리드 검색 검사 | 합성 근거를 운영 검증으로 사용하지 않고 아래의 순서가 지정된 외부 종료 근거를 충족합니다. |
...
- [x] 독립적이고 권위 있는 효과 관측에서만 종결하고 누락, 기한 초과, 불완전, 상충 근거에서는 실패 시 차단하는 재생으로 SRE `successful_full_loop` 차원의 근거를 확보했습니다. 근거: `services/core-control-plane/tests/scenarios/enrichment/v2026.07/sre-cluster-diagnostics-missing-001.json`, `test_v2026_07_replay.py`의 집중 전체 루프 테스트 6건, `test_frozen.py`의 고정 효과 근거 가드 2건.
- [ ] 현재 관측은 shadow로 재생한 프로세스 내 고정 공급자이므로, 배포 런타임의 권위 있는 관측 소스로 SRE `successful_full_loop`를 종결하고 재발 종결 근거를 추가해야 합니다.
- [x] 이미 제공되는 Forseti-Odin 중재 경계에서 런타임으로 근거를 확보한 경쟁 선택지를 종결해 SRE `cross_objective_conflict`를 완료했습니다. 근거: `services/core-control-plane/tests/scenarios/cross-objective/v2026.07-sre.json`, 효과가 일치할 때의 음성 대조와 자동 안전 실패 HIL 종결을 포함한 `test_v2026_07_replay.py`의 집중 충돌 테스트 5건, `test_frozen.py`의 고정 충돌 보호 장치 6건.
- [ ] 현재 근거는 메모리 내 버스에서 실행한 shadow 재생이므로, 프로덕션 이벤트 버스에서 해당 목표 간 충돌의 배포 런타임 중재 증적을 확보해야 합니다.
- [ ] 충돌 관계가 읽는 `objective_effects` 보강을 고정 충돌 명세가 더 이상 공급하지 않도록, 출시된 규칙이나 계획 산출물이 부호가 있는 목표 효과를 직접 산출하게 합니다.
- [x] 기능별 FDAI-CONST-005 결과를 완료했습니다. 필요한 결과는 SRE 복구 및 재발 종결, ARB 승인 조건 및 변경 후 검증, 보호된 신뢰성 목표를 유지한 FinOps 실현 절감, 측정된 RTO와 RPO를 포함한 DR 데이터 무결성 검사, 지속적인 중지 가드와 검증된 복구를 포함해 명시적으로 사람이 승인한 Chaos 주입입니다. 근거: `services/core-control-plane/tests/scenarios/manifests/v2026.10.json`, `test_v2026_10_outcomes.py`, `test_v2026_10_frozen.py`의 검토된 재생 상속 검사.
- [x] 통제된 `BaselineTreatmentCohortReceipt`와 결정론적 평가기를 정의하고, 고정된 개정 하나와 동일한 고정 시나리오 집합 위에서 갈래마다 30개 이상인 비합성 표본, 완전한 메트릭, 구간, 출처, 임계값 0 가드, 최신 독립 의사 결정 근거 승인을 모두 갖춘 통제된 외부 코호트만 적격이 됨을 입증했습니다. 근거: `packages/service-contracts/src/fdai_service_contracts/baseline_cohort.py`, `services/core-control-plane/src/fdai/core/measurement/baseline_cohort_claim.py`, `tools/cohort_receipt.py`와 집중 계약, 주장 및 실행기 테스트.
- [x] 평가 대상 코호트 요건을 근거가 아니라 신뢰 버전 관리 저장소 정책에서 로드하고, 모든 승인을 평가 대상 갈래 사실 전체의 정규 다이제스트에 결속했으며, 저장소 산출물이 자신의 요건도 자신의 승인도 제공할 수 없음을 입증했습니다. 근거: `config/sre-cohort-claim-policy.json`, `services/core-control-plane/src/fdai/core/measurement/cohort_claim_policy.py`, `packages/service-contracts/src/fdai_service_contracts/baseline_cohort.py`의 `cohort_arm_fact_digest`, `services/core-control-plane/src/fdai/core/measurement/baseline_cohort_claim.py`의 공급자 전용 및 검증기 전용 승인 연결부, 그리고 지어낸 다이제스트, 신뢰 입력, 저장소 출처 부정 사례를 포함한 집중 정책, 계약, 주장 및 실행기 테스트.
- [x] 코호트 근거 계약에서 `artifact_origin`을 제거하고 출처를 신뢰할 수 있는 반입 채널에서만 도출하며, 갈래 승인 두 건에 더해 증적 다이제스트 전체에 결속한 신뢰 코호트 수준 승인을 요구하고, `fdai_revision`을 증적과 신뢰 요건과 CLI 모두에서 변경 불가능한 40자 또는 64자 16진수 커밋 다이제스트로 고정했습니다. 근거: `packages/service-contracts/src/fdai_service_contracts/baseline_cohort.py`의 `CommitRevision`, `import_origin`, `cohort_not_admitted`, `services/core-control-plane/src/fdai/core/measurement/baseline_cohort_claim.py`의 `admitted_cohort_claim_digest`, `services/core-control-plane/src/fdai/core/measurement/cohort_claim_policy.py`의 `require_commit_revision`과 이름표 변경 산출물, 출처 선언, 조작한 코호트 승인, 이동 가능한 개정 부정 사례를 포함한 집중 계약, 정책, 주장 및 실행기 테스트.
- [ ] 검토된 기준군 및 처리군 exporter workflow를 군별 `allowed_exporter_workflow_paths` 항목과 함께 추가한 뒤, 동일한 운영 프로토콜 아래에서 표본 크기, 신뢰 구간, 절대값 및 지원되지 않는 배수 주장이 없는 참조 기준선과 FDAI 처리를 보존합니다.
...
- [ ] 프로덕션 공급자와 보호된 승인 워크플로를 통해 `config/decision-boundary-inventory.json`의 경계마다 통제된 최신 실제 묶음을 하나씩 보존한 다음, 합성 또는 불완전 근거가 실제 운영 주장을 충족하지 못한다는 결과와 함께 `python3 scripts/quality/architecture/check-decision-boundary-coverage.py` 통과 결과를 기록합니다. 공급자 구현과 프로덕션 조립은 현재 변경에서 완료되었지만 실제 기록이 생길 때까지 이 항목은 열린 상태로 유지합니다.
- [x] 공급자 중립 `OperationalCoverageReceipt`를 정의하고 집중 테스트를 통해 분모,
  처리 결과, 최신성, 임계값, 무관용 조건 및 다이제스트 불변 조건을 입증합니다.

#### 운영 인텔리전스 99% 완료 차단 요인

각 후속 주장이 앞선 관리 입력에 의존하므로 다음 항목을 순서대로 해결합니다:

1. [ ] 명시적인 범위와 최종 책임 담당 체계가 포함된 부서 검토 완료 Azure 거버넌스
   기준선을 로드하고, 정확히 같은 기준선과 범위에 대한 관리되는 실제 드리프트 증적을
   보존합니다.
2. [ ] 승인된 APM 또는 서비스 의존성 출처와 승인된 인시던트 말뭉치를 연결하고, 검토된
   결정론적 애플리케이션 대 인프라 분류를 구현한 다음 실제 정확도, 판단 보류 및 지연 시간
   근거를 보존합니다.
3. [ ] 현재 사람 승인 또는 유효한 standing 권한이 적용되는 승격된 ActionType을 선택하고,
   테스트를 마친 롤백 대상과 독립 효과 검증 훈련을 증명합니다. 그런 다음 실제 영향 범위
   근거가 `winning_axis=live_blast`를 만들고 권한을 낮추며 변경을 일으키지 않는 관리되는
   shadow 증적을 보존합니다.
4. [ ] 배포가 소유하는 문서 커넥터와 ACL 출처를 구현된 접근 권한 필터 하이브리드 검색
   경로에 연결하고, 권한 확인, 인용, 삭제, 정확도 및 지연 시간에 대한 관리되는 PostgreSQL
   말뭉치 근거를 보존합니다.
5. [ ] 검토된 30-90일 구간 동안 고정된 배포 개정 번호 하나를 관찰하고, 모든 무관용 보호
   지표가 0인 완전한 6개 도메인 99% KPI 증적을 보존합니다.
6. [ ] 범위가 지정된 읽기 전용 실제 검증 전에 Kubernetes Private DNS 경로를 해결하고
   승인된 고객 Azure 프로필의 대화형 재인증을 완료합니다.
7. [ ] Azure 전용 정책이 개정되고 승인된 계정, 신원 및 ActionType 범위가 생길 때까지 AWS와
   GCP 구현 및 실제 검증을 차단된 상태로 유지합니다.
8. [ ] 관리되는 AWS 및 GCP 생성기가 존재하고 권위 있는 근거를 공급할 수 있을 때까지 교차
   클라우드 온톨로지 검증을 차단된 상태로 유지합니다.
...
> 상속한 결정론적 재생 근거와 함께 검토된 기능별 결과 테스트를 인용합니다. 별도의 운영 증적을
> 확보하기 전에는 배포 런타임이나 측정된 프로덕션 기준선의 완전한 도메인 커버리지를 주장하면
> 안 됩니다.
