...
translation_source_sha: d40f7743b8000b24f3bf8a22544f7a098a9eb1b8
...
| 독립적인 의사 결정 근거 검증 | in-progress | `decision_evidence_verification.py`; `core/readiness/decision_evidence.py`; `delivery/persistence/state_store_decision_evidence.py`; `delivery/azure/decision_evidence.py`; `.github/workflows/decision-evidence-admission.yml`; 집중 계약, 영속성, 조립, 워크플로 및 Azure 어댑터 테스트 | 이제 프로덕션 조립은 준비 상태, 의미 기반 조회, 워크플로, 인과 관계, 운영 맥락, 분석기 및 승격 경로에 영속적인 정확한 일치 승인 조회를 연결합니다. 보호된 워크플로는 정확한 배포 근거를 독립적으로 다시 확인하고 내용 기반 증명, 묶음, 요건 및 승인 기록을 보존합니다. 등록된 모든 경계의 통제된 실제 묶음은 아직 남아 있으므로 기록이 없거나 만료되면 의사 결정을 검토 보류로 처리합니다. |
| 고정된 시나리오 집합 계산 | in-progress | `tests/scenarios/manifest.schema.json`; `tests/scenarios/manifests/v2026.09.json`; `test_v2026_09_frozen.py`; `test_v2026_09_replay.py` | 다섯 묶음 모두 일반 재생 커버리지를 갖추었고 현재 상태는 모두 `partial`이며, 집계 상태는 `incomplete`입니다. FDAI-CONST-005의 기능별 결과 근거가 생기기 전에는 완료 상태가 될 수 없습니다. 스키마는 묶음과 집계의 완료 상태를 양방향으로 결정하므로 일반 커버리지나 모순되는 하위 상태로 완료 여부를 잘못 나타낼 수 없습니다. |
...
- [ ] 기능별 FDAI-CONST-005 결과를 완료합니다. 필요한 결과는 SRE 복구 및 재발 종결, ARB 승인 조건 및 변경 후 검증, 보호된 신뢰성 목표를 유지한 FinOps 실현 절감, 측정된 RTO와 RPO를 포함한 DR 데이터 무결성 검사, 지속적인 중지 가드와 검증된 복구를 포함해 명시적으로 사람이 승인한 Chaos 주입입니다. 인용하는 집중 테스트가 존재하고 통과한 뒤에만 각 결과를 `services/core-control-plane/tests/scenarios/manifests/v2026.09.json`에 기록합니다.
...
> **현재 커버리지 공백:** `services/core-control-plane/tests/scenarios/manifests/v2026.09.json`은
> 모든 고정본을 SRE, ARB / 변경 안전성, FinOps, DR 또는 Chaos에 할당하지만, 다섯 묶음과
> 집계는 모두 `incomplete` 상태를 유지합니다. 일반 재생 커버리지는 FDAI-CONST-005의
> 기능별 결과를 입증하지 않습니다. 매니페스트가 각 필수 결과에 대해 통과한 근거를 인용할
> 때까지 FDAI는 완전한 도메인 커버리지를 주장하면 안 됩니다.
