...
| **Ontology instance reads** | `OntologyInstanceStore.query_objects(...)` and `scan_objects(...)` in `shared/providers/ontology_instance.py` | - | in-memory and PostgreSQL adapters implement bounded object-id, property, optional relationship, and one-connection candidate snapshot reads | replace the store while preserving the 1,000-object relationship-query limit, 50,000-object candidate ceiling, one-snapshot source generation, truncation, and no-authority graph receipts |
