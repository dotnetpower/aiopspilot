...
- **Operational catalog review and measurement**: `DeterministicCatalogValidator` reuses the
  ...
  Its protected workflow injects exact policy fields, accepts strictly typed metric numbers, and persists idempotently; empty arm-specific exporter allowlists keep the path unavailable until a reviewed exporter and policy entry land together.
