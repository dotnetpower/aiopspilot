...
- **Operational catalog review and measurement**: `DeterministicCatalogValidator` reuses the
  ...
  Its protected workflow rejects duplicate JSON keys, injects strict arm, revision, run, path, and time context, and persists idempotently; arm-specific exporter allowlists accept only regular in-repository workflows and otherwise keep the path unavailable.
