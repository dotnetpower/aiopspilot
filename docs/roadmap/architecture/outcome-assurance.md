...
| ActionType canonical content digest | implemented | `core/measurement/operational_promotion.py`; `core/measurement/__init__.py`; focused promotion tests | `action_type_digest` is a public function that computes a deterministic SHA-256 content digest of an ActionType excluding provenance. The evaluator uses this digest to detect schema drift between sealed batches and the current ActionType definition. |
...
|------|-------|--------|----------|-----------|
| 2026-09-09 | implemented | Bound each required cohort metric to its reviewed 95% interval method. Auto-resolution uses Wilson; continuous and count means use deterministic bootstrap, so a policy cannot reinterpret MTTR, change lead time, cost, or human touchpoints as Bernoulli rates. | `current change`; cohort policy loader and focused policy tests. | Retain and independently admit the real operational cohort. |
| 2026-09-01 | implemented | Promoted `_action_type_digest` to the public `action_type_digest` function and exported it from `core/measurement/__init__.py`. The canonical content digest excludes provenance and enables external callers to verify ActionType consistency during promotion evaluation. | `current change`; `core/measurement/operational_promotion.py`; `core/measurement/__init__.py`; focused promotion tests. | Bind authenticated authoritative source receipts into the broader Outcome Assurance projection and retain a governed live cohort. |
