...
|------|-------|----------|-------|
| Pod lifecycle detection reduction and projection split | implemented | `fdai/core/readiness/detection_lifecycle.py`; `fdai/delivery/detection_lifecycle_state.py`; `fdai_operator_service/detection_lifecycle_projection.py`; `check-independent-services` OK | Core owns the reduction and the single `runtime:detection-lifecycle:` writer; the Operator service validates and aggregates the stored projection for its existing authenticated `/detection-readiness` family and imports no `fdai.*` module to do it. A defective stored row makes the section unavailable with a named reason instead of being reinterpreted by the reader. |
...
| System Knowledge Service candidate | in-progress | `services/system-knowledge-service/`; `packages/service-contracts/src/fdai_service_contracts/system_knowledge.py`; focused service checks in the current change | The independent read-only package and image exist without changing the validated five-service baseline. Production graduation evidence remains open. |
...
| Recorded-state read projection | validated | Core and Azure inventory projection; Operator recorded-state projection; Console decoder and reason-aware presentation; focused and live evidence | The Operator remains a read-only converter over the active inventory generation. The Console does not query Azure, infer health, or gain mutation authority. |
### Implementation history

| Date | State | Change | Evidence | Remaining |
|------|-------|--------|----------|-----------|
| 2026-09-09 | in-progress | Approved and implemented the System Knowledge Service as a sixth read-only candidate instead of merging repository knowledge into Core, Operator Service, or the A3 edge. | `current change`; package, contract, catalog, Teams mention, ledger, image, and focused test paths. | Record persistent-volume, identity, cost, provider canary, disable, and timed rollback evidence before production graduation. |
| 2026-09-06 | validated | Added reason-aware recorded Resource state across existing Core, delivery, Operator, and Console ownership without adding a service, writer, or cross-service implementation import. | `current change`; focused boundary checks, live ARG collection into local PostgreSQL, and authenticated standard-port browser validation. | No service-graduation work remains for this bounded read path. |
| 2026-09-06 | validated | Classified the newly merged `llm-model-deployment` type in the existing Operator read projection without changing service or writer ownership. | `current change`; exact catalog parity and 268 focused backend checks passed on the merged main snapshot. | Future ResourceType additions must update the same read contract. |
| 2026-09-06 | implemented | Distinguished the unbound Azure Resource Health source for Application Insights and Log Analytics from generic provider unavailability. | `current change`; the existing Operator read projection and Console localization changed without adding provider calls, persistence, or authority. | Persist Resource Health observations through a separately reviewed source binding before displaying availability values. |
| 2026-08-31 | implemented | Kept Pod lifecycle detection reduction in Core and left the Operator service a validating projection over `runtime:detection-lifecycle:`, so the reporting surface added for issue #291 crossed no service boundary and added no shared module. | `current change`; `check-independent-services` OK (services=5, service_forbidden=0), `check-operator-api-boundaries` OK, and 122 focused Python tests passed, including an 8-test cross-service proof pinned at one revision. | Live Pod lifecycle evidence still reaches Core only through the `FDAI_POD_LIFECYCLE_EVIDENCE_JSON` composition seam. |
...
| System Knowledge Service | Approved for independent implementation; production graduation pending | A dedicated bot keeps repository-derived release artifacts, Teams identity, and search failures outside Core, Operator Service, and the operational A3 edge. The package and image may proceed, but production enablement still requires persistent claim storage, exact-image canary, cost, identity, and timed rollback evidence from the [System Knowledge Service](../interfaces/system-knowledge-service.md). |
...
| `state_kv` `runtime:detection-lifecycle:` records | Core analyzer tick through the Pod lifecycle recorder | Authenticated Operator API `/detection-readiness` lifecycle projection, read-only | Alembic migration job |
