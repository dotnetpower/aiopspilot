...
translation_source_sha: 5bdcfb26ff6d5c80b23267ded4c2bb9ad9d19529
...
| 실행기 안전조건과 독립 효과 종결 | in-progress | `packages/service-contracts/src/fdai_service_contracts/execution_safeguards.py`; `schemas/execution-safeguard-proof-bundle/1.0.0.json`; `test_execution_safeguards.py`; `core/executor/safeguards.py`; `shared/providers/resource_lock.py`; `test_resource_lock_receipt.py`; `config/constitution-traceability.json`의 `FDAI-CONST-007` 요구 사항; 이슈 `#81`, `#620`, `#627`, `#628`, `#633`, `#660`, `#664` | 공급자 중립 7개 증명 wire 묶음과 공급자가 증명한 과거 및 현재 잠금 근거 계약을 구현했습니다. 이 계약은 권한이나 효과 검증 주장을 부여하지 않습니다. #660 최종 처리기가 정확한 현재 소유권 평가를 사용할 때까지 이슈를 열어 둡니다. Core 및 작업 흐름 생성기, Isolated 실행기 검증, 별도 승인된 통제된 교차 경로 효과 근거도 열린 작업으로 남아 있습니다. |
...
- [ ] 하위 이슈 `#660`에서 순수 전체 액션 안전조건 증명 최종 처리기가 정확한 현재 `LiveLockOwnershipAssessment`를 사용하게 합니다. 기존 최종 처리기와 통과한 집중 검사 63개는 구현 근거로 유지하지만, 공급자가 증명한 현재 소유권을 입증하지는 않습니다.
