...
translation_source_sha: c2c4289258dccca92aa43a188007cd67029db7a5
...
| 실행기 안전조건과 독립 효과 종결 | in-progress | `packages/service-contracts/src/fdai_service_contracts/execution_safeguards.py`; `core/executor/{safeguards,safeguard_proofs,lock,lock_continuity,idempotency_reservation,audit_intent}.py`; `delivery/persistence/postgres_idempotency_reservation.py`; `shared/providers/resource_lock.py`; 집중 안전조건, 잠금, 예약, 영속성, 이행 테스트; `config/constitution-traceability.json`의 `FDAI-CONST-007` 요구 사항; 이슈 `#81`, `#627`, `#628`, `#633`, `#678`, `#680`, `#681`; 완료된 이슈 `#620`, `#660`, `#664`, `#669`-`#672`, `#674`, `#679` | 공급자 중립 증명 묶음, 잠금 근거, 최종 처리기, 잠금 수명 주기, 로컬 테스트 공급자, 소유권 연속성 모델, 장애에 안전한 예약 계약 및 PostgreSQL 저장소, 감사 의도 계약을 권한이나 효과 검증 주장 없이 구현했습니다. PostgreSQL 근거 잠금 및 감사 저장소, 공유 조정, 실제 생성기, Isolated 실행기 검증, 통제된 효과 근거는 열린 작업으로 남아 있습니다. |
...
- [ ] 이슈 `#678`에서 운영 PostgreSQL 근거 리소스 잠금을 구현합니다.
...
- [ ] #679 다음에 이슈 `#680`에서 영구 PostgreSQL 감사 의도 저장소를 구현합니다.
- [ ] #678-#680 다음에 이슈 `#681`에서 공유 근거 수명 주기 조정기를 구현합니다.
