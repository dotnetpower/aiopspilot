...
| Deployed model inventory discovery | Read-only collection, authoritative rate-limit TPM normalization, separate requested/current capacity with transition state, readable model/SKU/status projection, bilingual semantic lookup, complete resource taxonomy, and ontology projection of existing Azure AI model deployments under their parent accounts, with normal scope, freshness, completeness, and no-execution constraints. Conversational deployment requests produce no ActionType. | [resource type](../../../rule-catalog/vocabulary/resource-types.yaml), [resource class](../../../rule-catalog/vocabulary/resource-classes.yaml), [model deployment projection](../../../services/core-control-plane/src/fdai/delivery/azure/model_deployment.py), [Azure ARG projection](../../../services/core-control-plane/src/fdai/delivery/azure/arg_query.py), [provider relationship mapping](../../../rule-catalog/vocabulary/provider-relationship-mappings/azure-arg-v1.yaml), [architecture map](../../../console/src/components/architecture-map.model.ts), and [Operator rows](../../../services/operator-service/src/fdai_operator_service/families/conversation/presentation_rows.py) | [model deployment tests](../../../services/core-control-plane/tests/delivery/azure/test_model_deployment.py), [semantic planning tests](../../../services/core-control-plane/tests/conversation/test_semantic_planning.py), [Azure projection tests](../../../services/core-control-plane/tests/delivery/azure/test_arg_query.py), [resource class tests](../../../services/core-control-plane/tests/rule_catalog/test_resource_class.py), [architecture-map tests](../../../console/src/components/architecture-map.model.test.ts), [service-suite tests](../../../tests/integration/scripts/test_service_test_suites.py), and [Operator row tests](../../../services/operator-service/tests/test_presentation_rows.py) |
...
| Assurance Twin posture/review recorder, durable ledger, Operator API, and Console panel (partial: no trusted producer) | Unbound bounded `agent.operational-activity` tip (`assurance-twin.posture` kind, schema `1.2.0`, no authority) built for a computed `PostureAssessmentReport` or an ambient `IacReview`; the published schema pins Heimdall ownership, the `assurance-twin` producer, and a null observation domain. Categorical source labels and SHA-256 correlation and review identities keep caller-supplied scope, repository references, review keys, and correlation values out of the activity record and durable provenance. The tip is only ever published for a posture report - a change review's tip is built and returned for audit use only, never published, because an unordered async publish cannot be atomically ordered against the ledger's durable CAS-based conflict tombstone; the durable body persists in the existing `state_kv` ledger (latest-wins per scope, idempotent per `review_key`, durable conflict tombstone on a mismatched redelivery) with activity, correlation, and evidence-digest provenance, and every write is rejected above the Operator API projection's own bounds (`review_key` <= 256 chars, <= 200 findings, <= 200 bounded/unique/non-blank <= 512-char `reason_codes`/`evidence_refs`, non-blank <= 512-char `evidence_source_revision`); the Operator API renders it verbatim through `/assurance-twin/posture`, `/assurance-twin/reviews`, and `/assurance-twin/review?review_key=`, while `/system/data-sources` assigns all three routes to the service-local operational projection and reports an explicit unavailable reason when it isn't configured. Stale, unavailable, unknown, malformed, digest-mismatched, or conflict-tombstoned rows remain explicit gaps; the Console `assurance-twin` route renders the posture KPIs, review list, opaque-key drill-down, and withheld-evidence gaps. No shipped component writes these rows: an ambient ingress payload is not trusted evidence, so the recorder stays unbound until a trusted producer exists | [posture activity](../../../services/core-control-plane/src/fdai/core/assurance_twin/posture_activity.py), [delivery recorder](../../../services/core-control-plane/src/fdai/delivery/assurance_twin_posture.py), [durable ledger](../../../services/core-control-plane/src/fdai/delivery/persistence/state_store_assurance_twin_posture.py), [Operator projection](../../../services/operator-service/src/fdai_operator_service/assurance_twin_posture_projection.py), and [Console route](../../../console/src/routes/assurance-twin.tsx) | [posture activity tests](../../../services/core-control-plane/tests/assurance_twin/test_posture_activity.py), [delivery tests](../../../services/core-control-plane/tests/delivery/test_assurance_twin_posture.py), [ledger tests](../../../services/core-control-plane/tests/persistence/test_state_store_assurance_twin_posture.py), [Operator projection tests](../../../services/operator-service/tests/test_assurance_twin_posture_projection.py), [Operator reader tests](../../../services/operator-service/tests/test_runtime_projection_reader.py), and [Console tests](../../../console/src/routes/assurance-twin.test.ts) |
