...
| AKS fleet observation binding | implemented | `infra/main.tf`; Container Apps Inventory Job; focused AKS identity tests | One sensitive JSON value supplies 1-32 exact workload-identity bindings. Terraform assigns only AKS RBAC Reader per exact cluster and passes no bearer token. |
