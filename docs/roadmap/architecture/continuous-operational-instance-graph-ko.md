...
translation_source_sha: 68b6295fbbffc85d8ee06340f741570623662c90
...
| Archive lifecycle | implemented | Content-addressed 매니페스트, 비공개 Azure Blob writer, principal 범위의 검증된 reader, database gate 기반 source purger, 추가 전용 verification, restore, coverage, hold 및 purge 증적, 전용 고정 shadow Container Apps Job을 구현했습니다. 보호된 인증은 정확한 source attestation 검증을 위해 GitHub API와 registry 자격 증명을 분리해 연결하고 Terraform ACR 출력 또는 검증된 배포 Job 이미지를 Azure login host로 정규화하며 같은 digest를 다시 빌드하지 않고 명시적으로 가져올 수 있습니다. OI-12 인벤토리 새로 고침은 최상위 출력을 우선 사용하며, 출력이 없으면 mode 0600의 특정 시점 상태 스냅샷에서 추적된 `module.compute.azurerm_container_app_job.inventory[0]` 주소 하나만 정확히 확인하고 단계가 끝날 때 스냅샷을 삭제합니다. Job 이름에서 신원을 유추하지 않습니다. 보호된 계획은 이전 archive data owner를 보존하고 저장소에 바인딩된 deploy UAMI를 별도 주소에 추가하며 제거는 별도의 파괴적 작업으로 유지합니다. |
## 운영 상태 전이 원장
