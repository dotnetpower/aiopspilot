...
객체 전용 ObjectSet은 저장소에서 직접 평가할 수 없는 조건식을 적용할 수 있습니다. 구체화기는
먼저 관계를 제외한 일반 객체 1,000개 구간을 확인합니다. 이 구간이 완전하거나 더 작은 결과 제한을
이미 증명하면 즉시 종료합니다. 완전성을 확인해야 하는 읽기만 하나의 저장소 연결에서 관계를 제외한
...
운영 상태 ResourceType 허용 목록을 재사용하지 않습니다. 구체적인 전원 또는 런타임 상태를 지정한
조회는 운영 상태 축만 유지합니다. 구체적인 가용성 상태는 같은 정규화 값이 provider 가용성 또는
운영 상태에 나타날 수 있으므로 두 축을 모두 읽습니다. 두 경로 모두 구간 coverage가 불완전해도
검증된 일부 전이를 보존하며, 누락된 coverage를 완전한 0건 결과로 바꾸지 않습니다. 변환된 각 전이는
명시적인 `conflict_free` 판정을 포함하므로 정본 직렬화에서 빈 충돌 목록을 생략해도 검증된 근거가
미확정 행으로 바뀌지 않습니다.
