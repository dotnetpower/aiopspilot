...
| Capacity graduation advice | Freyr combines complete capacity evidence with current Njord cost evidence through a versioned provider-neutral policy | Graduation remains a shadow recommendation judged by Forseti. It cannot dispatch a topology change without a separately reviewed ActionType. |
...
| Catalog assets | Cost-category rules under `rule-catalog/catalog/` and `cost-aware-remediation.yaml` | The migration must preserve stable ids and decide whether each generic asset stays in the base catalog or moves into the package. CI serializes schema-mutating lifecycle tests and a forward service migration restores the root-owned T2 lookup index. |
