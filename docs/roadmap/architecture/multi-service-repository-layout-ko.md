...
translation_source_sha: f389853674175b8be54e36635e496ad657c6ea33
translation_revised: 2026-09-10
---
# 다중 서비스 저장소 레이아웃

FDAI는 하나의 개발 저장소에 독립적으로 패키징하고 검증한 백엔드 서비스 5개, 독립 패키지
읽기 전용 서비스 후보 1개 및 공유 서비스 계약 SDK를 둡니다. 루트는 도구와 통합 테스트를
조정하지만 모놀리식 런타임 distribution이 아니며 어떤 서비스도 다른 서비스의 구현을 가져오지
않습니다.
...
| 표면 | 소유권 계약 |
|------|-------------|
| 백엔드 서비스 | 각 `services/*` 루트가 자체 `pyproject.toml`, 소스 패키지, 테스트 및 이미지를 소유합니다. 검증된 5개 서비스 기준선은 프로세스 신원과 migration branch도 소유합니다. 시스템 지식 서비스는 승격 근거가 완료될 때까지 배포 gate를 유지합니다. |
...
| 저장소 루트 | 루트 `pyproject.toml`과 `uv.lock`은 개발 도구와 서비스 간 통합을 조정합니다. 루트 pytest 경로는 배포가 차단된 시스템 지식 서비스 후보를 포함한 모든 서비스 소스 루트를 포함하며, 배포 CLI를 uv workspace에 추가하지 않고도 독립 설치형 CLI를 가져올 수 있습니다. 루트는 FDAI 런타임 배포판을 발행하지 않습니다. `pytest-timeout`은 개별 테스트당 120초 벽시계 상한을 적용하여 단일 테스트가 xdist 샤드를 무기한 차단하지 못하게 합니다. |
...
│   │   ├── rca/                # event-time topology와 관리 문서를 사용하는 grounded T0/T1/T2 RCA, 작업 권한 없음
