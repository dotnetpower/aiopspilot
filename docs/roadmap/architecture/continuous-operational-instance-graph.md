...
| Archive lifecycle | implemented | Content-addressed manifests, the private Azure Blob writer, principal-scoped verified reader, database-gated source purger, append-only verification, restore, coverage, hold, and purge receipts, and a dedicated fixed-shadow Container Apps Job are implemented. Protected certification binds separate GitHub API and registry credentials to exact-source attestation verification, normalizes the Terraform ACR output or verified deployed Job image to an Azure login host, and can explicitly import the same digest without rebuilding. The OI-12 inventory refresh prefers the root output and otherwise enumerates at most 64 generic ARM Job IDs inside the exact platform resource group, reads each through the stable Container Apps `2024-03-01` API under a 30-second bound, and requires one container with the reviewed inventory name, command, and empty arguments; it never derives identity from a Job name or persists provider output. A protected plan preserves the prior archive data owner and adds the repository-bound deploy UAMI at a separate address; any retirement remains a separate destructive operation. |
