...
어댑터는 열린 파일 handle 하나에서 `max_bytes + 1`바이트보다 많이 읽지 않으므로, metadata 확인과
콘텐츠 읽기 사이의 교체나 증가로 구성된 크기 제한을 우회할 수 없습니다.
JSON 객체 키도 고유해야 합니다. 중복 키를 정규 해싱 전에 last-write-wins 방식으로 접을 수 없습니다.

이 바인딩은 **기본으로 켜져 있습니다**. Core 이미지가 승인된 일반 출처를
`/app/config/operating-intent/generic-source.json`에 함께 제공하고, 배포가 재정의하지 않는 한
Terraform 호출부가 그 경로, 개정 번호, digest, 타입별 개수를 고정합니다. 고정값은 객체 수준 `default`
블록이 아니라 각각의 `optional(...)` 속성에 둡니다. 호출자가 속성 하나라도 지정하면 Terraform이 객체
수준 default를 무시하기 때문이며, 그 덕분에 `revalidate_seconds`만 조정해도 승인이 조용히 꺼지지 않고
바인딩이 유지됩니다. 함께 제공되는 경로만 바꾸고 개정 번호와 digest를 바꾸지 않으면 즉시 거부합니다.
함께 제공되는 산출물의 자리표시자 참조와 의도적으로 유효하지 않은 change window 때문에, 이 출처를
바인딩하면 intent의 형태만 공급되고 권한은 부여되지 않습니다.
...
시작을 중단하거나 이전 승인을 만료까지 살려 두는 대신 반환 전에 격리를 durable하게 기록합니다.

## 소비자 울타리

권한 소비자는 투영된 객체를 그대로 신뢰하지 않고, 세 가지 울타리를 거쳐 판단 시각의 승인 기록을
확인합니다.

**바인딩.** 소비자는 자신의 구성이 고정한 정확한 개정 번호와 문서 전체 digest를 지니므로, 다른
바인딩이 쓴 기록은 형식이 맞다는 이유로 수용되지 않고 거부됩니다. 출처를 구성하지 않은 소비자는
명시적으로 unconfigured입니다. 이 소비자는 기록을 아예 읽지 않으며, 이것이 기존 일반
`FDAI_OPERATING_MODEL_PATH` 동작을 유지하는 유일한 방법입니다. 반대로 구성된 소비자에게는 기록
*부재*가 거부입니다. 삭제, 손상, 첫 기록 이전 구간은 서로 구분할 수 없고 그중 어느 것도 아무것을
증명하지 않기 때문입니다.

**세대.** `FDAI_OPERATING_INTENT_SOURCE_GENERATION`(Terraform 필드
`operating_intent_source.generation`, 고정된 바인딩이 바뀔 때마다 올립니다)은 그 밖에는 똑같이
유효한 두 바인딩의 순서를 정합니다. 롤링 배포 중 물러나는 replica의 재검증 worker는 더 높은 세대의
...
둘 다 이 고정값의 보호를 받지 않으므로, *허용* window는 승인 기록이 그 식별자를 소유할 때만 열립니다.
*차단* window는 어느 출처가 공급했든 계속 차단합니다. 행동하지 않는 쪽은 결코 안전하지 않은 방향이
아니기 때문입니다. 격리·만료·이용 불가·형식 오류 승인은 소유권을 전혀 지정하지 않으므로 유지보수 표면
전체를 거부합니다.

## Replica 직렬화

매니페스트 확인, 중단된 적용 복구, 투영, 승인 기록은 모두 고정된 `operating-intent-source:apply`
