...
수집기는 기계적인 읽기 플레인 어댑터입니다. 수집기는 Huginn 수신을 통해 타입이 지정된 근거를
게시합니다. Heimdall은 출처, 범위, 스키마, 기준 시점 및 완전성을 검증합니다. 결정론적 축약기는
최종 근본 원인 판정이 아니라 T0 가설 근거를 생성합니다. Forseti는 근본 원인의 최종 책임자로
유지되며 기존 신뢰 계층 및 근거 계약을 통해 가설을 수락, 기각 또는 보류합니다. Saga는 수락된
모든 증적과 가설 전환을 기록합니다.
