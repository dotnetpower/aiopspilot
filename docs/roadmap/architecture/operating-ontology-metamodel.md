...
| D3 | Add complete, missing-endpoint, reversed-input, duplicate, preview, and partial-coverage fixtures. | Only verified links enter the active graph; ambiguous or incomplete paths remain absent and reported. |
...
Incremental candidate rebuild expands a changed type or type-version through only its transitive
relationship references. Unrelated candidate components remain reusable. A reviewed promotion
ledger persists both immutable graph generations, the complete D4 comparison, and every distinct
review assessment before appending the proposal history. Duplicate review delivery is idempotent,
and neither the history nor its active proposal pointer can carry graph or migration authority.
...
| Prospective lineage and evidence-conflict revisions | implemented | `ProspectiveLineage.yaml`; `EvidenceConflict.yaml`; Forseti, Heimdall, Muninn, and Saga focused checks | Content-addressed prospective records are finalized before execution. Conflict revisions are immutable, superseding, exact-target records that can only preserve or lower authority. |
...
| Versioned relationship candidate materialization | implemented | `provider_schema_relationship_generation.py`; `provider_schema_relationship_ledger.py`; `provider_schema_relationship_review.py`; `provider_relationship_mapping.py`; `provider_schema_watcher.py`; focused generation, fixture, D4, and ledger tests | Exact provider-schema and REST evidence digests, recomputed review digest, every semantic mapping field including catalog cardinality, provider type@version identity, mapping revision, projection manifest, explicit direction/cardinality/link metadata, changed-type plus transitive-reference invalidation, bounded candidates, rollback, replay, and watcher receipt authority are content addressed. Complete observed endpoints prove canonical `contains`, `attached_to`, and `depends_on`; missing or partial endpoints remain absent. The reviewed-promotion ledger retains both immutable contexts, comparison, and append-only review history. Active pointers and history cannot be tampered into graph or migration authority. |
...
|------|-------|--------|----------|-----------|
| 2026-08-31 | implemented | Completed the local D2-D4 relationship lifecycle: the full infrastructure fixture is order-invariant, complete observed endpoints prove canonical `contains`, `attached_to`, and `depends_on`, incremental invalidation follows only transitive references, and reviewed promotion history retains immutable prior/aligned contexts plus the exact comparison. All proposal, graph, execution, and migration authority remains false. | `current change`; provider relationship generation and ledger, direction generation serialization, infrastructure and preview fixtures, D4 comparator/promotion tests, and focused schema/projection/replay checks. | Retain a complete release-bound real-generation D4 review and the separately governed provider-schema/Saga runtime receipts; no external evidence is claimed. |
...
| 2026-08-27 | implemented | Closed the watcher receipt authority gap: `ProviderSchemaRefreshReceipt.grants_authority` is now a runtime-enforced `Literal[False]`, and serialization always emits false. | `current change`; `provider_schema_watcher.py`; `test_provider_schema_watcher.py` (`11 passed`); Ruff, formatter, strict mypy, and pre-commit hooks passed. | Capture deployed watcher and Saga evidence; no live or remote validation was performed. |
