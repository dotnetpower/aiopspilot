...
|---------|--------------------|
| Backend services | Each `services/*` root owns its `pyproject.toml`, source package, tests, and image. The validated five-service baseline also owns process identities and migration branches. The System Knowledge Service additionally owns an isolated Terraform root, Managed Identity Blob CAS ledger, Azure Bot, and protected workflow, while live graduation evidence remains open. |
| Core service entry points | The Core manifest exposes service-owned bounded maintenance entry points, including the `once`/`loop` inventory synchronization wrapper, active-generation projection release migration, sanitized Incident and forecast evidence aggregate queries, shadow operational-history lifecycle Job, and protected certification runner used by Container Apps. Each entry point composes delivery adapters inside Core's package boundary and does not grant executor authority. |
...
| Core Azure runtime dependency | The Core manifest owns `azure-core` because deployed Azure model adapters import its typed authentication failures during runtime bootstrap. The image builder cold-imports that bootstrap after installing the service wheel, so an omitted direct dependency blocks publication instead of failing a Container Apps revision. |
