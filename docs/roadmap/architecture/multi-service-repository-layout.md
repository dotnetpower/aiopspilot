...
| Core service entry points | The Core manifest exposes service-owned bounded maintenance entry points, including the `once`/`loop` inventory synchronization wrapper, active-generation projection release migration, sanitized Incident evidence aggregate query, shadow operational-history lifecycle Job, and protected certification runner used by Container Apps. Each entry point composes delivery adapters inside Core's package boundary and does not grant executor authority. |
