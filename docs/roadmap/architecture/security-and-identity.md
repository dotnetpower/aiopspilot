...
| Executor safeguards and independent effect closure | in-progress | `packages/service-contracts/src/fdai_service_contracts/execution_safeguards.py`; `core/executor/{safeguards,safeguard_proofs,lock,lock_continuity,idempotency_reservation*,audit_intent,target_dispatch_fence*,safeguard_dispatch*,post_release_closure*}.py`; `delivery/persistence/postgres_{resource_lock,idempotency_reservation,audit_intent,target_dispatch_fence,safeguard_dispatch,post_release_closure}.py`; focused safeguard, lock, fence, persistence, and migration tests; `config/constitution-traceability.json` requirement `FDAI-CONST-007`; issues `#81`, `#627`, `#628`, `#633`, and `#681`; completed issues `#620`, `#660`, `#664`, `#669`-`#672`, `#674`, `#678`-`#680`, `#692`-`#694` | The proof bundle, production lock evidence, finalizer, ownership-continuity model, reservation and audit stores, target-wide fence, persisted dispatch checkpoints, and atomic post-release closure are implemented without authority or effect-verification claims. Shared orchestration, real producers, isolated-Executor validation, and governed effect evidence remain open. |
...
| Standing human authorization (A3-E) | in-progress | `config/constitution-traceability.json` requirement `FDAI-CONST-008`; [Escalation and Standing Authority](../decisioning/escalation-and-standing-authority.md); `core/standing_authority/{lease,promotion_candidate*}.py`; completed issues `#331`, `#621`, and `#629`; issues `#631` and `#632` | The schema, evaluator, immutable lifecycle, read-time fence, effect-spanning lease, and inert promotion-candidate lifecycle exist but remain unwired. Bounded local shadow evidence and separately authorized governed promotion remain open. |
...
- [ ] Implement the shared evidence lifecycle orchestrator under issue `#681` after #692-#694.
...
- [ ] Retain bounded local shadow evidence and complete separately authorized governed promotion under issues `#631` and `#632`; completed issues `#331`, `#621`, and `#629` remain the lifecycle, lease, and inert review evidence.
