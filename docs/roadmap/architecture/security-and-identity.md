...
| Executor safeguards and independent effect closure | in-progress | `packages/service-contracts/src/fdai_service_contracts/execution_safeguards.py`; `schemas/execution-safeguard-proof-bundle/1.0.0.json`; `test_execution_safeguards.py`; `core/executor/{safeguards,safeguard_proofs}.py`; `shared/providers/resource_lock.py`; focused safeguard and resource-lock tests; `config/constitution-traceability.json` requirement `FDAI-CONST-007`; issues `#81`, `#627`, `#628`, `#633`, `#670`-`#672`, and `#674`; completed issues `#620`, `#660`, `#664`, and `#669` | The provider-neutral seven-proof wire bundle, provider-attested historical/current lock evidence, pure Core finalizer, and evidenced-lock lifecycle/migration contract are implemented and grant no authority or effect-verification claim. Ownership-through-commit, crash-safe reservation, authoritative audit-intent readback, local/PostgreSQL providers, real Core and workflow producers, isolated-Executor validation, and governed cross-path effect evidence remain open. |
...
- [ ] Implement the local test-only evidenced provider with exact per-acquisition identity and production rejection under issue `#674`.
