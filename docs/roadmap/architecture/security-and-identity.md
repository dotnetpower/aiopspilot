...
| Automation-hold recovery approval hardening | in-progress | [Process Automation implementation status](../../roadmap-implementation/decisioning/process-automation.md#implementation-status); issues `#622` and `#630` | FDAI-CONST-009 remains implemented: incomplete compensation durably blocks forward dispatch and matching verified recovery releases the hold. The open work strengthens exact human approval admission, atomic evidence consumption, and release-versus-new-hold fencing without reopening that implemented requirement. |
...
- [ ] Bind separately approved recovery through existing approval and decision-evidence contracts, then atomically consume that admission while releasing the exact hold revision under issues `#622` and `#630`; FDAI-CONST-009 remains implemented during this hardening.
