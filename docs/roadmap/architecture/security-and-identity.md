...
| Automation-hold recovery approval hardening | in-progress | `core/workflow/recovery_admission.py`; `test_recovery_admission.py`; [Process Automation implementation status](../../roadmap-implementation/decisioning/process-automation.md#implementation-status); issues `#622`, `#630`, and `#640` | Exact human recovery approval admission is implemented without hold mutation or authority. Atomic evidence consumption and release receipt remain under `#630`; final dispatch fencing remains under `#640`. FDAI-CONST-009 remains implemented. |
...
- [ ] Atomically consume the admitted recovery, release the exact hold revision, and emit the action-bound no-authority release receipt under issue `#630`.
