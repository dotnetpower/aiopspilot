...
| Executor safeguards and independent effect closure | in-progress | `packages/service-contracts/src/fdai_service_contracts/execution_safeguards.py`; `core/executor/{safeguards,safeguard_proofs,lock,lock_continuity,idempotency_reservation,audit_intent}.py`; `delivery/persistence/postgres_{resource_lock,idempotency_reservation,audit_intent}.py`; `shared/providers/resource_lock.py`; focused safeguard, lock, persistence, and migration tests; `config/constitution-traceability.json` requirement `FDAI-CONST-007`; issues `#81`, `#627`, `#628`, `#633`, and `#681`; completed issues `#620`, `#660`, `#664`, `#669`-`#672`, `#674`, and `#678`-`#680` | The proof bundle, lock evidence and production PostgreSQL provider, finalizer, ownership-continuity model, crash-safe reservation and audit-intent PostgreSQL stores are implemented without authority or effect-verification claims. Shared orchestration, real producers, isolated-Executor validation, and governed effect evidence remain open. |
