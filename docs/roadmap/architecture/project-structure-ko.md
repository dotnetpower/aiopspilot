...
translation_source_sha: aae19f578f78056d3ae622882de58e86391e6665
...
| **온톨로지 인스턴스 읽기** | `shared/providers/ontology_instance.py`의 `OntologyInstanceStore.query_objects(...)` 및 `scan_objects(...)` | - | 메모리 및 PostgreSQL 어댑터는 범위가 제한된 객체 ID, 속성, 선택적 관계 및 단일 연결 후보 스냅샷 읽기를 구현합니다. | 객체 1,000개 관계 조회 제한, 객체 50,000개 후보 제한, 단일 스냅샷 출처 세대, 잘림 및 권한 없는 그래프 증적을 보존하면서 저장소를 교체합니다. |
