...
translation_source_sha: 58cdd6bb50817d9d4bf3ba98507fae97c0b4294f
...
- **Operational catalog 검토 및 측정**: `DeterministicCatalogValidator`는 고정 시나리오 디렉터리에서
  ...
  보호된 workflow가 중복 JSON 키를 차단하고 엄격한 군, 리비전, 실행, 경로 및 시간 맥락을 주입하며 타입이 지정된 지표 숫자만 받아 멱등하게 보존합니다. 군별 exporter allowlist는 검토된 exporter와 정책 항목이 함께 추가될 때까지 경로를 차단합니다.
