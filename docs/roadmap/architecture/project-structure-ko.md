...
translation_source_sha: 6a63c872c951c7c5e8029e42457bef291f3c14d3
...
- **Operational catalog 검토 및 측정**: `DeterministicCatalogValidator`는 고정 시나리오 디렉터리에서
  ...
  보호된 workflow가 엄격한 군, 리비전, 실행, 경로 및 시간 맥락을 주입하고 타입이 지정된 지표 숫자만 받아 멱등하게 보존하며, 군별 exporter allowlist는 검토된 exporter와 정책 항목이 함께 추가될 때까지 경로를 차단합니다.
