...
FDAI는 코드형 인프라가 리소스 생성을 소유하는 환경을 대상으로 합니다. FDAI에 새 모델이나
클라우드 리소스를 배포하라는 대화 요청은 지원되지 않으며 ActionType, 작업 초안 또는 공급자
변경을 만들 수 없습니다. 권한이 있는 운영자는 대신 이미 존재하는 LLM 또는 GPT 배포를
조회할 수 있습니다.

읽기 전용 인벤토리는 `Microsoft.CognitiveServices/accounts/deployments`를
`llm-model-deployment`로 등록하고 각 하위 리소스를 상위 `llm-endpoint`에 매핑합니다. 따라서
일반적인 예약 Azure Resource Graph 수집, 온톨로지 변환 결과 및 의미 쿼리 경로는 호출자의 기존
범위에서 볼 수 있는 배포만 반환할 수 있습니다. 최신성, 완전성 및 출처 제한은 명시적으로
유지되며 이 관찰은 배포, 추론, 승인 또는 실행 권한을 부여하지 않습니다.

각 배포 인스턴스는 배포 이름, 모델 이름, 모델 버전, 모델 형식, SKU 이름, 요청한 공급자 용량
단위, 현재 공급자 용량 단위, 전환 상태, 프로비저닝 상태를 읽기 가능한 사실로 유지하고 근거
검토를 위한 범위가 제한된 원시 공급자 속성도 보존합니다. `sku.capacity`는 요청 값을 유지하고,
`properties.currentCapacity` 또는 레거시 활성 용량 필드가 현재 값을 제공합니다. 공급자가
모호하지 않은 토큰 속도 제한 규칙 하나를 반환하면 FDAI는 횟수와 갱신 주기를
`capacity_tpm`으로 정규화하고 `properties.rateLimits`를 출처로 기록합니다. 토큰 규칙이
없거나, 잘못되었거나, 소수이거나, 서로 충돌하면 TPM은 알 수 없음으로 유지합니다. FDAI는
`sku.capacity`를 TPM으로 변환하지 않습니다.

"배포된 LLM 목록", "GPT 모델 목록", `show deployed LLMs`, `list GPT models` 같은 요청은
이 리소스 타입으로 해석됩니다. 인벤토리 리소스 변경 피드는 기본적으로 사용 설정되며 구성된
최소 간격 안에서 변경된 리소스를 병합합니다. 완전한 조정은 별도로 구성된 간격에 실행되어 누락된
이벤트를 복구하고 삭제를 확인합니다. 배포에서 `FDAI_INVENTORY_RESOURCE_TYPES`를 좁히는 경우
이 보기를 유지하려면 `llm-model-deployment`를 포함해야 합니다.
...
쿼럼 적용은 `t1.judge`와 `t2.reasoner.primary`의 실제 상태만 확인하고 관련 없는 모델 드리프트와
Core 애플리케이션 상태 검사를 건너뜁니다.
...
코어 코드는 기능 계약에만 의존합니다. 시작 시 하나의 비동기 코어 소유자가 인라인 또는 탑재된
`resolved-models.json` 리비전을 한 번만 불러옵니다. 산출물 바이트를
`LLM_RESOLVED_MODELS_SHA256`과 비교하고, 같은 불변 리비전을 수명 주기 평가와 모든 운영 기능
...
하나만 가져와 경로와 페이로드 다이제스트를 다시 계산하며 `activation_authority: false`를
요구합니다. 소유자는 불러온 모델 바이트를 제안 스키마 v3의 원본 다이제스트로 정규화하고, 각
결정 다이제스트를 다시 계산해 검증한 뒤 연결 전에 기존 영속 상태 저장소에 기록합니다.
스테이징과 운영 시작에는 인증된 관찰 출처가 필요합니다. 출처가 없으면 빈 검토 집합으로
취급하지 않고 시작을 중단합니다.

현재 원본에 대한 만료되고 병합되지 않은 제안은 연결 전 보류 집합에 포함됩니다. 기본 LLM,
의미 판단, 온톨로지 협의체, 대화 후 검토, 대화 보증 연결기는 시작 소유 리비전에서 보류된
기능을 제외합니다. 이 과정은 해석된 매핑을 다시 쓰거나 매핑 권한 또는 실행 권한을 부여하지
않습니다. 오래된 원본의 제안은 보류를 만들지 않습니다.
...
주간 작업은 더 선호되는 새 계열, 60일 안의 사용 중단, 측정된 용량 또는 품질 표류를
감시합니다. 멱등 초안 PR만 열고 실제 매핑은 바꾸지 않습니다. 제안 스키마 v4는 모델 계열,
발행자, 상태뿐 아니라 정확한 모델 버전, SKU, 유효 용량 단위 및 값도 비교합니다. 공급자 사용
중단 근거는 60일 구간을 적용하기 전에 일치하는 모델 버전과 SKU로 정규화합니다. Core는 이미
열려 있는 신뢰된 스키마 v3 제안을 계속 평가하며, 새 버전 전용 표류나 SKU 범위 표류를 변경
없음으로 잘못 분류하지 않습니다.

초안 상태를 다시 읽은 뒤 워크플로는
