...
translation_source_sha: a44fb1a27e8ce7f685146d2391732d0fde95b9a8
...
| 대상 전체 준비 디스패치 fence | 영구 예약 직후이면서 감사, 묶음, 대상 시스템 작업 전에 대상별 고유 generation fence 레코드를 시작합니다. 정확한 레코드 PostgreSQL CAS와 읽기는 예약 시도, 획득, 연속성 정책, 클라이언트 상관관계, 대상 시스템 멱등성 신원, 감사 증적, 안전조건 묶음을 결속합니다. 해결되지 않은 모든 상태는 새 대상 변경을 차단하며 격리 해제에는 새로운 조정 근거가 필요합니다. 모델, JSON 코덱, 영구 저장소 프로토콜은 별도 책임으로 유지합니다. | [fence 모델](../../../services/core-control-plane/src/fdai/core/executor/target_dispatch_fence.py), [영구 저장소 경계](../../../services/core-control-plane/src/fdai/core/executor/target_dispatch_fence_store.py), [엄격한 코덱](../../../services/core-control-plane/src/fdai/core/executor/target_dispatch_fence_codec.py), [PostgreSQL 저장소](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_target_dispatch_fence.py) | [모델 및 코덱 테스트](../../../services/core-control-plane/tests/core/executor/test_target_dispatch_fence.py), [PostgreSQL 저장소 테스트](../../../services/core-control-plane/tests/persistence/test_postgres_target_dispatch_fence.py), [서비스 이행 테스트](../../../tests/integration/services/test_service_migration_inventory.py) |
