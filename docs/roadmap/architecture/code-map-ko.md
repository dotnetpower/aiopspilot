...
translation_source_sha: 9736da42aec3fb6eafbada49b6178333fb8aa3d1
...
| Assurance Twin 자세/검토 활동, 운영 원장, Operator API, 콘솔 패널 | 계산된 `PostureAssessmentReport` 또는 선제적 `IacReview`에 대해 Heimdall이 소유하는 범위가 제한된 `agent.operational-activity` 신호(`assurance-twin.posture` 종류, 스키마 `1.2.0`, 무권한). Heimdall은 이미 구독 중인 `object.event`로 선제적 Twin 후보가 도착하면 이를 기록합니다. 운영 본문은 기존 `state_kv` 원장에 저장(범위별 최신 우선, `review_key`별 멱등, 불일치 재전달은 명시적 충돌)되며 활동, 상관관계, 증거 다이제스트 출처를 함께 담습니다. Operator API는 `/assurance-twin/posture`, `/assurance-twin/reviews`, `/assurance-twin/reviews/{review_id}`를 통해 그대로 렌더링하고 오래되었거나 사용 불가, 알 수 없음, 형식 오류, 다이제스트 불일치 행은 명시적 공백으로 보류합니다. 콘솔 `assurance-twin` 경로는 자세 KPI, 검토 목록, 불투명 키 드릴다운, 보류된 증거 공백을 렌더링합니다 | [자세 활동](../../../services/core-control-plane/src/fdai/core/assurance_twin/posture_activity.py), [런타임 트리거](../../../services/core-control-plane/src/fdai/runtime/assurance_twin_posture.py), [전달 레코더](../../../services/core-control-plane/src/fdai/delivery/assurance_twin_posture.py), [운영 원장](../../../services/core-control-plane/src/fdai/delivery/persistence/state_store_assurance_twin_posture.py), [Operator 변환](../../../services/operator-service/src/fdai_operator_service/assurance_twin_posture_projection.py), [콘솔 경로](../../../console/src/routes/assurance-twin.tsx) | [자세 활동 테스트](../../../services/core-control-plane/tests/assurance_twin/test_posture_activity.py), [런타임 트리거 테스트](../../../services/core-control-plane/tests/runtime/test_assurance_twin_posture.py), [전달 테스트](../../../services/core-control-plane/tests/delivery/test_assurance_twin_posture.py), [원장 테스트](../../../services/core-control-plane/tests/persistence/test_state_store_assurance_twin_posture.py), [Operator 변환 테스트](../../../services/operator-service/tests/test_assurance_twin_posture_projection.py), [콘솔 테스트](../../../console/src/routes/assurance-twin.test.ts) |
