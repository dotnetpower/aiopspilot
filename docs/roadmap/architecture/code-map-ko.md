...
translation_source_sha: 61be0187fd3e2086dd9e28d5651fd21a4d22cc4d
...
| 장애에 안전한 멱등성 예약 | 안정적인 키, 액션, 실행 경로, 지문, 소스 개정, 잠금 획득 신원을 정확히 결속하고 예약됨, 실행 중, 최종, 중단됨, 결과 불명 상태를 단조롭게 전환합니다. 변경 불가능한 레코드와 CAS 및 읽기 증적은 정확한 이전 상태를 요구하고 소유자와 시간 순서를 보존하며 모호한 상태의 재디스패치를 허용하지 않습니다. 권위 있는 미디스패치 근거가 있을 때만 더 높은 시도를 허용합니다. 공급자 경계는 정의했으며 영구 PostgreSQL 구현은 생성기 통합 의존성으로 남아 있습니다. | [멱등성 예약](../../../services/core-control-plane/src/fdai/core/executor/idempotency_reservation.py) | [예약 계약 테스트](../../../services/core-control-plane/tests/core/executor/test_idempotency_reservation.py) |
