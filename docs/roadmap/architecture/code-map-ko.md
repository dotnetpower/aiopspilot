...
translation_source_sha: d6f08fa31a9eabaf9bcf1e4b97febdef0ccc46e9
...
| Core 계약과 프로바이더 경계 | Core 전용 타입, 프로바이더 프로토콜, 구성, 스트리밍, 텔레메트리, 범위가 제한된 알림 표시 및 기능 계약, 엄격한 operating-intent 다이제스트, 공유 승인 결속 change-window 구성, 무권한 리소스 잠금 근거를 포함합니다. 잠금 경계는 공급자가 증명한 과거 획득 증적과 현재 fence 또는 세션 소유권 평가를 분리하고, 기존 운영 대체 경로가 없는 명시적 근거 잠금 요청 및 보유 핸들 수명 주기를 제공합니다. 프로세스 내 관리자는 로컬 테스트 전용으로 정확한 획득별 소유권과 취소에 안전한 정리를 구현하며 `production_eligible=false`를 반환합니다. | [shared](../../../services/core-control-plane/src/fdai/shared/), [resource_lock.py](../../../services/core-control-plane/src/fdai/shared/providers/resource_lock.py), [로컬 잠금 관리자](../../../services/core-control-plane/src/fdai/core/executor/lock.py) | [리소스 잠금 계약 테스트](../../../services/core-control-plane/tests/providers/test_resource_lock_receipt.py) 및 [로컬 잠금 테스트](../../../services/core-control-plane/tests/core/executor/test_lock.py) |
