...
translation_source_sha: a2eaeb2c81b5a934ad5cdde460aef66fe6541403
...
| 감지 거버넌스 정책 | 시작 시 로드하고 신원, 의미 체계 버전, 항목 수 상한을 고정한 저장소 정책으로 관리하는 이상 감지 방식과 완화할 수 없는 콜드 스타트 하한, 타입이 지정된 계절 위상, 관리되는 예측 대상 종류, 모델 계열, 대상 기간, 샘플 및 적합도 하한, 런타임이 지원하는 고정 신뢰수준, 축소할 수 없는 정확한 상관관계 키 집합, 완화할 수 없는 T1 유사도 하한, 공유 근거 필드 2개 하한, 최소 14일의 관찰 모드 이후 주간 백테스트, 완화할 수 없는 예측 정밀도, 재현율, 구간 포괄률, 선행 시간, 판단 보류율 경계, 정확한 범위와 완전한 근거를 모두 요구하는 변경 창 보류. 현재 실행 가능한 연결은 예측 대상 섹션을 강제합니다. 다른 섹션은 별도 런타임 재정의 연결이 아니라 고정 evaluator를 위한 검토된 계약으로 유지됩니다. 머신 필드 버전 검증은 대화 의미 라우팅과 분리됩니다. | [거버넌스 정책](../../../services/core-control-plane/src/fdai/core/detection/governance_policy.py), [예측 런타임 강제](../../../services/core-control-plane/src/fdai/runtime/forecast_learning.py) 및 [신뢰된 구성](../../../config/detection-governance-policy.json) | [거버넌스 정책 테스트](../../../services/core-control-plane/tests/core/detection/test_governance_policy.py) 및 [런타임 정책 테스트](../../../services/core-control-plane/tests/runtime/test_forecast_learning.py) |
| 예측 운영 근거 | 닫힌 예측 에피소드에서 정밀도, 재현율, 놓친 위반, 구간 포괄률, 양수인 액션 가능한 선행 시간, 양수가 아닌 선행 시간 수, 판단 보류, 개입으로 검열된 결과, 점수화할 수 없는 결과의 분모를 명시적으로 계산하고 승격 권한은 부여하지 않는 결정론적 배포 메트릭 | [운영 메트릭 reducer](../../../services/core-control-plane/src/fdai/core/detection/forecast_operational_metrics.py) 및 [PostgreSQL 근거 snapshot](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_forecast_episode.py) | [운영 메트릭 테스트](../../../services/core-control-plane/tests/core/detection/test_forecast_operational_metrics.py) |
