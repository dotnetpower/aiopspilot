...
translation_source_sha: e9834813769346927f0f9ae239fa284856c050c1
...
| 통제된 기준선 및 처리 코호트 주장 | 사전에 커밋한 하나의 향후 운영 프로토콜 아래에서 비합성 기준군 및 처리군 근거의 적격성을 결정론적으로 평가하며 실패 시 차단합니다. 합성 고정 시나리오는 별도의 release 회귀 근거로 남습니다. 신뢰 정책은 모든 필수 성공 지표, 임계값 0 가드 전체, 프로토콜 다이제스트, 최대 90일 구간, 독립성 키 중복 제거, 구간 계산법과 표본 30개 하한을 고정합니다. 축약기는 각 지표와 가드의 자연 분모를 보존하고 가장 작은 유효 표본 수를 공개하며, 충돌하는 독립성 키를 차단하고 Wilson 또는 결정론적 부트스트랩 95% 구간을 사용합니다. 평가 대상 군 사실은 모두 정규 해시로 계산하며, 적격 판정에는 신뢰 채널에서 받은 서로 다른 군별 승인과 코호트 수준 승인 하나가 계속 필요합니다. | [주장 정책](../../../services/core-control-plane/src/fdai/core/measurement/cohort_claim_policy.py)과 [신뢰 구성](../../../config/sre-cohort-claim-policy.json), [코호트 계약](../../../packages/service-contracts/src/fdai_service_contracts/baseline_cohort.py), [운영 축약기](../../../services/core-control-plane/src/fdai/core/measurement/operational_cohort.py), [승인 연결](../../../services/core-control-plane/src/fdai/core/measurement/baseline_cohort_claim.py), [증적 가져오기](../../../tools/cohort_receipt.py), [기준선 실행기](../../../tools/baseline_run.py) | [정책 테스트](../../../services/core-control-plane/tests/core/measurement/test_cohort_claim_policy.py), [코호트 계약 테스트](../../../packages/service-contracts/tests/test_baseline_cohort.py), [운영 축약기 테스트](../../../services/core-control-plane/tests/core/measurement/test_operational_cohort.py), [주장 테스트](../../../services/core-control-plane/tests/core/measurement/test_baseline_cohort_claim.py), [기준선 실행기 테스트](../../../services/core-control-plane/tests/tools/test_baseline_runner.py) |
