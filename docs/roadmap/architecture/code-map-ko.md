...
- **플랫폼-서비스 바인딩:** 루트 Terraform은 검토된 대상과 Key Vault 참조를 내보냅니다. 보호된 배포와 봇 소유 래퍼는 정확한 서비스 계획만 수락하고, AKS 인벤토리는 managed cluster의 정확한 ARM 리소스 ID 범위에만 `Azure Kubernetes Service RBAC Reader`를 부여합니다.
  ...
  런타임 호출 인계는 같은 정확한 Operator 및 Core Container App Resource ID를 두 엔드포인트 서비스에 전달하고, 인벤토리 Job은 단일 기록기 변환 전에 두 플랫폼 로그를 독립적으로 결합합니다. 비공개 운영 이력 근거의 경우 플랫폼 root가 서비스 distribution 또는 선택적 package 경계를 변경하지 않고 배포 실행기의 기존 ops Blob DNS 영역에 계정별 A record를 소유합니다.
...
내보내며 800줄 상한 아래를 유지합니다. `semantic_query_current_evidence.py`는 정확한 레지스트리, 보호된 graph-first Resource 범위, 호출 권한 및 principal 다이제스트를 준비 상태 프로브에 연결합니다.
...
프롬프트 조립은 역할 및 안전 레이어를 `core/prompts/`에 유지하고 Azure 시작 조립은 `composition/wire_azure_prompts.py`에 둡니다. 정확한 프로필은 아티팩트 본문, 복구 지시, 생성된 스키마, 적응형 서버 프로필 및 정리된 cross-check 시스템 텍스트를 실제 전송 시스템 재실행 근거, 품질 게이트 감사, 예산 초과로 거부된 시도와 결정론적 또는 진단용 재구성을 포함한 영구 대화 보증 결정, 온톨로지 협의회 적합성 신원 및 다시 생성한 의미 커버리지 소스 다이제스트에 결속하며 작성기는 프로필 도입 전의 주입형 레지스트리도 지원합니다. 보증 평가자가 실패하면 반환 전에 동료 작업을 취소하고 완료를 기다립니다. 프로필 인식 카탈로그는 익명 또는 불완전한 근거, 잘못된 스키마와 값, 중복 신원, legacy shadow 토글 및 프로필 없는 대체를 차단합니다. 모델 tokenizer가 없으면 정리된 최종 시스템 내용과 모든 최종 의미, 제안자, RCA, 턴 후, cross-check, 보증 및 온톨로지 협의회 요청은 바이트당 토큰 하나의 상한과 예약 출력을 사용합니다. [적응형 조립](../../../services/core-control-plane/src/fdai/composition/wire_adaptive_conversation.py)은 역할별 단계를 연결하고, `adaptive_model_targets.py`는 적응형 답변과 선택형 `wire_t1_routing.py` 지연 시간 프로브가 함께 쓰는 작성, 검토 및 선택적 보강 대상을 배선 모듈 간 가져오기 없이 해석합니다. [답변](../../../services/core-control-plane/src/fdai/core/conversation/adaptive_service.py)은 [내부 프로바이더 사용량 제한](../../../services/core-control-plane/src/fdai/core/conversation/adaptive_call_scope.py)을 공유하며, [Operator 담당 관계 확인](../../../services/operator-service/src/fdai_operator_service/adaptive_relationship.py)은 권한 없는 만료형 맥락을 제공합니다.
...
| 작업 흐름 복구 승인 및 보류 해제 | 정확한 복구 승인과 승인 사용, fence 세대, 무권한 증적을 포함한 승인 보호 원자적 해제 기본 연산입니다. 두 positive 경계는 decision-evidence inventory에 등록됩니다. 운영 보상 통합은 #630, 최종 실행 경로 fence는 #640에 남아 있습니다. | [복구 승인](../../../services/core-control-plane/src/fdai/core/workflow/recovery_admission.py), [자동화 보류](../../../services/core-control-plane/src/fdai/core/workflow/automation_hold.py), [작업 흐름 승인 상태](../../../services/core-control-plane/src/fdai/delivery/persistence/workflow_approval.py), [PostgreSQL 상태](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres.py), [감사 필드 도우미](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_audit_fields.py) | [복구 및 보류 테스트](../../../services/core-control-plane/tests/core/workflow/test_automation_hold.py) |
...
| 배포된 모델 인벤토리 검색 | 기존 Azure AI 모델 배포를 상위 계정 아래에서 읽기 전용으로 수집하고, 공급자의 속도 제한에 근거한 TPM 정규화, 분리된 요청·현재 용량과 전환 상태, 읽기 가능한 모델, SKU, 상태 정보와 이중 언어 의미 검색, 완전한 리소스 분류를 제공하며, 온톨로지에 변환할 때 일반 범위, 최신성, 완전성 및 실행 권한 없음 제약을 유지합니다. ARG가 이러한 하위 리소스를 안정적으로 열거하지 않으므로 범위가 제한된 ARM `accounts/{name}/deployments` 목록을 이 ResourceType의 권위 있는 출처로 사용하며 모델 배포를 생성하지 않습니다. 대화를 통한 배포 요청은 ActionType을 만들지 않습니다. | [리소스 타입](../../../rule-catalog/vocabulary/resource-types.yaml), [리소스 클래스](../../../rule-catalog/vocabulary/resource-classes.yaml), [모델 배포 변환](../../../services/core-control-plane/src/fdai/delivery/azure/model_deployment.py), [Azure ARG 변환](../../../services/core-control-plane/src/fdai/delivery/azure/arg_query.py), [Azure ARM 하위 리소스 출처](../../../services/core-control-plane/src/fdai/delivery/azure/arm_inventory.py), [공급자 관계 매핑](../../../rule-catalog/vocabulary/provider-relationship-mappings/azure-arg-v1.yaml), [아키텍처 맵](../../../console/src/components/architecture-map.model.ts), [Operator 행](../../../services/operator-service/src/fdai_operator_service/families/conversation/presentation_rows.py) | [모델 배포 테스트](../../../services/core-control-plane/tests/delivery/azure/test_model_deployment.py), [의미 계획 테스트](../../../services/core-control-plane/tests/conversation/test_semantic_planning.py), [Azure 변환 테스트](../../../services/core-control-plane/tests/delivery/azure/test_arg_query.py), [ARM 하위 리소스 출처 테스트](../../../services/core-control-plane/tests/delivery/azure/test_arm_inventory.py), [리소스 클래스 테스트](../../../services/core-control-plane/tests/rule_catalog/test_resource_class.py), [아키텍처 맵 테스트](../../../console/src/components/architecture-map.model.test.ts), [서비스 제품군 테스트](../../../tests/integration/scripts/test_service_test_suites.py), [Operator 행 테스트](../../../services/operator-service/tests/test_presentation_rows.py) |
...
| 컨트롤 루프, decisioning 및 기능 라이선스 Trial 상한 | Event 정규화, 계층 라우팅, 정확한 Rego allow/deny 평가 증적, 품질, 위험, 승인, 실행 조정, 복구 및 감사, 패키지된 Ed25519 검증, 고정 배포판 연결, 소비자가 강제하는 UTC 30일 상한, 암호학적으로 검증된 로컬 발급자 모드, 거부 감사 저장에 실패해도 권한을 부여하지 않고 공유 Thor 포트 전체에 적용하는 현재 시각 기준 종료 거부 | [코어](../../../services/core-control-plane/src/fdai/core/), [신뢰 어댑터](../../../services/core-control-plane/src/fdai/delivery/trust/), [런타임 연결](../../../services/core-control-plane/src/fdai/runtime/licensing.py), [발급기](../../../scripts/deployment/release/issue-license.py) | [코어 테스트](../../../services/core-control-plane/tests/core/), [신뢰 테스트](../../../services/core-control-plane/tests/delivery/trust/test_ed25519.py), [런타임 테스트](../../../services/core-control-plane/tests/runtime/test_licensing.py), [발급기 테스트](../../../tests/integration/scripts/test_issue_license.py) |
...
| Core 안전조건 증명 최종 처리 | 디스패치 전 평가에서 고정한 전체 액션 다이제스트와 액션, 경로, 지문, 소스 개정, 영구 멱등성 예약, 저장된 감사 의도에 결속된 타입 지정 증명 설명을 사용합니다. 논리 대상 잠금 근거는 자체 발행 연산 설명을 정확한 과거 공급자 증적에 결속하고, 최종 처리 시점의 현재 공급자 증명 평가를 요구하며, 구성된 검증기와 신뢰 앵커를 확인하고, 두 다이제스트를 무권한 #620 묶음에 기록합니다. 순수 최종 처리기는 I/O를 수행하지 않습니다. | [디스패치 전 안전조건](../../../services/core-control-plane/src/fdai/core/executor/safeguards.py), [증명 최종 처리기](../../../services/core-control-plane/src/fdai/core/executor/safeguard_proofs.py), [리소스 잠금 근거](../../../services/core-control-plane/src/fdai/shared/providers/resource_lock.py) | [최종 처리기 테스트](../../../services/core-control-plane/tests/core/executor/test_safeguard_proofs.py) |
| 대상 잠금 소유권 연속성 | fence 및 멱등 실행, 잠금 세션 원자적 커밋, 취소와 영구 결과 불명 격리를 위한 검토된 대상 시스템 전략입니다. 최종 무권한 증적은 정확한 획득과 현재 평가를 결속하며 디스패치, 대상 시스템 커밋, 독립 효과 검증을 별도 축으로 유지합니다. 모호한 디스패치, 알 수 없는 커밋, 상실되거나 알 수 없는 해제는 격리 상태를 유지합니다. | [잠금 연속성](../../../services/core-control-plane/src/fdai/core/executor/lock_continuity.py) | [잠금 연속성 테스트](../../../services/core-control-plane/tests/core/executor/test_lock_continuity.py) |
| 대상 전체 디스패치 근거 fence | 영구 예약 뒤이면서 감사, 묶음, 대상 시스템 작업 전에 대상별 고유 generation fence를 시작합니다. 정확한 레코드 PostgreSQL CAS는 안전조건 묶음, 디스패치 시작, 전송 및 권위 있는 대상 시스템 관측, 새로운 해제 전 잠금 연속성 checkpoint를 영속화합니다. 해결되지 않은 모든 상태는 대상 변경을 차단하며 권한이나 효과 검증 주장을 부여하지 않습니다. 모델, 엄격한 codec, 전이 gate, 영구 저장소 프로토콜, PostgreSQL 어댑터는 별도 책임으로 유지합니다. | [fence 모델](../../../services/core-control-plane/src/fdai/core/executor/target_dispatch_fence.py), [안전조건 디스패치 수명 주기](../../../services/core-control-plane/src/fdai/core/executor/safeguard_dispatch_checkpoint.py), [저장소 경계](../../../services/core-control-plane/src/fdai/core/executor/safeguard_dispatch_store.py), [PostgreSQL 근거 저장소](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_safeguard_dispatch.py) | [fence 테스트](../../../services/core-control-plane/tests/core/executor/test_target_dispatch_fence.py), [checkpoint 테스트](../../../services/core-control-plane/tests/core/executor/test_safeguard_dispatch_checkpoint.py), [PostgreSQL 테스트](../../../services/core-control-plane/tests/persistence/test_postgres_safeguard_dispatch.py), [서비스 이행 테스트](../../../tests/integration/services/test_service_migration_inventory.py) |
| 장애에 안전한 멱등성 예약 | 안정적인 키, 액션, 실행 경로, 지문, 소스 개정, 잠금 획득 신원을 정확히 결속하고 예약됨, 실행 중, 최종, 중단됨, 결과 불명 상태를 단조롭게 전환합니다. 변경 불가능한 레코드와 CAS 및 읽기 증적은 정확한 이전 상태를 요구하고 소유자와 시간 순서를 보존하며 모호한 상태의 재디스패치를 허용하지 않습니다. 권위 있는 미디스패치 근거가 있을 때만 더 높은 시도를 허용합니다. 정확한 키 집합의 JSON 직렬화는 중첩 근거를 복원하고 모든 불변식을 다시 검사합니다. 이행 파일이 소유하는 PostgreSQL 테이블은 원자적 삽입, 전체 레코드 compare-and-set, 정확한 읽기, 재시작 복구, 획득자 또는 동일 중복 또는 충돌 결과를 제공합니다. | [예약 모델 및 facade](../../../services/core-control-plane/src/fdai/core/executor/idempotency_reservation.py), [영구 codec](../../../services/core-control-plane/src/fdai/core/executor/idempotency_reservation_codec.py), [수명 주기 전환](../../../services/core-control-plane/src/fdai/core/executor/idempotency_reservation_lifecycle.py), [PostgreSQL 예약 저장소](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_idempotency_reservation.py) | [예약 계약 테스트](../../../services/core-control-plane/tests/core/executor/test_idempotency_reservation.py), [PostgreSQL 어댑터 테스트](../../../services/core-control-plane/tests/persistence/test_postgres_idempotency_reservation.py), [서비스 이행 테스트](../../../tests/integration/services/test_service_migration_inventory.py) |
| 권위 있는 효과 전 감사 의도 | 콘텐츠 주소 기반 무권한 의도는 정확한 영구 예약 전환 증적을 포함하며 예약이 현재 상태일 때 생성해야 합니다. 이행 파일이 소유하는 추가 전용 PostgreSQL 테이블은 후보를 먼저 커밋한 뒤 별도의 정확한 읽기 트랜잭션에서 데이터베이스 시각을 확보합니다. Lease 안에서 영구 콘텐츠가 일치할 때만 증적을 생성하며 후보에 결속된 결과는 추가됨, 동일 중복, 충돌을 구분합니다. | [감사 의도](../../../services/core-control-plane/src/fdai/core/executor/audit_intent.py) 및 [PostgreSQL 감사 의도 저장소](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_audit_intent.py) | [감사 의도 테스트](../../../services/core-control-plane/tests/core/executor/test_audit_intent.py), [PostgreSQL 어댑터 테스트](../../../services/core-control-plane/tests/persistence/test_postgres_audit_intent.py), [서비스 이행 테스트](../../../tests/integration/services/test_service_migration_inventory.py) |
...
| 감지 거버넌스와 운영 근거 | 시작 시 로드하고 신원과 의미 체계 버전을 고정한 정책이 감지기 및 예측 설정을 제한하며, 관리되는 예측 대상 종류는 실행 시 강제하고 고정 이상 감지, 상관관계, 승격, 변경 창 evaluator는 검토된 계약으로 유지합니다. 닫힌 예측 에피소드는 승격 권한 없이 정밀도, 재현율, 놓친 위반, 구간 포괄률, 양수 및 양수가 아닌 선행 시간, 판단 보류, 검열, 점수화 불가 분모를 노출합니다. 머신 필드 검증은 대화 의미 라우팅과 분리됩니다. | [거버넌스 정책](../../../services/core-control-plane/src/fdai/core/detection/governance_policy.py), [필드 검증](../../../services/core-control-plane/src/fdai/core/detection/governance_policy_fields.py), [예측 런타임 강제](../../../services/core-control-plane/src/fdai/runtime/forecast_learning.py), [운영 메트릭](../../../services/core-control-plane/src/fdai/core/detection/forecast_operational_metrics.py), [PostgreSQL 근거 snapshot](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_forecast_episode.py) 및 [신뢰된 구성](../../../config/detection-governance-policy.json) | [거버넌스 정책 테스트](../../../services/core-control-plane/tests/core/detection/test_governance_policy.py), [런타임 정책 테스트](../../../services/core-control-plane/tests/runtime/test_forecast_learning.py) 및 [운영 메트릭 테스트](../../../services/core-control-plane/tests/core/detection/test_forecast_operational_metrics.py) |
...
| Assurance Twin 자세/검토 레코더, 운영 원장, Operator API, 콘솔 패널(부분 구현: 신뢰된 생산자 없음) | 계산된 `PostureAssessmentReport` 또는 선제적 `IacReview`에 대해 범위가 제한된 `agent.operational-activity` 값(`assurance-twin.posture` 종류, 스키마 `1.2.0`, 무권한)을 만들지만 연결되어 있지 않습니다. 공개 스키마는 Heimdall 소유권을 고정하고 `assurance-twin` 생산자를 해당 종류에만 예약하며 null 관측 도메인을 요구합니다. 범주형 출처 레이블과 SHA-256 근거, 상관관계 및 검토 신원을 사용하므로 호출자가 제공한 범위, 리포지토리 참조, 검토 키 및 상관관계 값이 활동 레코드와 영속 출처 이력에 포함되지 않습니다. 트랜잭션 아웃박스가 정확한 영속 개정 번호와 순서를 맞출 때까지 두 값 모두 게시하지 않습니다. 기존 `state_kv` 원장은 범위를 더 새로운 `generated_at` 근거로만 진행하고, 같은 시각의 동일 입력은 재현으로 처리하며, 같은 시각의 다른 입력은 충돌로 표식합니다. 또한 `review_key`별 멱등성을 유지하고 불일치 검토 재전달을 충돌로 표식합니다. 모든 쓰기는 Operator API 변환 결과 자체의 경계(`review_key` <= 256자, 발견 사항 <= 200개, 범위가 제한되고 중복 없는 비어 있지 않은 <= 512자 `reason_codes`/`evidence_refs`, 비어 있지 않은 <= 512자 `evidence_source_revision`)를 넘어서면 거부됩니다. Operator API는 `/assurance-twin/posture`, `/assurance-twin/reviews`, `/assurance-twin/review?review_key=`를 통해 그대로 표시하고, 목록의 각 검토 영속 키를 정확한 본문 신원에 결속하며, `/system/data-sources`에서 세 경로를 서비스 로컬 운영 변환 결과에 할당합니다. 오래되었거나 사용 불가, 알 수 없음, 형식 오류, 키 또는 다이제스트 불일치, 충돌 표식이 있는 행은 명시적 공백으로 보류합니다. 콘솔은 모든 자세 범위, 검토 목록, 불투명 키 드릴다운, 서로 다른 빈 상태와 보류 상태를 표시합니다. 이 행을 쓰는 구성 요소는 없습니다. 선제적 수집 페이로드는 신뢰된 근거가 아니므로 레코더는 신뢰된 생산자가 생길 때까지 연결되지 않은 상태로 둡니다 | [자세 활동](../../../services/core-control-plane/src/fdai/core/assurance_twin/posture_activity.py), [전달 레코더](../../../services/core-control-plane/src/fdai/delivery/assurance_twin_posture.py), [운영 원장](../../../services/core-control-plane/src/fdai/delivery/persistence/state_store_assurance_twin_posture.py), [Operator 변환](../../../services/operator-service/src/fdai_operator_service/assurance_twin_posture_projection.py), [콘솔 경로](../../../console/src/routes/assurance-twin.tsx) | [자세 활동 테스트](../../../services/core-control-plane/tests/assurance_twin/test_posture_activity.py), [전달 테스트](../../../services/core-control-plane/tests/delivery/test_assurance_twin_posture.py), [원장 테스트](../../../services/core-control-plane/tests/persistence/test_state_store_assurance_twin_posture.py), [Operator 변환 테스트](../../../services/operator-service/tests/test_assurance_twin_posture_projection.py), [Operator 리더 테스트](../../../services/operator-service/tests/test_runtime_projection_reader.py), [콘솔 테스트](../../../console/src/routes/assurance-twin.test.ts) |
...
| 통제된 기준선 및 처리 코호트 주장 | 사전에 커밋한 하나의 향후 운영 프로토콜 아래에서 비합성 기준군 및 처리군 근거의 적격성을 결정론적으로 평가하며 실패 시 차단합니다. 합성 고정 시나리오는 별도의 release 회귀 근거로 남습니다. 신뢰 정책은 모든 필수 성공 지표, 임계값 0 가드 전체, 프로토콜 다이제스트, 최대 90일 구간, 독립성 키 중복 제거, 구간 계산법과 표본 30개 하한을 고정합니다. 지표 값과 구간 경계는 유한해야 하며 코호트 기준 시점은 가장 최신인 군 근거 기준 시점과 같아야 합니다. 주장 평가기는 95%가 아닌 모든 필수 구간을 차단합니다. 축약기는 각 지표와 가드의 자연 분모를 보존하고 가장 작은 유효 표본 수를 공개하며, 충돌하는 독립성 키를 차단하고, basis point를 올림 계산해 모든 임계값 0 위반을 보존하며, 자동 해결률에는 Wilson을 고정하고 연속값 및 횟수 평균에는 결정론적 부트스트랩 95% 구간을 사용합니다. 배포된 인벤토리는 private PostgreSQL을 읽기 전용 트랜잭션으로 읽고 정확한 프로토콜과 리비전, 유효한 SHA-256 신원, 타입이 지정된 지표 또는 가드 값이 있는 비합성 행만 계산하며 집계 수만 로그에 기록합니다. 평가 대상 군 사실은 모두 정규 해시로 계산하며, 적격 판정에는 신뢰 채널에서 받은 서로 다른 군별 승인과 코호트 수준 승인 하나가 계속 필요합니다. | [주장 정책](../../../services/core-control-plane/src/fdai/core/measurement/cohort_claim_policy.py)과 [신뢰 구성](../../../config/sre-cohort-claim-policy.json), [코호트 계약](../../../packages/service-contracts/src/fdai_service_contracts/baseline_cohort.py), [운영 축약기](../../../services/core-control-plane/src/fdai/core/measurement/operational_cohort.py), [집계 인벤토리](../../../services/core-control-plane/src/fdai/delivery/measurement/cohort_inventory.py), [승인 연결](../../../services/core-control-plane/src/fdai/core/measurement/baseline_cohort_claim.py), [증적 가져오기](../../../tools/cohort_receipt.py), [기준선 실행기](../../../tools/baseline_run.py) | [정책 테스트](../../../services/core-control-plane/tests/core/measurement/test_cohort_claim_policy.py), [코호트 계약 테스트](../../../packages/service-contracts/tests/test_baseline_cohort.py), [운영 축약기 테스트](../../../services/core-control-plane/tests/core/measurement/test_operational_cohort.py), [집계 인벤토리 테스트](../../../services/core-control-plane/tests/delivery/measurement/test_cohort_inventory.py), [주장 테스트](../../../services/core-control-plane/tests/core/measurement/test_baseline_cohort_claim.py), [기준선 실행기 테스트](../../../services/core-control-plane/tests/tools/test_baseline_runner.py) |
...
| 런타임 | 불변 시작 계획, 타입이 지정된 active-runtime 조립, 컨테이너 배치를 인식하는 제공 자산 해석, 지속형 operating-model 구독, Core 이미지에 함께 제공되는 일반 출처를 기본 바인딩하는 별도의 배포 소유 six-type operating-intent 출처 바인딩(정확히 고정된 개정 번호·자체 신고 provenance·문서 전체 digest, 타입별 정확한 인스턴스 개수, 유효 구간이 아니라 고정된 조회 시각 기준 신선도, 누락·중복·미달·stale·cross-release·미래 시각 인스턴스에서 fail closed, 고정된 다이제스트가 수용된 문서 전체를 덮고 정확한 6종 문서만 허용하도록 문서·provenance·객체·링크의 알 수 없는 멤버와 예상 밖 ObjectType을 거부, 출처가 스스로를 더 이상 증명하지 못하면 투영된 그래프가 아니라 권한만 격리하고 유효한 갱신 후 다시 승인하는 유계 재검증(투영 또는 온톨로지 카탈로그 실패 포함), 배포 전역 리소스 잠금으로 직렬화하고 롤아웃 세대로 울타리를 쳐서 물러나는 복제본이 더 새로운 롤아웃의 승인을 덮어쓰지 못하게 하는 시작 매니페스트·중단된 적용 복구·투영·승인 기록), ControlLoop가 ontology store를 노출한 뒤 연결되는 effect reconciliation, 집중된 메시징, Incident, 의미, 리소스 소유권 및 작업 hook 경계, 명시적 종료 순서, process-critical 상태 및 감사 쓰기와 authority-critical 전체 체인 증명을 분리하는 준비 상태, 작업 감독을 포함하는 Core 프로세스 수명 주기 | [런타임](../../../services/core-control-plane/src/fdai/runtime/), [bootstrap_plan.py](../../../services/core-control-plane/src/fdai/runtime/bootstrap_plan.py), [bootstrap_core.py](../../../services/core-control-plane/src/fdai/runtime/bootstrap_core.py), [bootstrap_resources.py](../../../services/core-control-plane/src/fdai/runtime/bootstrap_resources.py), [bootstrap_messaging.py](../../../services/core-control-plane/src/fdai/runtime/bootstrap_messaging.py), [bootstrap_incidents.py](../../../services/core-control-plane/src/fdai/runtime/bootstrap_incidents.py), [bootstrap_semantics.py](../../../services/core-control-plane/src/fdai/runtime/bootstrap_semantics.py), [bootstrap_task_hooks.py](../../../services/core-control-plane/src/fdai/runtime/bootstrap_task_hooks.py), [operating_intent_source.py](../../../services/core-control-plane/src/fdai/runtime/operating_intent_source.py), [operating_intent_binding.py](../../../services/core-control-plane/src/fdai/runtime/operating_intent_binding.py), [operating_intent_revalidation.py](../../../services/core-control-plane/src/fdai/runtime/operating_intent_revalidation.py), [core/operational_context/operating_intent_source.py](../../../services/core-control-plane/src/fdai/core/operational_context/operating_intent_source.py) 및 [core/operational_context/operating_intent_admission.py](../../../services/core-control-plane/src/fdai/core/operational_context/operating_intent_admission.py) | [런타임 테스트](../../../services/core-control-plane/tests/runtime/), [부팅 계획 테스트](../../../services/core-control-plane/tests/runtime/test_bootstrap_plan.py), [메시징 테스트](../../../services/core-control-plane/tests/runtime/test_bootstrap_messaging.py), [Incident 테스트](../../../services/core-control-plane/tests/runtime/test_bootstrap_incidents.py), [종료 테스트](../../../services/core-control-plane/tests/runtime/test_bootstrap_shutdown.py), [operating-intent 출처 테스트](../../../services/core-control-plane/tests/runtime/test_operating_intent_source.py), [operating-intent 재검증 테스트](../../../services/core-control-plane/tests/runtime/test_operating_intent_revalidation.py) 및 [operating-intent 승인 테스트](../../../services/core-control-plane/tests/core/operational_context/test_operating_intent_admission.py) |
| Core 계약과 공급자 경계 | Core 전용 타입, 공급자 프로토콜, 구성, 스트리밍, 텔레메트리, 무권한 리소스 잠금 근거를 포함합니다. 잠금 경계는 획득, 현재 소유권, 최종 해제 증적을 분리합니다. 로컬 관리자는 정확한 보유자 테스트 근거만 제공합니다. 운영 PostgreSQL 공급자는 다시 연결하지 않는 한 세션을 사용하고 advisory 획득과 backend 신원을 원자적으로 확보하며, 모든 평가에서 정확한 데이터베이스, PID, 시작 시각, `pg_locks` 소유권을 검사합니다. Unlock을 확인하고 취소를 보존하며 검토된 격리 조정 정책만 수락합니다. | [리소스 잠금 계약](../../../services/core-control-plane/src/fdai/shared/providers/resource_lock.py), [로컬 잠금 관리자](../../../services/core-control-plane/src/fdai/core/executor/lock.py), [PostgreSQL 잠금 공급자](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_resource_lock.py) | [리소스 잠금 계약 테스트](../../../services/core-control-plane/tests/providers/test_resource_lock_receipt.py), [로컬 잠금 테스트](../../../services/core-control-plane/tests/core/executor/test_lock.py), [PostgreSQL 잠금 테스트](../../../services/core-control-plane/tests/persistence/test_postgres_resource_lock.py) |
...
| [cli/](../../../cli/), [deployment-cli](../../../packages/deployment-cli/), [genesis-foundation](../../../infra/genesis-foundation/) | 운영자 클라이언트와 별도 배포 도구입니다. 비공개 기반 계층 계획, 상태 비교, 런타임 입력 검증 및 runner의 기존 Blob DNS 영역에 대한 프로바이더 수렴형 비공개 근거 A record를 제공합니다. 기반 계층이 애플리케이션 그룹을 소유하며 새 플랫폼 상태는 두 번째 소유자 없이 이를 참조합니다. [Genesis 진행 상태](../../roadmap-implementation/deployment/subscription-genesis-provisioning.md)를 참조하세요. |
