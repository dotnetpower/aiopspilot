...
translation_source_sha: d2372b80592afe0448abffcc143a76964a508368
...
| 감지 거버넌스 정책 | 신원, 의미 체계 버전, 항목 수 상한을 고정한 저장소 정책으로 관리하는 이상 감지 방식과 완화할 수 없는 콜드 스타트 하한, 타입이 지정된 계절 위상, 예측 모델 계열, 대상 기간, 샘플 및 적합도 하한, 런타임이 지원하는 고정 신뢰수준, 축소할 수 없는 정확한 상관관계 키 집합, 완화할 수 없는 T1 유사도 하한, 공유 근거 필드 2개 하한, 최소 14일의 관찰 모드 이후 주간 백테스트, 완화할 수 없는 예측 정밀도, 재현율, 구간 포괄률, 선행 시간, 판단 보류율 경계, 정확한 범위와 완전한 근거를 모두 요구하는 변경 창 보류 | [거버넌스 정책](../../../services/core-control-plane/src/fdai/core/detection/governance_policy.py) 및 [신뢰된 구성](../../../config/detection-governance-policy.json) | [거버넌스 정책 테스트](../../../services/core-control-plane/tests/core/detection/test_governance_policy.py) |
