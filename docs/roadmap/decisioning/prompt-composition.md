...
Runtime selection uses a catalog-owned profile that names one exact root artifact and every ordered
pack by id, version, and layer. File order and the highest available version do not activate a
prompt. One active profile exists per capability; additional profiles remain shadow candidates and
run only through an explicit treatment selection. A profile also pins its system-input budget,
complete provider-request budget, reserved output, and promotion evidence.

Composition emits a digest-bound selection receipt with the profile, ordered artifacts, effective
text, and budget. Frame and plan adapters consume the same receipt instead of opening the catalog
...
Long prompts can silently drop instructions. We treat "the model actually reads
what we sent" as a first-class KPI, gated before promoting a prompt to enforcement mode.

- **Complete request budget** - the composer and adapter account for the selected artifacts,
  generated schema, descriptors, bounded context, operator input, and reserved completion. An
  overrun holds the request before provider I/O and increments
  `prompt.token_budget.exceeded_rate`. Lower-priority optional layers are dropped only through an
  audit-visible profile rule; protected role and safety layers are never trimmed.
...
| To learn about | Read |
|----------------|------|
| Delivery status and remaining work | [Implementation ledger](../../roadmap-implementation/decisioning/prompt-composition.md) |
