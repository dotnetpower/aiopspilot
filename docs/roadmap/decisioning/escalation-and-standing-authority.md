...
- [x] Define the inert effect-spanning A3-E lease with acquisition, bounded validity, provider-commit
  fence, stable idempotency, crash reconciliation, and terminal release, all unwired from authority.
- [x] Implement the inert A3-E promotion-candidate lifecycle with typed candidate, review, revocation,
  and terminal-denial records; creator/reviewer separation; two-phase audit; append-only hash-chained
  history; and static proofs that no authority path imports the candidate module.
- [x] Retain a local synthetic shadow cohort with a canonical content-addressed receipt proving zero
  policy escapes across ≥1 eligible candidate plus quorum, scope, expiry, revocation, lease,
  evidence, identity-separation, and unsupported-provider denials.
