...
translation_source_sha: ef2646d70456f62c995b51ebe3c10cf2fdd60610
...
원자적 사용과 보류 해제를 담당합니다. 이제 보류 원장은 최신 영구 승인 정책과 승인 유효
구간을 같은 트랜잭션에서 확인하고, 승인을 사용하며, fence 세대를 증가시키고, 내용 주소 기반
무권한 해제 증적과 최종 감사를 저장하는 원자적 연산을 제공합니다. 운영 보상 조정기는 아직
기존의 검증된 복구 해제를 사용하므로 이슈 #630에서 새 연산을 연결해야 합니다. 이후 이슈
#640이 각 실행 경로의 기존 논리 대상 잠금 안에서 최종 디스패치 재검사를 담당합니다.
...
런타임은 온톨로지 저장소가 구성되지 않은 경우를 포함하여 모든 운영 게이트 평가기를 이 승인
검사로 감쌉니다. 승인 요청, 근거 읽기, 병렬 분기, 대기 또는 결정 단계에서 부수 효과를
만들거나 진행하기 전에 게이트를 평가합니다.
...
적용 모드의 승인 정족수도 의사 결정 핵심 근거입니다. 실행기는 승인 단계를 완료로 기록하기
전에 영속 프로세스, 단계, 시도, 개정 번호, 요청자, 정족수, 자체 승인 규칙 및 결정 집합을
하나의 승인 요청에 연결합니다. 승인 결과가 없거나 거부되거나 사용할 수 없으면 단계를
명시적으로 실패 처리하며 프로세스를 진행할 수 없습니다.
...
작업 흐름 승인 상태와 HIL 자리 신원은 프로세스, 단계, 시도에 연결됩니다. 시도 1은 기존 영속 기록을 위해 이전 방식 키를 유지하고 이후 시도는 서로 다른 키를 사용합니다. 한 명의 거절은 전체 정족수 시도를 최종으로 만들고 모든 형제 자리를
닫으므로 late 승인이 거절과 경쟁할 수 없습니다. `approval_rejected` 또는 `approval_timed_out` 뒤 범위가 제한된 재시도는 새 시도용 fresh 자리만 만듭니다. 형제 보류 종결이 중단되어도 최종 작업 흐름 CAS가
권위 있는하므로 큐는 stale 또는 만료된 자리를 숨기고 다음 프로바이더 읽기가 physical 보류 상태를 복구합니다. 취소와 시간 초과는 exact 시도를 닫고 거절, 취소, 시간 초과는 서로를 덮어쓰지 않습니다. 작업 흐름 프로바이더가 시간
초과 terminalization을 소유하며 범용 HIL 만료 워커는 작업 흐름 자리를 건너뜁니다. Approval 결정은 영속 기한 전에만 수락됩니다. Late 결정이 먼저 개정 번호를 변경하면 실행기는 해당 시도를 다시 읽고 시간 초과 CAS를
...
디자이너는 폼이 아니라 **오퍼레이터와 함께 워크플로를 공동 설계하는 채팅**입니다.
결정론적 인터뷰 엔진은 평문 질문, 이해한 내용의 재진술, 옵션 칩을 제공하며 `ActionType`
팔레트 밖의 변경을 만들지 않습니다. 자유 텍스트의 의미는 서버가 소유하고 검증한 의미
판단에서만 오며 텍스트 전용 브라우저 호출은 어휘 기반 대체 경로를 사용하지 않습니다
...
- welcome 턴은 **작동 예시** (예: "`aks-cluster-01` 의 pod 가 과열되면 알림을
  ...
  가능한지 본다;
- 검증된 의미 워크플로 판단은 [`suggestDraftFromJudgment`](../../../console/src/routes/workflow-builder.intent.ts)를
  통해 정확한 trigger와 `ActionType` 식별자를 미리 채울 수 있습니다. `0.75` 이상의
  confidence, 모호하지 않은 결과, `execution_authority=false`가 필요하며 알 수 없거나 중복된
  식별자는 인터뷰에서 채울 필드로 남습니다.
