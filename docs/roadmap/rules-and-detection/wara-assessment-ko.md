...
translation_source_sha: bada8c5984e2f6912c049cd5d48185419c81a8dc
...
초기 Operator 카탈로그 변환 결과는 이 요구 사항을 구조화된 읽기 전용 데이터로 보존합니다.
Console 세부 화면은 근거 종류, 권위 있는 생성자, 정확한 범위 계약, 최신성 상한, 책임 담당자
슬롯 및 차단 사유를 표시합니다. 근거 증적을 작성하는 컨트롤은 제공하지 않습니다. 별도로
권한이 부여된 생성자가 허용 가능한 근거를 제공할 때까지 권고는
`manual_evidence_required`와 함께 `not_evaluated`로 유지됩니다.
...
충족 상태 필터를 지원합니다.
공유 물리 전송은 별도의 WAF 및 CAF 프레임워크 평가 논리 토픽도 전달할 수 있습니다. 토픽
필터링, 소비자 그룹, 변환 결과 키, 카탈로그 고정값은 서로 독립적이므로 프레임워크 평가
이벤트가 WARA 상태를 변경하거나 WARA 근거를 충족할 수 없습니다.
Operator 조립 루트는 검토된 fanout 상한을 유지하기 위해 하나의 평가 변환 결과 facade에서
...
관련 없는 Incident 개입 보낼 편지함도 감독할 수 있습니다. 해당 작업자의 요청과 준비 상태는
WARA 근거, 변환 결과 또는 평가 권한에 들어갈 수 없습니다.
...
## 정확한 평가기 overlay와 Azure 관측

생성된 원본 교차워크는 변경하지 않습니다. 별도의 내용 주소화 평가기 overlay가 검토된 APRL
GUID와 정확한 쿼리 다이제스트를 결정론적 평가기 하나에 결속합니다. Loader는 바인딩이
`missing_exact_evaluator`를 제거하기 전에 원본, 교차워크, 쿼리, 안전성, 리소스 종류 및 차단
사유의 변경을 차단합니다.

첫 overlay는 검토된 의미가 "일치하는 행은 실패"인 읽기 전용 쿼리 3개를 연결합니다. Azure
...
집중 검사는 스키마, 가져오기 일치, 교차워크 수량, 평가기 overlay 신원, 쿼리 안전성, 정확한
범위 및 endpoint 적용, 수동 근거, 런타임 실패 사례, 재현, 온톨로지 불변 조건, Operator API
...
토픽 필터링은 해당 레코드를 WARA 소비자에서 분리하며 평가 권한을 부여하지 않습니다. Multiplex
게시는 논리 레코드를 인코딩하거나 보내기 전에 설정된 null이 아닌 물리 토픽 하나를 확인합니다.
