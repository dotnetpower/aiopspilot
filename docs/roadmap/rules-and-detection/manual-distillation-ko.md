...
아티팩트로 바꾼다. 이 아티팩트는 T0/T1 계층이 무료로 평가하고 감사 추적을 갖는다. RAG는
...
| Grounding | 최선 노력 | `provenance` 필수 또는 거부 |
| 감사 / 롤백 | 기본 없음 | 카탈로그 버저닝 + PR |
...
라이브 검색 색인이 필요로 하는 지속적·광범위 읽기 크레덴셜을 결코 필요로 하지 않는다. 업계
...
|---|---|---|---|
| 폐기 / push | 운영자가 문서를 전달: 포크의 매뉴얼 폴더로 PR, 콘솔 업로드, 또는 email-in 주소 | 없음 | 애드혹, 소량, 가장 민감 |
| 지정 공간 | 회사가 FDAI 소스로 지정한 SharePoint 라이브러리 1개 / Confluence 스페이스 1개 / Notion 데이터베이스 1개; 그 한 위치로만 읽기 스코프 | 좁고 저민감 스코프 1개 | 안정적 큐레이션 매뉴얼 |
| iPaaS 트리거 | 엔터프라이즈가 이미 인증한 Power Automate / Logic Apps 플로우가 변경된 페이지를 ingest 웹훅으로 게시 | 없음 (인증은 엔터프라이즈 소유) | 변경 시 자동 갱신 |
| 위임 fetch | 운영자가 ChatOps에 링크를 붙이면 서술기가 운영자의 단기 위임(on-behalf-of / device-code) 토큰으로 읽음 | 상시로는 없음 | 가끔, 권한 있는 읽기 |
...
긍정이 함께 폭발한다. 답은 FDAI 자신의 tiering 철학을 코퍼스에 적용하는 것이다: 싸게
거르고, 비싸게 컴파일하되 소수에만.
