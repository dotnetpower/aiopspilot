...
Framework definitions sit beside these four evidence-bearing artifacts. They preserve the pinned
CAF and WAF hierarchy, source identity, mapping state, and exact completeness scope without becoming
an evaluation verdict. Azure WAF maps all 59 coded checklist recommendations to `BestPractice`
records. CAF records seven adoption methodologies and eight landing-zone design areas as
reference-only controls until a deployment supplies applicability and typed evidence.

WARA uses the Azure Proactive Resiliency Library (APRL) rather than a separate closed question
catalog. FDAI imports all pinned APRL recommendation metadata, including resource type, advisory
impact, product-group verification, automation availability, tags, source digest, and optional
Azure Resource Graph query digest. The query body remains external and inert. An APRL recommendation
...
|-------|---------|---------------|--------------|
| Azure-native | WAF checklists, WARA/APRL, AKS Baseline, MCSB, Azure Policy built-in initiatives, Advisor, Defender recommendations | YAML / JSON / ARM policy / docs | pinned git source, REST API, policy definitions export, docs repo |
...
### MCSB controls and implementation coverage

FDAI stores Microsoft Cloud Security Benchmark (MCSB) controls separately from executable rules.
One benchmark control can need several rules, runtime observations, and manual evidence records;
some strategy or process controls cannot be evaluated from resource configuration alone.
...
of each technology-neutral control. These values describe FDAI implementation coverage, not whether
...
- Missing crosswalk entries materialize as `unmapped`; unknown references, duplicate controls,
  invalid domain prefixes, and contradictory coverage fail closed.

Regenerate the v1 definitions from a downloaded workbook, then run the strict gate:
...
- The normalized `Rule` requires `schema_version` and has no `kind` discriminator. Its strict
  ...
  Config-baseline and measurement-baseline use the `config-baseline` and `measurement-baseline`
  discriminators and their own strict schemas in the same directory.
...
The shipped, reviewed example is [`kubernetes-cluster.hardening.baseline.yaml`](../../../rule-catalog/baselines/configuration/kubernetes-cluster.hardening.baseline.yaml), composed from four Rule ids already in `rule-catalog/catalog/`. `controls` names Rule ids, not inline check logic, so this artifact is **not** the runtime resource-attribute drift snapshot (`FrozenConfigurationBaseline`, see [Storage Layout](#storage-layout)). Its deterministic T0 consumer is [`evaluate_configuration_baseline_control_set`](../../../services/core-control-plane/src/fdai/rule_catalog/schema/baseline_catalog.py), which resolves every `controls` entry against the loaded Rule catalog; the `baseline_deep` step in [`validate-catalog-full.py`](../../../scripts/catalog/validate-catalog-full.py) runs it over every shipped baseline.
...
A verified watcher run also durably mirrors each source's snapshot tree to a private, content-addressed Blob store when configured ([`snapshot_mirror.py`](../../../services/core-control-plane/src/fdai/rule_catalog/pipeline/snapshot_mirror.py), [`delivery/azure/rule_catalog_snapshot_store.py`](../../../services/core-control-plane/src/fdai/delivery/azure/rule_catalog_snapshot_store.py)), and, when a GitHub token is configured, opens one **review-only, draft** pull request per source describing that mirrored evidence - never `rule-catalog/catalog` or `rule-catalog/baselines` content - ([`rule_catalog/pipeline/review.py`](../../../services/core-control-plane/src/fdai/rule_catalog/pipeline/review.py), [`delivery/gitops_pr/collection_review.py`](../../../services/core-control-plane/src/fdai/delivery/gitops_pr/collection_review.py)), reusing the existing `RemediationPrPublisher` / `GitOpsPrAdapter` GitOps seam. Landing new catalog content still requires a separate, human-reviewed catalog-as-code change.
