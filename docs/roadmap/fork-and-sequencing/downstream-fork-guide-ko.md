...
사본은 수정하지 않습니다. 따라서 일부만 준비한 프레임워크 파일은 준비한 내용만 증명하며, 관련 없는
작업 트리 무결성 변경은 해당 세션에서 계속 사용할 수 있습니다. Pre-push 검증은 게시 전에 커밋된
스냅샷을 계속 확인합니다. 인덱스 생성은 준비된 매니페스트의 기존 `generated_at` 값을 유지하므로
pre-commit을 반복 실행해도 바이트가 같은 결과를 만듭니다.
