...
covered by focused tests. An accountable Heimdall trigger, a durable `state_kv` ledger, and a
bounded read-only operator panel are implemented and covered by focused tests; production
inventory, natural-language, and review-delivery bindings remain incomplete. No governed live
runtime receipt has been captured, so no area is claimed as operationally validated against live
Azure evidence.
...
| Twin-specific operator panel and governed remediation proposal bridge | in-progress | [`posture_activity.py`](../../../services/core-control-plane/src/fdai/core/assurance_twin/posture_activity.py), [`runtime/assurance_twin_posture.py`](../../../services/core-control-plane/src/fdai/runtime/assurance_twin_posture.py), [`assurance_twin_posture.py` (delivery)](../../../services/core-control-plane/src/fdai/delivery/assurance_twin_posture.py), [`state_store_assurance_twin_posture.py`](../../../services/core-control-plane/src/fdai/delivery/persistence/state_store_assurance_twin_posture.py), [`assurance_twin_posture_projection.py`](../../../services/operator-service/src/fdai_operator_service/assurance_twin_posture_projection.py), the `/assurance-twin/posture`, `/assurance-twin/reviews`, and `/assurance-twin/reviews/{review_id}` Operator API routes, the [`assurance-twin` Console route](../../../console/src/routes/assurance-twin.tsx), and their focused tests (`110 passed`) | Heimdall records an ambient twin candidate that arrives on its declared `object.event` subscription: it writes the durable `state_kv` body with bounded activity, correlation, and evidence-digest provenance, then publishes a schema-validated `agent.operational-activity` tip (`assurance-twin.posture` kind, schema `1.2.0`, `execution_authority` const `false`). A conflicting redelivery under one `review_key` never replaces the durable body and publishes an explicit unavailable tip. The Operator API and Console render stored evidence verbatim and withhold stale, unavailable, unknown, malformed, or digest-mismatched rows as explicit gaps. No upstream component publishes these candidate events yet, and remediation-proposal bridging and the production inventory binding remain not-started, so this row stays in-progress rather than implemented. |
...
- [x] Implement a read-only Twin posture API and console panel over an accountable, schema-validated
  Heimdall trigger, with focused tests covering the durable ledger, redelivery, conflicting
  redelivery, malformed candidates, and fail-closed rendering.
- [ ] Capture a governed runtime receipt for one complete inventory-to-report rendering. This
  requires the production `Inventory` binding and an upstream producer for the ambient twin
  candidate events, so implementation is complete but validation is not: no live receipt exists.
...
| `posture_activity` | Build the bounded, schema-validated `agent.operational-activity` tip (Heimdall-owned, `assurance-twin.posture` kind) for a computed `PostureAssessmentReport` or `IacReview`; carries no findings, only a bounded evidence count and freshness. The accountable trigger that calls it lives in [`runtime/assurance_twin_posture.py`](../../../services/core-control-plane/src/fdai/runtime/assurance_twin_posture.py) and is bound to Heimdall at the composition root. |
