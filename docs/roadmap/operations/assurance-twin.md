...
- [ ] Design a change-review activity-publication path that survives the CAS/async-publish ordering
  hazard (a durable conflict tombstone and a bus publish are separate, unordered steps), with tests
  proving a published tip can never misrepresent a row's durable truth. Until then, a change review
  is durably persisted and readable through the Operator API/Console but publishes no activity tip.
