...
| 2026-09-04 | implemented | Hardening round 2 made the WARA read-plan deadline cover the complete paginated observation instead of resetting for every ARG page. | `current change`; focused delayed multi-page regression. | Preserve the same total-deadline contract when a scheduled assessment worker is added. |
| 2026-09-04 | implemented | Added an exact-scope Azure Resource Graph reader for three reviewed WARA evaluators. The path remains outside general campaign discovery and cannot widen registered source scope, query templates, or action authority. | `current change`; focused WARA overlay, runtime, endpoint, scope, pagination, and deterministic receipt checks. | Add it to a scheduled assessment only through a separately reviewed source registration and retain the authorized live shadow receipt. |
| 2026-08-25 | implemented | Isolated campaign Event Bus composition from full Core configuration validation. A long-running worker could retain an older environment provider in memory, read a newer JSON Schema from the updated checkout on its next loop, and exit when unrelated web-search fields no longer matched. The worker now reads only its required Kafka bootstrap binding and optional dead-letter suffix. | `current change`; `delivery/observation_campaign_cli.py`; `test_observation_campaign_cli.py`; the focused mixed-configuration regression passed. | Restart the local stack so the recurring worker loads this revision. Deployed-revision campaign evidence remains open. |
| 2026-08-28 | implemented | Preserved valid observation Job names and added a compact environment-scoped form only when Azure's 32-character limit would be exceeded. | Failed staging plan-only run `33111410162`; `current change`; focused naming checks and Terraform validation. | Retain one successful deployed campaign run on the corrected name. |
...
|--------|--------------------|-------------------|-------------------|
| `inventory` | Wake every minute; reconcile when the six-hour gate is due or an observed change remains unreconciled above its floor | Huginn | Azure Resource Graph with ARM fallback |
