...
| Web chat and memory | JSON/SSE chat, principal-scoped history/preferences/memory, AnswerPlan, and progressive verification ship. The bounded timing parser preserves Core's separate durable queue phase instead of discarding the complete timing envelope. |
