...
During Kubernetes identity schema rollout, a legacy Resource row remains visible without exact
identity or diagnostic details. The Console never fills missing identity fields from the Resource
...
completeness, cutoff, source revisions, gaps, conflicts, evidence references, and no-authority
boundary. The decoder rejects unknown status values and mismatched source maps. When Operator
withholds a stale or mismatched receipt, the Inspector shows an explicit unavailable explanation
instead of reusing the previous diagnosis.
...
Knowledge > Documents restores a bounded collection-scoped document list after refresh. Changing the collection clears prior rows before the next authorized read, and loading, unavailable, error, and empty states remain distinct without granting document or collection authority. An unavailable document action remains keyboard focusable so its explanation is discoverable, but it has no activation handler and cannot invoke the operation.
...
semantic projection rather than substituting generic inventory data.
