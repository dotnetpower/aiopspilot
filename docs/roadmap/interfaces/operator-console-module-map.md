...
[`test_operator_api_layout.py`](../../../services/operator-service/tests/) also pins the exact default method, path, and route-name set plus representative HTTP envelopes. An intentional default route addition updates this reviewed baseline in the same change.
Repository catalogs are materialized into revisioned Operator projections, while `runtime_projection_reader.py` and `conversation_assurance_reader.py` read only the durable tables granted to the Operator role. Conversation assurance detail joins a stored assessment to semantic request and result records only when principal, conversation, and turn identities all match. Browser replay retains that authoritative assessment identity, hydrates an older cached answer once when the identity is missing, and uses the same detector for initial restoration and explicit retry. List refresh accepts only the latest in-flight response. A valid content-addressed link can read principal-scoped detail beyond the bounded list, while a missing detail stays unavailable and refreshes both list and detail generations. Multiplexed non-request Kafka publications add their logical-topic marker before one bounded encode; schema-validated semantic requests retain their producer-codec boundary. These readers return measured records or explicit empty evidence states and never infer execution eligibility.
...
The review list also selects each durable `state_kv` key and requires its suffix to match the body's opaque `review_key` byte for byte. The Console renders every usable posture scope and uses distinct
copy for a genuinely empty ledger and for rows withheld by an evidence gap. At narrow widths, opaque provenance values wrap inside the primary content, table overflow stays inside its scroll
container, and mobile back and review-detail links provide 44-pixel touch targets.
...
| `families/cost_governance/` | Authenticated availability, aggregate analytics, disclosure, and exact-revision enablement routes for the optional Cost Governance workspace | Keep raw provider data and action authority outside the family. Only an Owner may write the enablement preference; activation never promotes or executes an action. |
...
requester, roles, pinned Workflow catalog entry, and durable Var approval state into
presentation-safe requirements and requestable transitions. The workflow family adapter repeats
the same pure precheck under the Process row lock before it writes an inert proposal. None of these
modules consumes the proposal, changes Process state, approves, or executes.
