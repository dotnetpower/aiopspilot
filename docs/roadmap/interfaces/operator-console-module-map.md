...
The review list also selects each durable `state_kv` key and requires its suffix to match the body's
opaque `review_key` byte for byte. The Console renders every usable posture scope and uses distinct
copy for a genuinely empty ledger and for rows withheld by an evidence gap.
