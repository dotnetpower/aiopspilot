...
translation_source_sha: 5f634fc95d96421bddf0ac8ea9ff0ab8d4842765
...
> Console `done` 변환 결과는 이제 compose됩니다. 로컬에서 검증한 답변 완료 변환 결과는 서로 다른
> 재생 커서를 가진 누적 증적 결속 `confirmed` 구간도 보낼 수 있습니다. 최종 검증, 근거, 증적,
> 개정 번호, 순서 또는 텍스트가 다르면 해당 구간을 철회합니다. POST 재생은 요청 기한까지 기다리고 Core 변환 결과가
...
| 온톨로지 매니페스트 목록 및 선언 개수 | 구현됨 | `semantic_manifest_planning.py`, `semantic_planning_frame_checks.py`, `semantic_planning_plan_dispatch.py`, `semantic_turn_processor.py`, 집중 판단, 계획 및 이중 언어 표현 회귀 검사 | 현재 보이는 범위의 queryable 선언을 대상으로 하는 검증된 무대상 요청은 정확한 선언 종류를 서버 소유 `query.manifest` 목록으로 컴파일하고 프레임 또는 플랜 모델을 호출하지 않습니다. 정확한 타입 지정 형식은 `queryable`과 `visible`, `object_type_visibility`와 `list`, 단수 종류와 `visible`, `current_scope`, `list`, 복수 종류와 `visible_to_operator`, 복수 종류와 `visible_in_current_scope`를 포함합니다. 모두 `current_scope`를 보존하고 가시성, 현재 범위 또는 `list`와 `queryable`이 모두 없으면 거부합니다. 선언 개수 요청에서는 canonical `*Type` target 하나가 충돌하는 frame subject보다 우선하고 선언이 아닌 영역 target은 제외합니다. 완전한 출력은 타입이 지정된 행 또는 개수와 읽기 전용 출처를 표시하고, 표시 종류를 모델이 작성한 node ID가 아니라 검증된 frame에 결속합니다. 모호성, 충돌하는 canonical target, 사용할 수 없는 매니페스트 Function, 불완전한 출력 및 다른 측정값은 기존의 안전한 실패 경로를 유지합니다. |
...
| 2026-09-09 | implemented | 모델이 제안한 ResourceType 대상을 보존하는 결정론적 지역화 명확화로 하위 유형만 있는 exact-target 모호성을 안정화했습니다. | `current change`, 집중 판단 검사 및 요청 지표가 모두 100%인 탐색용 v14 집단 | 동기화 후 최종 clean source에서 반복합니다. |
| 2026-09-09 | validated | upstream 동기화 후 clean source `a5d3627b3`에서 v14를 반복했습니다. | 요청한 모든 의도 지표와 보조 의도 재현율 및 정확한 범위 유효성은 100%이고, 안전 오탐률 두 가지와 만들어 낸 신원 수는 0이었습니다. | 더 넓은 이중 언어 반복 및 frame v41 종단 간 근거가 승격을 승인할 때까지 shadow 모드를 유지합니다. |
...
| 2026-09-01 | implemented | 실제 변형 두 개에서 승인된 판단은 canonical `ActionType`을 포함했지만 이후 frame이 `LinkType`을 제안한 것을 확인하고 우선순위를 수정했습니다. 고유한 canonical 선언 target은 충돌하는 frame subject보다 우선하고, 정확한 frame subject는 target이 없을 때의 대체값으로 유지하며, 여러 canonical target은 해결되지 않은 상태로 유지합니다. | `current change`, canonical target과 frame 충돌 및 target 없는 대체값 회귀 검사, 운영 개수 제어, Ruff, strict mypy 및 anti-hardcoding 검사 | 커밋한 뒤 원래 질문과 저장된 유사 질문 3개를 다시 측정합니다. |
| 2026-09-01 | implemented | 남은 두 변형이 의미 판단 target을 생략하거나 결속되지 않은 화면 target을 추가하면서도 정확한 `ActionType` frame subject를 제공한 것을 확인하고 선언 종류 정규화가 frame을 우선하도록 수정했습니다. 고유한 canonical target은 검증된 frame에 정확한 subject가 없을 때만 대체값으로 사용합니다. | `current change`, frame 우선 및 target 없는 판단 회귀 검사, 운영 개수 제어, Ruff, strict mypy 및 anti-hardcoding 검사 | 커밋한 뒤 원래 질문과 저장된 유사 질문 3개를 다시 측정합니다. |
| 2026-09-01 | implemented | 정규화 후 회귀 질문 집단이 유효한 매니페스트 개수를 실행했지만 일반 행 개수 표현기를 계속 선택하는 것을 확인하고 남아 있던 모델 노드 ID 의존성을 제거했습니다. 조회 표현기는 검증된 frame subject를 전달받고 타입이 지정된 선언 종류로 집계값에 이름을 붙입니다. | `current change`, 의미 턴 처리기, 영어 및 한국어 표현기 회귀 검사, Ruff 및 strict mypy | 커밋한 뒤 원래 질문과 저장된 유사 질문 3개를 다시 측정합니다. |
| 2026-09-01 | implemented | 회귀 질문 집단의 세 변형이 canonical `ActionType`과 `count` facet을 보존했지만 추가 표시 facet 또는 스키마 타입 이름을 방출한 것을 확인하고 승인된 의미 판단에서 선언 개수 frame을 정규화했습니다. 정규화는 검증된 기계 계약 필드만 사용하며 구문 또는 토큰 라우팅을 추가하지 않습니다. | `current change`, 의미 매니페스트 플래너, frame 검사, 승인된 판단 회귀 검사, 운영 개수 실패 제어, Ruff, strict mypy 및 anti-hardcoding 검사 | 커밋한 뒤 원래 질문과 저장된 유사 질문 3개를 다시 측정합니다. |
| 2026-09-01 | implemented | 실제 회귀 질문 집단에서 일반 표현기가 집계 행 안의 개수 대신 집계 행 1개를 보고하는 문제를 확인하고 선언 개수 표현을 수정했습니다. 집계는 선언 종류를 보존하며 Core 표현기는 완전한 출력에 대해서만 종류별 고유 개수를 보고하고 읽기 전용 출처가 `query.manifest`임을 밝힙니다. | `current change`, 매니페스트 계획, 의미 턴 처리기, 성공 및 실패 계획 회귀 검사, 영어 및 한국어 표현 회귀 검사 | 이 후보를 커밋한 뒤 원래 질문과 저장된 유사 질문 3개를 다시 측정합니다. |
| 2026-09-01 | implemented | 유효한 `query.manifest` 개수 조회가 여전히 모델이 작성한 계획에 의존한다는 대화 보증 실패를 확인하고 결정론적 선언 개수 계획을 추가했습니다. Core는 자연어를 다시 해석하거나 권한을 추가하지 않고 검증된 집계 frame을 컴파일합니다. | `current change`, `semantic_manifest_planning.py`, 계획 디스패치, 집중 성공 및 운영 개수 회귀 검사 | 대화 보증 실패를 해결된 것으로 처리하기 전에 원래 질문과 유사 질문 집단의 실제 통과 결과를 보존합니다. |
...
| 2026-09-08 | 구현됨 | 목표, 완료 증적 및 숫자 행 수 출력이 일치한 뒤에만 범위가 제한된 정확한 ObjectSet 정의를 표시합니다. 같은 no-authority 실행 기록이 반환 행 수, 전체 행 수, 완전성 및 잘림 메타데이터를 전달하며, 실행 중인 진행 상황과 모순된 행 수는 기능 이름만 표시합니다. | `current change`, [Issue #241](https://github.com/dotnetpower/fdai/issues/241), 집중 Operator 테스트 153개, Ruff, 서식 검사, 엄격한 소스 mypy, Console 타입 검사 및 인증된 세 뷰포트 실행 기록 시나리오 통과 | 검증된 ObjectSet과 행 수 표시 범위에 남은 작업은 없습니다. |
### 남은 작업

- [ ] 활성 의미 프롬프트 프로필을 바꾸기 전에 하나의 정확한 소스 개정 번호에서 활성 및
  compact 판단, frame, plan 집단을 승인된 모델로 비교합니다.
