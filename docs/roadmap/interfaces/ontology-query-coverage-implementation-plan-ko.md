...
| 온톨로지 매니페스트 목록 및 선언 개수 | 구현됨 | `semantic_manifest_planning.py`, `semantic_planning_frame_checks.py`, `semantic_planning_plan_dispatch.py`, `semantic_turn_processor.py`, 집중 판단, 계획 및 이중 언어 표현 회귀 검사 | 현재 보이는 범위의 queryable 선언을 대상으로 하는 검증된 무대상 요청은 정확한 선언 종류를 서버 소유 `query.manifest` 목록으로 컴파일하고 프레임 또는 플랜 모델을 호출하지 않습니다. 정확한 타입 지정 형식은 `queryable`과 `visible`, `object_type_visibility`와 `list`, 단수 종류와 `visible`, `current_scope`, `list`, 복수 종류와 `visible`, `current_scope`, 복수 종류와 `visible_to_operator`, 복수 종류와 `visible_in_current_scope`를 포함합니다. 모두 `current_scope`를 보존하고 가시성 또는 현재 범위가 없으면 거부하며 단수 종류에는 `list` 또는 `queryable`도 필요합니다. 선언 개수 요청에서는 canonical `*Type` target 하나가 충돌하는 frame subject보다 우선하고 선언이 아닌 영역 target은 제외합니다. 완전한 출력은 타입이 지정된 행 또는 개수와 읽기 전용 출처를 표시하고, 표시 종류를 모델이 작성한 node ID가 아니라 검증된 frame에 결속합니다. 모호성, 충돌하는 canonical target, 사용할 수 없는 매니페스트 Function, 불완전한 출력 및 다른 측정값은 기존의 안전한 실패 경로를 유지합니다. |
