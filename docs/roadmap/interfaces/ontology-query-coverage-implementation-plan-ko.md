...
translation_source_sha: 70aeca8a5700bda64fec7eddd9d36b6ad81d862a
...
| 영역 | 상태 | 근거 | 참고 |
|------|------|------|------|
| Azure 및 인시던트 의미 판단 | implemented | `semantic_judgment.py`, `semantic_planning_judgment.py`, shadow 프롬프트 v9, v10, v41, 집중 골든 재생 | 추가 후보는 명시적 금지 작업, 제공된 의도와 신원, 검토된 FunctionType 측정값 및 정확한 범위를 보존합니다. 첫 실제 v9 집단은 안전했지만 정확도 게이트를 통과하지 못해 v10을 shadow로 유지하며 운영 검증은 열어 둡니다. |
...
허용하고 범위 수정을 거부합니다. 이 후보는 shadow pack으로 유지하며, 활성 v8 프롬프트와 엄격한
출력 스키마는 승격 전까지 추가 필드를 제외합니다.
