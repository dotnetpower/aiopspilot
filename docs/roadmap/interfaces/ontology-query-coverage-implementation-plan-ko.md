...
translation_source_sha: 9b9a8154364d5bc187c9aedbc382c259c3ce0403
...
| 온톨로지 매니페스트 목록 및 선언 개수 | 구현됨 | `semantic_manifest_planning.py`, `semantic_planning_frame_checks.py`, `semantic_planning_plan_dispatch.py`, `semantic_turn_processor.py`, 집중 판단, 계획 및 이중 언어 표현 회귀 검사 | 현재 visible 범위의 queryable 선언을 대상으로 하는 검증된 무대상 요청은 정확한 선언 종류를 서버 소유 `query.manifest` 목록으로 컴파일하고 frame 또는 plan 모델을 호출하지 않습니다. 정확한 typed 형식은 queryable 및 visible, 선언 visibility 및 list, 복수 종류 및 operator visibility, 복수 종류 및 결합된 visible-current-scope를 포함하며 모두 current scope를 보존하고 visibility 또는 scope가 없으면 거부합니다. 선언 개수 요청에서는 canonical `*Type` target 하나가 충돌하는 frame subject보다 우선하고 선언이 아닌 영역 target은 제외합니다. 완전한 출력은 타입이 지정된 행 또는 개수와 읽기 전용 출처를 표시하고, 표시 종류를 모델이 작성한 node ID가 아니라 검증된 frame에 결속합니다. 모호성, 충돌하는 canonical target, 사용할 수 없는 매니페스트 Function, 불완전한 출력 및 다른 측정값은 기존의 안전한 실패 경로를 유지합니다. |
