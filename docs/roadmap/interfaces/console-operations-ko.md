...
간단한 그래프 노드는 정확한 운영 값을 먼저 유지합니다. 운영 축이 적용 대상이 아니거나 공급자가
운영 상태를 제공하지 않으면 정확한 가용성 값이나 근거 누락을 선택하고, 그다음 프로비저닝 값을
운영 상태 사용 불가 자리표시자보다 먼저 사용하며 선택한 축을 표시합니다. 적용 가능한 운영 값의
누락은 계속 드러납니다. 이는 화면 표현 우선순위일 뿐이며 축을 합치거나 가용성 또는
프로비저닝에서 운영 성공이나 정상 상태를 추론하지 않습니다.

Resource 목록에는 상한이 있으므로 검색하지 않은 페이지는 완전한 인벤토리가 아닙니다. Dashboard
v2는 현재 인벤토리가 커밋된 온톨로지 매니페스트와 일치할 때만 기록 상태를 읽고, 서로 다른 세대의
페이지를 차단하며, 알 수 없음의 원인을 구분해 유지합니다. 무효화 알림을 받으면 범위가 제한된
스냅샷을 교체합니다. 검색 중에는 목록 상한 밖의 Resource도 계속 찾을 수 있습니다. 제안을 직접
...
관계 개수는 중첩된 관계도 표시된 것으로 셉니다. 중첩은 선을 없애므로 선만 세면 그래프가 실제로
그리는 것보다 적게 보고하게 됩니다. 실측한 클러스터에서 상자가 포함 관계 36개를 보여 주는 동안
개수는 190개 중 11개로 떨어졌습니다. 운영자에게 그래프를 얼마나 보고 있는지 알리는 개수라면
상자가 담고 있는 관계까지 포함해야 합니다.
...
Process 상세 변환 결과는 변경할 수 없는 `process.created` 근거에 기록된 인증된 요청자 범위로
제한됩니다. 저널을 불러온 뒤 Process를 다시 읽고, 권위 있는 카탈로그 변환 결과에서 고정된
Workflow 버전과 승인 단계의 영속 Var 상태를 결합하여 현재 단계 유형, 요구 사항, 기한 또는
근거 상태, 정확한 리비전 및 요청할 수 있는 전환만 반환합니다. 요청자 근거 누락, 카탈로그
불일치, 리비전 경합 또는 잘못된
단계 근거는 전환이 없는 사용 불가 제어 상태를 반환합니다.

Operator 워크플로 어댑터는 Process 행 잠금 아래에서 재개, 취소 또는 재시도 제안을 영속화하는
동일한 데이터베이스 트랜잭션 안에서 이 변환 결과를 다시 검사합니다. 정확한 중복 요청은 상태
진행 후에도 멱등성 재생을 보존하도록 재검증 전에 반환합니다. `If-Match`는 현재 숫자 Process
리비전과 같아야 하고, 적용 모드 요청에는 Owner가
필요하며, 재시도에는 런타임과 같은 효과 없는 종료 근거가 필요합니다. 잘못되거나 사용할 수 없는
전환은 차단됩니다. `202` 증적은 shadow 우선 요청이 런타임 검토 대상으로 수락되었다는 뜻일
뿐입니다. Process 진행, 승인, 실행 또는 검증된 운영 성공이 아닙니다. 사람 승인은 별도의 Var
소유 Approvals 경로에 유지되며 서로 다른 principal 규칙을 보존합니다.
...
[리소스 대시보드 시안](../../../mocks/ui/dashboard-v2.html)은 합성 데이터를 사용하는 화면 설계이며,
실제 인벤토리 조회 기능이 아닙니다. 로컬 예제 어댑터는 리소스 24개, 100개, 1,000개, 10,000개를 제공합니다.

| 고려 사항 | 시안 및 실제 콘솔 적용 기준 |
|-----------|---------------------------|
| 콘솔 구성 일치 | 개요, 운영, 에이전트 시안은 Console 화면 등록 정보를 따르며 운영 성과 대시보드와 리소스 대시보드를 구분합니다. 운영 기록 화면은 탭과 미리 보기 선택기를 요약 지표 위에 배치하고 공간이 충분하면 상세 내용을 목록 옆에 유지합니다. [운영 시안 검사](../../../console/tests/e2e/operator-workspaces.spec.ts)는 경로, 로컬 조작, 반응형 화면을 확인합니다. 합성 예제는 요청을 전송하지 않으며, 대기열 접수 예제로 수명 주기를 변경하거나 감사 근거를 만들어 내지 않습니다. |
| 대규모 범위 | 리소스가 48개를 넘으면 개별 리소스를 조밀하게 배치한 허니콤으로 시작합니다. 작은 예제는 넓은 셀을 사용합니다. 그룹 보기는 구독과 리소스 그룹을 탐색하는 선택 사항이며, 개별 리소스의 전체 현황을 대체하지 않습니다. |
| 렌더링 한도 | 조밀한 셀의 크기는 24 x 28 px입니다. 사용 가능한 너비에 따라 최대 34열, 14행을 구성하며 페이지당 최대 476개를 표시합니다. 넓은 셀과 목록 보기는 48개, 그룹 요약은 6개 한도를 유지합니다. 활성 페이지와 보기만 렌더링합니다. 이 표시 한도는 서버 처리 용량을 뜻하지 않습니다. |
| 집계와 선택 | 관측된 인벤토리, 필터 일치 수, 표시 수를 구분합니다. 범례 집계는 상태 필터 적용 전의 선택 범위를 기준으로 합니다. 선택한 리소스가 페이지나 필터 밖에 있어도 식별자와 근거를 유지하며, 예제 세대가 바뀌면 선택을 해제합니다. |
| 리소스 유형 검색 | 검색형 선택기는 표시 이름, 명시된 약칭, Azure 고유 리소스 형식으로 검색합니다. 입력 중인 텍스트는 필터에 적용하지 않으며, 목록 항목을 선택하거나 명시적으로 해제해야 적용됩니다. 제안은 최대 12개를 표시하고 전체 일치 수를 알립니다. 건수는 유형과 상태 필터 적용 전의 구독, 그룹, 검색 범위에서 수신한 기록을 기준으로 합니다. 관측 건수가 0인 카탈로그 항목이 해당 범위 밖에도 없다는 뜻은 아닙니다. 실제 적용 시 키보드, 한글 조합 입력, 터치, Escape 취소 동작을 유지합니다. |
| 정보 확인과 입력 | 마우스를 올리거나 키보드 초점을 옮기면 같은 스냅샷의 상태, 범위, 관측 시각을 표시합니다. 미리 보기 위로 포인터를 옮겨도 유지되며 Escape로 닫을 수 있습니다. 클릭 또는 Enter는 지도나 초점을 이동하지 않고 비모달 상세 패널을 고정합니다. 지도에는 Tab 진입점을 하나만 두고 방향키와 Home/End로 탐색합니다. 좁은 화면이나 터치 입력은 44 px 이상의 선택 영역과 닫을 수 있는 비모달 고정 패널을 사용합니다. |
| 근거 상태 | 운영 상태, 가용성, 관측 범위를 독립적으로 표시합니다. 인벤토리가 일부만 있으면 전체 수는 알 수 없습니다. 오래된 관측으로 현재 상태를 판단하지 않습니다. 로딩은 뼈대 화면으로 시작하고, 읽기 실패를 0으로 표시하지 않으며, 빈 인벤토리와 필터 일치 결과 없음을 구분합니다. |
| 확인 항목과 이력 | 미리 작성된 확인 항목은 지도 필터와 독립적이며 전체 작업 큐가 아닙니다. 예제 이벤트와 과거 성과 지표는 각각의 시간 구간을 유지합니다. 수신한 인벤토리에 없는 리소스를 이벤트에서 선택해 가상의 기록을 만들지 않습니다. |

별도 `/dashboard-v2` 경로는 기존 `/overview`와 기본 대시보드를 유지합니다.
기존 그래프의 단일 상태 문자열 대신 온톨로지 인스턴스와 같은
[공통 기록 상태 API](recorded-resource-state-ko.md)를 사용합니다. 운영, 프로비저닝, 가용성 값마다
별도 출처 경로와 근거 한정 정보를 유지합니다. 서버 페이지는 세대, 기준 시점, 검색, principal
문맥에 결속되며 로컬 필터는 제한된 수신 집합에만 적용합니다. 최신성을 알 수 없어도 기록된
값을 지우지 않고, 기록된 값으로 현재 정상 여부를 판정하지도 않습니다. 합성 예제는 사용하지 않습니다.
스냅샷 교체 시 선택을 해제하고 이전 응답은 반영하지 않습니다. 공통 조회에서 역할 할당과
범위 컨테이너를 제외합니다. 독립적인 변경 이력은 리소스 근거 화면에서 확인합니다.
로컬 검사는 화면 동작만 입증하며 툴팁 위치 계산은 필요할 때 로드합니다.
