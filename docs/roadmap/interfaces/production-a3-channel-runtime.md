...
| 2026-09-08 | implemented | Bound the dedicated non-executor edge identity from the authoritative platform state output into the Operator service tfvars during an enable transition. The materializer validates the exact identity shape and refuses an enabled edge without both resource and client identifiers. | Failed service plan `34225350538`; `current change`; focused service materializer and workflow tests. | Rerun the guarded create-only plan, then apply and retain the runtime receipts. |
| 2026-09-08 | implemented | Added the masked `OPERATOR_CHANNEL_EDGE_PROVIDER_JSON` deployment binding so an older `SERVICE_DEPLOY_TFVARS_JSON` can enable Slack without exposing tenant resource identifiers in workflow logs or widening its service configuration. | `current change`; focused service materializer and workflow tests. | Run and apply a guarded create-only Operator plan, then retain health, delivery, restart-reconciliation, disable, and removal receipts. |
| 2026-09-08 | implemented | Added an exact-revision VNet-runner workflow that materializes and reads back the fixed Slack A3 secrets from provider-hosted GitHub Secrets without logging values or retaining an artifact. | `current change`; focused secret transport, workflow security, runner registration, login binding, and CI contract tests. | Run the workflow on a green revision, bind the resulting versionless secret identifiers to the platform and Operator plans, then retain delivery and rollback receipts. |
...
- [x] Configure one real Slack or Teams provider profile and principal mapping through local-only
