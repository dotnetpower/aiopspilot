...
|------|-------|----------|-------|
| Protected Slack secret materialization | implemented | `deploy-channel-edge-secrets.yml`; `materialize_channel_edge_secrets.py`; focused workflow and transport tests | An exact-green development revision can transfer five GitHub Secrets to four fixed secrets in the single tagged deployment-owned Key Vault. The platform and tenant policy continue to own private-network and RBAC posture. The VNet runner verifies the stable deploy identity, writes no value to arguments or artifacts, reads every value back, and shreds identifier inventory. This step grants no channel, approval, or execution authority. |
