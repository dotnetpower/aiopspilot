...
| Dedicated system-knowledge Teams bot | in-progress | [System Knowledge Service](system-knowledge-service.md); `services/system-knowledge-service/`; focused service checks | The read-only bot uses a separate distribution, application identity, catalog, and delivery ledger. Production Teams and rollback evidence remain open and do not change A1-A4 readiness. |
...
answer clipped to the 4,000-character answer-block limit and a canonical Slack answer clipped to
the 2,900-character Block Kit section limit after mrkdwn escaping. The metric records only that
truncation occurred. When multibyte JSON encoding reaches the Teams card byte ceiling first, the
answer is clipped earlier with the same marker and metric. The omission count and clipping marker
stay visible in the vendor message, and the original activities and answer remain in the durable
response evidence.
...
Teams carries four different contracts with different owners. The A1 contract has separate send
and callback owners, so Settings > Integrations reports five independent rows instead of one
aggregate "ChatOps" status. Readiness means *configuration completeness observed in one runtime*;
it is never a provider health claim.

| Row | Owning runtime | Ready when |
|-----|----------------|-----------|
| `teams-a1-approval-send` | Core control plane | The Bot activity endpoint plus the group-connected team and channel are all present |
| `teams-a1-approval-callback` | Operator Service | The Bot application, tenant, JWKS, allowed service URLs, principal map, callback secret, HIL decision topic, and durable outbox are all present and structurally valid |
| `teams-a2-operational-alert` | Core control plane | A Teams Workflows binding declaring `a2_operational_alert` is activated and its endpoint is not the seeded placeholder |
| `teams-a4-digest` | Core control plane | The same, for `a4_digest` |
| `teams-a3-conversation` | Operator Service | The conversation channel edge enables Teams and its identity inputs are present |

Partial configuration is **incomplete**, not degraded: a half-configured approval path leaves
approvals unavailable rather than silently downgraded. Each row also carries its owning runtime and
an `observed` flag, because a projection produced by one process cannot see another process's
inputs. An unobserved row reports "not observed here" rather than claiming "not configured", so an
operator repairs the runtime that actually owns the prerequisite.
