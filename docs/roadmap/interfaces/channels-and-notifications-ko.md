...
| 2026-08-19 | 구현됨 | 프로바이더 중립 렌더러 Protocol과 기능 프로필, 엄격한 일회성 산출물 정규화, 필수 읽기 가능 대체 텍스트, 순수 Adaptive Card 및 Block Kit builder, 가짜 사용자 지정 렌더러를 추가했습니다. 큰 페이로드는 텍스트 대체 경로 전에 선택적 시각 상세를 제거하며 필수 제한, 근거, 권한 및 사용 불가 상태는 사라지지 않습니다. | `current change`, [이슈 #234](https://github.com/dotnetpower/fdai/issues/234), 집중 렌더러 및 영속 응답 검사 40개와 Ruff, formatting, strict mypy 통과 | 운영 Slack 및 Teams A3 전송, 신원, 확인 응답, 시작 및 런타임 증적은 열린 상태입니다. |
...
| 2026-09-09 | 구현됨 | Teams A1 안내를 Owner 전용 revisioned protected-plan 요청에 연결했습니다. Console이 준비 요청 상태를 표시하기 전에 Operator가 제안과 권한 없는 상태를 영속화합니다. | `current change`, 집중 Operator 영속화 및 route 검사, Console decoder 테스트, typecheck, build 및 strict mypy | 제안을 보호된 프로비저닝 workflow에 연결하고 프로바이더 및 배포 증적을 보존합니다. |
...
### 4.1 발신자 pairing trust 초기화

`ChannelAccessService`는 채널별 `disabled`, `allowlist`, `pairing`을 지원합니다. 영속
PostgreSQL 저장소는 channel-scoped 트랜잭션 잠금으로 요청 생성을 직렬화하고 pending 상한을
atomic하게 강제하고 만료된 요청을 상한에서 제외하고 저장된 다이제스트를 조건부 승인합니다.
승인된 발신자는 재등록해 principal 대응을 덮어쓸 수 없습니다.
...
Cross-channel 신원 링크는 신원 병합이 아니라 pairing 위의 명시적 관계 기록입니다.
두 발신자는 같은 FDAI principal에 각각 독립 승인되어 있어야 하고 링크는 서로 다른 두 채널
종류를 연결해야 하며 별도 authorized 행위자가 승인해야 합니다. 발신자 대응이 서로 다른 두
principal을 가리키면 서비스는 쓰기 전에 요청을 거부합니다. 결정론적 링크 id로 재시도가
멱등적하고 PostgreSQL 기록은 어느 발신자 대응 또는 principal 역할도 변경하지 않으면서
재시작 후에도 유지됩니다.
...
명시적 출처 또는 dedicated 스레드 모드와 opaque 기준점 id 메타데이터를 사용합니다. 응답은
...
서버 도구, 정제된 명령, 안전한 결과 요약, 상태, 권한 및 시각을 기록합니다. 전체
...
| 에이전트 활동 | 인계, plain-text 명령/결과, Bragi 답변 순서의 블록 키트 섹션; 게시, 스트림 갱신, 편집에서 같은 블록을 보존 | 24,000-byte serialized 카드 예산 안의 같은 순서 Adaptive 카드 블록; multibyte 답변을 바이트 기준으로 제한하고 생략된 활동 수를 표시 | 같은 에이전트 귀속 및 민감정보 제거 표시가 있는 범위가 제한된 텍스트 |

Slack은 알림 및 accessibility 대체 경로를 위해 top-level `text` 필드에 전체 활동
대체 경로를 유지합니다. 블록 키트는 각 활동을 자체 섹션에 한 번만 표시하고 최종 Bragi
섹션에는 정본 답변만 전달하므로 구조화된 클라이언트가 근거를 중복 렌더링하지 않습니다.
...
잘린 정본 Teams 답변과 mrkdwn escaping 후 2,900-character 블록 키트 섹션 한도에 맞춰
...
응답 데이터는 URL, 토큰, alternate API 메서드를 제공할 수 없습니다.
...
게시 전에 fail합니다. 범위가 제한된 세션 키는 명시적으로 선택된 scalar 값의 SHA-256
다이제스트이므로 raw 외부 신원 값이 세션 id가 되지 않습니다. Projected 이벤트는 여전히
event-ingest, trust 라우팅, risk gating, 감사에 진입하며 웹훅은 액션을 execute하지 않습니다.
...
- **A2/A4:** `NotificationChannel.send(NotificationMessage) -> DeliveryReceipt`.
  `NotificationMessage.category`는 의미 경로 키이고 `trust_tier`가 A1-A4를 표현합니다.
...
- **인시던트 전달 준비 상태** - non-local control-plane 런타임은 `operational_alert` 경로에
  A2 trust 계층을 지원하는 등록 채널이 없으면 시작에 실패합니다. 명시적 로컬 Azure CLI
...
- **영속 A1 결정** - `fdai-api`는 정규화한 승인자, 결정, 증적을 이벤트 버스에
  ...
  적용합니다. 성공하면 `delivered`를 체크포인트하고 resolved 항목을 pending 큐에서 제거합니다.
...
- **비율 정책은 배포가 소유** - 테넌트별 승인자 비율, quiet 시간, fatigue 제한은 인증된
  ...
  약화하지 않습니다.
- **Approval 부하 계획은 작업을 폐기하지 않음** - 모든 A1 요청은 versioned 부하 정책이
  ...
  및 담당자 그룹 구간을 공유하거나 quiet 시간 종료까지 기다릴 수 있지만, 모든 구성원은
  승인 큐에서 독립적으로 확인하고 결정할 수 있습니다. 담당자 fatigue 상한은 그룹의
  알림 모드만 바꾸며 parking을 막거나 승인을 의미하지 않습니다. 그룹 구간 또는
  quiet 시간 경계에서 런타임은 고정된 그룹 전달 id 하나를 점유하고 현재 pending 구성원
  개수를 포함한 기준점 다이제스트 하나만 보냅니다. Deferred 그룹과 구성원은 같은 점유를 사용하므로
...
- **Reminder는 범위가 제한된 시도** - 정책은 원래 승인 만료 이전의 결정론적 오프셋을
  ...
  시도합니다. 성공과 실패를 모두 감사하며 TTL을 연장하거나 pending 항목을 제거하거나 선언된
...
- [x] 들어오는 웹훅 범위 - 인증된 `TypedWebhookMapping`만 허용 목록에 있는 이벤트/에이전트
  대상을 publish하며 페이로드는 명령, 대상, 세션을 선택할 수 없습니다.
- [ ] 아티팩트 소유자가 **게스트** 사용자일 때의 `mention-artifact-owner` 동작 (Teams에서
...
- **라벨만 현지화된다.** L0 값(결정 word, 룰 id, 리소스 유형, 모드)은 모든
