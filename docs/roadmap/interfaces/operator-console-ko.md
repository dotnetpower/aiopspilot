...
translation_source_sha: 209ecc497aed21ecc337a6c715afebadf15921be
...
WAF 체크리스트는 변경 불가능한 워크로드 근거와 함께 고정된 정의 59개를 모두 표시하고, 인접한
CAF 보기는 담당자, 주기, 승인된 예외, 근거 제한 사항과 함께 방법론 및 랜딩 존 영역 15개를
표시합니다. 어떤 보기에서도 브라우저가 규정 준수를 계산하거나 근거를 쓰지 않습니다.
지식 > 문서는 새로 고친 뒤 컬렉션 범위가 적용된 문서 목록을 제한된 개수로 복원합니다. 컬렉션을 바꾸면 다음 권한 적용 읽기 전에 이전 행을 지우며, 문서나 컬렉션 권한을 부여하지 않고 로딩, 사용 불가, 오류, 빈 상태를 서로 구분합니다. 사용할 수 없는 문서 작업은 키보드로 초점을 이동하여 그 사유를 확인할 수 있지만 활성화 처리기가 없으므로 작업을 실행할 수 없습니다.
대시보드는 로딩, 부분 완료 및 사용 불가 상태에서도 범위가 제한된 서비스 상태 답변을 보존합니다. 일반 인벤토리 데이터로 대체하지 않고 의미 변환 결과의 정확한 리소스 식별자,
...
Console shell은 헤더에 간결한 FDAI 브랜드 락업을 유지하고, 외부는 투명하고 내부는 흰색인 정사각형 브라우저 아이콘을 사용하여 밝고 어두운 브라우저 UI에서 마크를 선명하게 유지합니다. 콘텐츠 컨테이너는 calm-slate 계층에 1 px 헤어라인 테두리를 사용하며, 장식적인 색상 상단 또는 좌측 가장자리 강조는 금지됩니다. 정적 디자인 목업 검사는 현재 탐색 분류 체계를 따르고 의미를 전달하는 선 견본과 금지된 컨테이너 가장자리 강조를 구분합니다. 기본 탐색은 콘텐츠 레이아웃에서 56 px 활동 레일만 차지합니다. 그룹 버튼이나 페이지 제목의 이동 경로를 선택하면 하위 탐색기가 오버레이로 열리고, 하위 메뉴를 선택하면 닫히며, 핀을 명시적으로 고정하면 300 px 고정 영역으로 전환됩니다. 오퍼레이터는 활동 레일의 상황에 맞는 메뉴나 접근 가능한 `...` 컨트롤에서 선택적 대메뉴를 표시하거나 숨길 수 있습니다. 전체 현황, 설정, 현재 표시 중인 그룹은 숨길 수 없습니다.
헤더는 계정 ID 바로 앞에 현지화된 설명서 명령을 배치합니다. 이 명령은 `VITE_MANUAL_STUDIO_URL`로 구성한 독립 Manual Studio URL의 엄격하게 검증된 `catalog.json`으로 채운 모달 우측 패널을 엽니다. 패널은 사용할 수 있는 모든 설명서를 `01`부터 `05`까지 한 번의 연속 스크롤에 순서대로 표시합니다. 단계 제목과 차분한 구분선으로 FDAI Value-to-Scale Journey를 유지하며 별도 페이지나 필터 컨트롤을 추가하지 않습니다. 각 카드는 4:5 책 표지를 중앙에 배치하고 설명서 소개만 카드 전체 폭을 사용하는 짧은 추천 문구로 아래에 표시합니다. 설명서 종류, `L100`부터 `L400`까지의 깊이, 단계, 제목, 예상 시간, 슬라이드 수는 책 표지에서만 표시하며 깊이 값이 없으면 `L100`을 기본값으로 사용합니다. 독립 라이브러리는 같은 정보 모델을 유지하면서 더 넓은 화면 구성을 사용할 수 있습니다. 표지 자산은 구성된 기준 경로 안에 있어야 하며, 설명서를 선택하면 HTML 슬라이드 프레젠테이션이 새 탭에서 열립니다. Manual Studio 서버는 `MANUAL_STUDIO_ALLOWED_ORIGINS`에 명시된 출처의 카탈로그 읽기만 허용합니다. 카탈로그를 사용할 수 없거나 검증할 수 없으면 그 상태를 명시적으로 표시하며 런타임 권한을 부여하지 않습니다.
