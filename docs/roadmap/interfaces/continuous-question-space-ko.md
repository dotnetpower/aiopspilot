...
translation_source_sha: 0324cef0bf360e40db6d1313bd0b47dbbcac19c4
...
멤버 목록으로 반환하지 않습니다. 일반 멤버 집합에서는 `authorization.role-assignment`를
제외하며, 명시적인 IAM 질문은 전용 근거 경로를 유지합니다.
...
| 공유 의미 판단 경계 | implemented | `core/conversation/semantic_judgment.py`, `fdai_service_contracts/semantic_judgment.py`, Azure 모델 adapter 및 집중 경계 보증 | 범위가 제한된 context와 selector 순서를 보존한 32 KiB 후보 기능 변환 결과가 스키마로 검증된 intent, target, facet, confidence, ambiguity, discourse mode, action posture를 생성합니다. 검증된 preflight Resource 컬렉션 타입은 재사용 뒤 value-group binding과 결속되지 않은 필터의 결정론적 명확화를 유지합니다. 수락되지 않은 Resource 이벤트 이력 제안은 다음 frame 모델의 context만 축소할 수 있습니다. 의도 map 일치는 명시적인 bool을 반환합니다. Unbound, unavailable, malformed, ambiguous, low-confidence 결과는 명시적으로 유지되고 exact-source, knowledge-status, reference-context 또는 phrase classifier로 fallback하지 않습니다. Exact capability availability, query verification, policy, authorization, execution은 결정론적으로 유지됩니다. |
...
| 2026-09-09 | implemented | 실제 요청이 judgment 및 frame profile 한도를 모두 넘은 뒤 전체 판단 기능 변환 결과를 제한하고 권한을 추가하지 않는 Resource 이벤트 이력 context 축소를 추가했습니다. | `current change`, 집중 서술자 한도 및 safe-unaccepted 검사, 이벤트 이력 회귀, 정적 gate, 인증된 원문 및 유사 질문 검사 | 모든 hard-zero 지표가 0인 clean-source CQAS 점수 95% 초과 근거를 보존합니다. |
...
| 2026-09-05 | implemented | 병합된 Console 영어 및 한국어 카탈로그가 변경된 뒤 machine question bank와 human review catalog를 다시 생성했습니다. 질문 identity 350개를 모두 보존하고 정확한 source digest만 갱신했습니다. | `current change`, 공식 question-bank generator 및 집중 generated-artifact 계약 검사 7개 통과 | 이후 catalog 변경은 같은 변경에서 두 artifact를 모두 다시 생성해야 합니다. |
