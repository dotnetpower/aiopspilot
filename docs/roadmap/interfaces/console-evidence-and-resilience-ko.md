...
translation_source_sha: d7b6fd30a7e57dcb5dfc94935a4d71faad6e1c5c
...
Command Deck은 전송 토큰을 명시적인 초안 상태로만 표시하고 증적에 결속된 수정본 또는 확정 구간은 도착 즉시 렌더링합니다. 이후 스트림이 잘못되거나 중단되거나 순서 공백이 있거나 최종 프레임이 누락되면 초안을 철회하고 성공처럼 보이는 부분 답변 대신 사용 불가 결과를 표시합니다. 서버가 소유한 최종 대화 연결 정보는 다음 턴 전에 영속 인덱스와 활성 메모리 내 메타데이터에 모두 반영됩니다. 검증된 답변은 의미 증적의 근거 상태도 유지합니다. 주장 검사를 통과했더라도 오래되거나 불완전하거나 충돌하거나 사용할 수 없는 근거는 주의 상태로 표시합니다. 인용이 비어 있으면 중립적인 근거 출처 레이블을 사용하고 잘못된 대화 사전 검사 출력에는 플래너 복구 안내를 표시합니다.
공통 페이지 제목은 영역과 패널 레이블이 다를 때 `전체 현황 / Dashboard`를 포함해 둘을 함께 렌더링합니다. 패널 제목이 영역 레이블을 반복하는 영역 루트와 독립 utility는 단일 제목을 유지합니다.
공통 상단 표시줄은 아이콘 전용 FDAI 마크를 원본 색상으로 렌더링하고 옆에 `FDAI Console` 워드마크를 표시합니다. 콘솔 테마는 브랜드 자산의 채도를 낮추거나 색을 변경하지 않습니다.
Overview 분석 및 모든 운영 하위 경로에서는 인시던트 주의 항목 앞에 `Sample | Live`를 표시합니다. 초기 기본값은 `Live`이며, `Sample`은 URL에 `data=sample`을 기록하고 탭에서 유지하며 전체 변환 결과를 결정적인 일반 예시로 교체하고 운영 근거가 아니라는 페이지 배너를 표시합니다.
GET 전용 Sample 클라이언트가 인증이나 네트워크 접근 없이 일반 변환 결과를 제공합니다. Sample Live는 60초 구간에 결정적인 종단 제어 루프 180개를 분산해 초기화한 뒤 초당 하나의 고유 루프를 시작합니다. 수집, 경로 선택, 검증, 게이트, 해당하는 경우 실행, 감사 단계를 800ms 간격으로 표시합니다. 최신 카드 30개는 위치를 고정한 채 상태, 진행률, 담당 에이전트 및 짧은 상단 업데이트 신호만 변경합니다. 권위 있는 Live는 12개 상한을 유지합니다. Live, 프로비저닝, 예약 연속 작업은 범위가 제한된 로컬 예시를 사용합니다. Sample은 인시던트 개입을 제거하고 모든 POST를 차단하며 API 실패로 자동 활성화되거나 실제 값과 혼합되지 않습니다. 지원하지 않는 경로는 컨트롤을 숨기고 Live를 유지합니다.
실제 운영도 `운영 / 실시간`과 같은 공통 title 계약을 따릅니다. 관찰 컨트롤은 공통 헤더 actions 영역에 유지되고 좁은 뷰포트에서는 제목 아래로 줄바꿈되어 화면 고정, 출처, 구간 및 연결 상태가 계속 표시됩니다.
열린 SSE 응답은 전송 연결만 증명합니다. 실제 운영은 검증된 최신 Huginn 런타임 heartbeat 또는 권한 있는 런타임이나 재생 단계 프레임을 관찰한 뒤에만 출처가 준비되었다고 표시합니다. 런타임 소스 표시는 새 관찰 없이 15초가 지나면 만료되며 제어 루프 타일을 만들지 않습니다. keepalive만 있는 연결은 `소스 대기`를 렌더링하고 운영 메트릭을 사용 불가 상태로 유지하며, 0을 측정된 상태로 제시하는 대신 Core 런타임과 단계 토픽 준비 상태를 확인하도록 안내합니다. Operator API는 허용된 단계 프레임을 최대 256개까지 60초 동안 유지하고 새 Live 구독자가 연결되면 관찰된 순서로 재생합니다. 이 프로세스 내부 복구는 현재 Live 구간만 채우고 Operator API를 다시 시작하면 초기화되며 이력을 대체하지 않습니다. 기본 보기는 제한된 12개 작업 풀의 흐름입니다.
...
Command Deck은 모듈 조회 전에 산출물 전체를 검증합니다. 레지스트리는 닫힌 블록 종류마다 Preact
컴포넌트, 반응형 정책, 접근성 대체 경로를 연결하며 서술에서 표, 차트, 상태 또는 단위를 추론하지 않습니다.
오프라인 및 운영 빌드는 Mermaid 파싱과 DOM 정화 의존성을 잠급니다. 제공 그래프에 알려진 Moderate
이상의 보안 권고가 있으면 Console 테스트, 형식 검사, 오프라인 빌드 및 감사 통과 전까지 release를 차단합니다.

| 블록 종류 | 모듈 책임 | 접근성 및 대체 경로 |
|----------|-----------|----------------------|
| `summary`, `callout`, `evidence` | KPI 사실, 제한, 출처 및 증적 | 기본 description list 또는 list를 사용하고 정확한 텍스트를 표시합니다. |
| `table`, `threshold_table`, `list` | 정확한 레코드와 비교 | 기본 table/list 의미 규칙을 사용하고 좁은 배치에서는 값을 바꾸지 않고 레이블을 다시 배치합니다. |
| `bar`, `coverage` | 범위가 제한된 범주 값 또는 검증된 구성비 | 보이는 값과 의미 설명을 제공하고 필요하면 인접한 정확한 값 표를 사용합니다. |
| `time_series` | 정렬된 단일 메트릭과 단위 | 초점을 받을 수 있는 점, 설명, 정확한 시각/값 표를 제공하며 모션에 의존하지 않습니다. |
| `comparison` | 기준선/현재/목표 또는 이전/이후 | 역할 레이블, 단위, 정확한 값 및 표 대체 경로를 제공합니다. |
| `timeline` | 근거가 되는 순서 | 기계 시각을 `time` 요소에 보존하는 순서 목록을 사용합니다. |

레지스트리는 파서 수락 뒤에만 컴포넌트를 선택합니다. 잘못된 v2 입력, 결속되지 않은 참조, 중복
슬롯, 유한하지 않은 값, 시각 순서나 단위 오류, 필수 대체 경로 누락은 산출물을 차단하고 정본 텍스트를 유지합니다.

차트는 캔버스 전용 화면 대신 CSS와 의미가 있는 HTML을 사용합니다. 툴팁이 있는 모든 데이터
점은 키보드로 접근할 수 있으며 보이는 툴팁은 새 텍스트를 만들지 않고 정확한 값을 반복합니다.
`prefers-reduced-motion`은 데이터를 숨기지 않고 전환 효과만 제거합니다. 고정된 크기와 overflow
정책은 레이블, 긴 한국어, 시각 및 불투명 식별자가 대화 기록을 움직이거나 겹치지 않게 합니다.
...
cell을 줄바꿈합니다.
한 번 호출하는 일반 지식 답변이 최종 스키마, 로캘, 프로필 및 권한 없음 검사를 통과하면 브라우저는
전체 문단을 한 번에 표시하지 않고 같은 범위가 제한된 토큰 콜백과 그리기 큐로 정본 텍스트를
전달합니다. 프로바이더의 부분 JSON이나 검증 전 문장은 노출하지 않습니다. 인증된 표준 포트 데스크톱
검사에서 답변은 3.959초에 표시되기 시작해 답변 텍스트 그리기 13단계를 거쳐 4.415초에 완료됐습니다.
진행 시간 표시를 포함한 전체 대화 기록은 53번 변경됐습니다.
...
메타데이터를 기록하며 답변 본문을 반복하지 않습니다. 범위가 제한된 Resource 목록이 잘리면 기술 세부에는 반환된 동일 행과 반환 개수, 전체 개수 및 잘림 메타데이터를 유지합니다. 제한 문구는 반환되지 않은 행이 브라우저 페이로드에 있다고 약속하지 않습니다.
...
Exact-answer 영속 재생에는 같은 범위가 제한된 브라우저 파서를 사용합니다. 서버는 프로바이더의 최종 content-policy 결정이 확인될 때까지 모델 토큰을 buffering합니다. 블록은 부분 토큰 또는 assistant 답변을 노출하지 않고 내용이 없는 증적만 기록하며 SSE와 JSON `422`에 같은 결정론적 대체 경로를 사용하고, 로그에는 단계와 집계 개수만 남깁니다. 명시적인 프로바이더 거절, 잘린 완료, malformed 스트림 프레임 또는 검증된 최종 신호 없이 끝난 스트림은 assistant 답변이 되지 않습니다. Operator가 로컬 권한 검사에서 `semantic_answer_verified`로 검증된 것으로 표시한 영속 의미 변환 결과를 이미 보유한 경우에만 해당 의미 증적, 근거 참조 및 구간별 재생 커서를 담은 누적 `confirmed` 구간을 `done` 전에 보낼 수 있습니다. 로컬에서 만든 문서 준비 또는 사용 불가 문구는 최종 단계에서만 표시하며 원래 답변의 확인 상태를 이어받지 않습니다. 브라우저는 순서 공백 뒤의 확인 구간을 표시하지 않고, 유효하지 않거나 다른 요청에 결속된 증적을 차단하며, 이전 개정을 무시합니다. 수정이나 취소가 발생하면 이전 텍스트와 확인 메타데이터를 즉시 지우고 새 개정이 자체 검증 증적을 설정할 수 있습니다. 최종 검증 상태, 근거 참조, 증적, 텍스트, 개정 번호, 순서 또는 스트림 무결성이 다르면 구간을 철회합니다.
...
freshness, 완전성, 충돌, 잘림 및 근거 참조를 보존하는 별도 권한 확인 컨텍스트 스냅샷을 통해서만 Console에 들어옵니다. WARA 컨트롤 변환 결과는 자문 카탈로그 데이터에 같은 출처 이력 규칙을 적용합니다. 상세 보기는 고정된 APRL 출처와 선택적 Learn 페이지를 연결하고 버전, 리비전, 다이제스트, 가져온 시각, 라이선스, 쿼리 다이제스트를 표시하며, URL에 오프셋을 보존하는 50개 행 페이지에서 권고, 매핑, 평가, 충족 상태, 출처 ID가 넘치지 않도록 유지합니다.
...
| 알아볼 내용 | 읽을 문서 |
|-------------|-----------|
| 구현 상태 및 남은 작업 | [구현 원장](../../roadmap-implementation/interfaces/console-evidence-and-resilience.md) |
