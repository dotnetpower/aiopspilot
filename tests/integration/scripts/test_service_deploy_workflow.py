...
def test_legacy_platform_imports_the_service_specific_core_image() -> None:
    ...
    assert 'docker_auth_config="$docker_config/config.json"' in _IMAGE_BINDER
    assert 'chmod 0700 "$docker_config"' in _IMAGE_BINDER
    assert "base64.b64encode(credential)" in _IMAGE_BINDER
    assert 'chmod 0600 "$netrc_file" "$docker_auth_config"' in _IMAGE_BINDER
