...
def test_core_runtime_role_and_forward_grants_cover_only_core_owned_tables() -> None:
    ...
    executor_reservation_path = (
        MIGRATION_ROOT / "branches/core-control-plane/versions/"
        "20260910_core_executor_idempotency_reservation.py"
    )
    executor_reservation_migration = inventory_module.load_revision_metadata(
        executor_reservation_path
    )
    executor_audit_intent_path = (
        MIGRATION_ROOT / "branches/core-control-plane/versions/"
        "20260910_core_executor_audit_intent.py"
    )
    executor_audit_intent_migration = inventory_module.load_revision_metadata(
        executor_audit_intent_path
    )
    target_dispatch_fence_path = (
        MIGRATION_ROOT / "branches/core-control-plane/versions/"
        "20260910_core_target_dispatch_fence.py"
    )
    target_dispatch_fence_migration = inventory_module.load_revision_metadata(
        target_dispatch_fence_path
    )
    safeguard_dispatch_evidence_path = (
        MIGRATION_ROOT / "branches/core-control-plane/versions/"
        "20260911_core_safeguard_dispatch_evidence.py"
    )
    ...
    granted_tables = (
        ...
        | set(executor_reservation_migration.owned_tables)
